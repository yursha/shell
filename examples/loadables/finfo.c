...
static int octal(s) char *s;
{
  int r;

  r = *s - '0';
  while (*++s >= '0' && *s <= '7') r = (r * 8) + (*s - '0');
  return r;
}

static int finfo_main(argc, argv) int argc;
char **argv;
{
  register int i;
  int mode, flags, opt;
...
static struct stat *getstat(f) char *f;
{
...
static int printfinfo(f) char *f;
{
  struct stat *st;
...
static void perms(m) int m;
{
  ...
  i = 0;
  if (m & S_IRUSR) ubits[i++] = 'r';
  if (m & S_IWUSR) ubits[i++] = 'w';
  if (m & S_IXUSR) ubits[i++] = 'x';
  ubits[i] = '\0';

  i = 0;
  if (m & S_IRGRP) gbits[i++] = 'r';
  if (m & S_IWGRP) gbits[i++] = 'w';
  if (m & S_IXGRP) gbits[i++] = 'x';
  gbits[i] = '\0';

  i = 0;
  if (m & S_IROTH) obits[i++] = 'r';
  if (m & S_IWOTH) obits[i++] = 'w';
  if (m & S_IXOTH) obits[i++] = 'x';
  obits[i] = '\0';

  if (m & S_ISUID) ubits[2] = (m & S_IXUSR) ? 's' : 'S';
  if (m & S_ISGID) gbits[2] = (m & S_IXGRP) ? 's' : 'S';
  if (m & S_ISVTX) obits[2] = (m & S_IXOTH) ? 't' : 'T';
...
#ifdef S_ISWHT
  if (S_ISWHT(mode)) printf("S_ISWHT ");
#endif
  perms(getperm(mode));
  printf("\n");
}

static int printst(st) struct stat *st;
{
  struct passwd *pw;
  struct group *gr;
  char *owner;
...
static int printsome(f, flags) char *f;
int flags;
{
  struct stat *st;
  struct passwd *pw;
...
#ifndef NOBUILTIN
int finfo_builtin(list) WORD_LIST *list;
{
  int c, r;
  char **v;
  WORD_LIST *l;
...
char *base_pathname(p) char *p;
{
  char *t;

  if (t = strrchr(p, '/')) return (++t);
  return (p);
}
...
long *result;
{
  ...
  while (digit(*string)) {
    if (result) value = (value * 10) + digit_value(*string);
    string++;
  }
