...
int id_builtin(list) WORD_LIST *list;
{
  int opt;
  char *user;
  ...
  if (inituser(user) < 0) return (EXECUTION_FAILURE);
...
static int inituser(uname) char *uname;
{
  struct passwd *pwd;
...
static int id_pruser(uid) int uid;
{
...
static int id_prgrp(gid) int gid;
{
...
static int id_prgroups(uname) char *uname;
{
  int *glist, ng, i, r;
...
static int id_prall(uname) char *uname;
{
  int r, i, ng, *glist;
  struct passwd *pwd;
  struct group *grp;
...
struct builtin id_struct = {"id", id_builtin, BUILTIN_ENABLED, id_doc,
                            "id [user]\n\tid -G [-n] [user]\n\tid -g [-nr] "
                            "[user]\n\tid -u [-nr] [user]",
                            0};
