...
static const yytype_uint16 yyrline[] = {
    0,    363,  363,  374,  383,  398,  408,  410,  414,  420,  426,  432,
    438,  444,  450,  456,  462,  468,  474,  480,  486,  492,  498,  504,
    511,  518,  525,  532,  539,  546,  552,  558,  564,  570,  576,  582,
    588,  594,  600,  606,  612,  618,  624,  630,  636,  642,  648,  654,
    660,  666,  672,  678,  686,  688,  690,  694,  698,  709,  711,  715,
    717,  719,  735,  737,  741,  743,  745,  747,  749,  751,  753,  755,
    757,  759,  761,  765,  770,  775,  780,  785,  790,  795,  800,  807,
    812,  817,  822,  829,  834,  839,  844,  849,  854,  861,  866,  871,
    878,  881,  884,  888,  890,  921,  928,  933,  950,  955,  972,  979,
    981,  983,  988,  992,  996,  1000, 1002, 1004, 1008, 1009, 1013, 1015,
    1017, 1019, 1023, 1025, 1027, 1029, 1031, 1033, 1037, 1039, 1048, 1056,
    1057, 1063, 1064, 1071, 1075, 1077, 1079, 1086, 1088, 1090, 1094, 1095,
    1098, 1100, 1102, 1106, 1107, 1116, 1129, 1145, 1160, 1162, 1164, 1171,
    1174, 1178, 1180, 1186, 1192, 1211, 1233, 1235, 1258, 1262, 1264, 1266};
#endif
    ...
    while (1) {
      ...
      if (shell_input_line_size > (SIZE_MAX - 256)) {
        ...
        if (n <= 2)       /* we have to save 1 for the newline added below */
          ...
          if (truncating == 0)
            internal_warning(
                "shell_getc: shell_input_line_size (%zu) exceeds SIZE_MAX "
                "(%lu): line truncated",
...
int *lenp, flags;
{
  ...
  while (count) {
    ...
    if
      MBTEST(posixly_correct && dolbrace_state != DOLBRACE_QUOTE &&
        ...
        MBTEST(shellquote(ch)) {
          ...
          if
            MBTEST((tflags & LEX_WASDOL) && ch == '\'' &&
              ...
              /* FLAG POSIX INTERP 221 */
              if ((rflags & P_DQUOTE) && (dolbrace_state == DOLBRACE_QUOTE2) &&
                  (flags & P_DOLBRACE)) {
  ...
  comsub_readchar:
    ...
    if (tflags & LEX_RESWDOK) {
      if
        ...
        MBTEST(
            (tflags & LEX_CKCOMMENT) && ch == '#' &&
            (lex_rwlen == 0 || ((tflags & LEX_INWORD) &&
...
static COND_COM *cond_term() {
  ...
  } else if (tok == WORD) /* left argument to binary operator */
  {
    ...
    if (tok < 256)
      parser_error(line_number, "unexpected token `%c' in conditional command",
                   tok);
    else if (etext = error_token_from_token(tok)) {
      parser_error(line_number, "unexpected token `%s' in conditional command",
                   etext);
      free(etext);
    } else
      parser_error(line_number, "unexpected token %d in conditional command",
                   tok);
    COND_RETURN_ERROR();
  }
  return (term);
