...
void initialize_aliases() {
  if (aliases == 0) aliases = hash_create(ALIAS_HASH_BUCKETS);
}

/* Scan the list of aliases looking for one with NAME.  Return NULL
   if the alias doesn't exist, else a pointer to the alias_t. */
alias_t *find_alias(name) char *name;
{
  BUCKET_CONTENTS *al;

  if (aliases == 0) return ((alias_t *)NULL);
...
char *get_alias_value(name) char *name;
{
  alias_t *alias;

  if (aliases == 0) return ((char *)NULL);
...
void add_alias(name, value) char *name, *value;
{
  ...
  if (temp) {
    ...
    if (n == ' ' || n == '\t') temp->flags |= AL_EXPANDNEXT;
  } else {
...
/* Remove the alias with name NAME from the alias table.  Returns
   ...
   exist. */
int remove_alias(name) char *name;
{
  BUCKET_CONTENTS *elt;
...
static alias_t **map_over_aliases(function) sh_alias_map_func_t *function;
{
  register int i;
...
static int qsort_alias_compare(as1, as2) alias_t **as1, **as2;
{
  int result;
...
alias_t **all_aliases() {
  ...
  if (aliases == 0 || HASH_ENTRIES(aliases) == 0) return ((alias_t **)NULL);

  list = map_over_aliases((sh_alias_map_func_t *)NULL);
  if (list) sort_aliases(list);
  return (list);
}

char *alias_expand_word(s) char *s;
{
  alias_t *r;
...
static int skipquotes(string, start) char *string;
int start;
{
  register int i;
...
/* Skip the white space and any quoted characters in STRING, starting at
   ...
   have been skipped. */
static int skipws(string, start) char *string;
int start;
{
  register int i;
  ...
  for (i = start; string[i]; i++) {
    ...
    if (quote_char(string[i])) {
      ...
      peekc = string[i + 1];
      if (ISLETTER(peekc)) backslash_quoted_word++;
      continue;
    }

    /* If we're in the middle of some kind of quoted word, let it
       pass through. */
    if (backslash_quoted_word) continue;
...
static int rd_token(string, start) char *string;
int start;
{
  register int i;
...
char *alias_expand(string) char *string;
{
  ...
  for (;;) {
    ...
    if (mbschr(token, '\\')) expand_this_token = 0;
