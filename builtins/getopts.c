...
void getopts_reset(newind) int newind;
{
  sh_optind = newind;
  sh_badopt = 0;
}

static int getopts_unbind_variable(name) char *name;
{
#if 0
...
static int getopts_bind_variable(name, value) char *name, *value;
{
  SHELL_VAR *v;

  if (legal_identifier(name)) {
    v = bind_variable(name, value, 0);
    if (v && (readonly_p(v) || noassign_p(v))) return (EX_MISCERROR);
...
static int dogetopts(argc, argv) int argc;
char **argv;
{
  ...
  if (argc > 1) {
    ...
    for (words = rest_of_args; words; words = words->next, i++)
      ;
    v = strvec_create(i + 1);
    for (i = 0; i < 10 && dollar_vars[i]; i++) v[i] = dollar_vars[i];
    for (words = rest_of_args; words; words = words->next, i++)
      v[i] = words->word->word;
    v[i] = (char *)NULL;
  ...
  if (special_error) sh_opterr = old_opterr;
...
int getopts_builtin(WORD_LIST *list) {
  char **av;
  int ac, ret;
