...
/* Here is the kill builtin.  We only have it so that people can type
   ...
   can still kill some. */
int kill_builtin(WORD_LIST *list) {
  ...
  if (listing) return (display_signal_list(list, 0));
  ...
  while (list) {
    ...
    if (*word && legal_number(list->word->word, &pid_value) &&
      ...
      builtin_error("%s: arguments must be process or job IDs",
                    list->word->word);
      CONTINUE_OR_FAIL;
    } else if (*word)
    ...
    { /* Must be a job spec.  Check it out. */
      ...
      if (INVALID_JOB(job)) {
        if (job != DUP_JOB) sh_badjob(list->word->word);
        UNBLOCK_CHILD(oset);
        CONTINUE_OR_FAIL;
      }
...
static void kill_error(pid, e) pid_t pid;
int e;
{
  char *x;

  x = strerror(e);
  if (x == 0) x = "Unknown error";
