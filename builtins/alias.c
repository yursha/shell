...
int alias_builtin(WORD_LIST *list) {
  ...
  if (list == 0 || pflag) {
    if (aliases == 0) return (EXECUTION_SUCCESS);

    alias_list = all_aliases();

    if (alias_list == 0) return (EXECUTION_SUCCESS);
    ...
    if (list == 0) return (sh_chkwrite(EXECUTION_SUCCESS));
  }
...
int unalias_builtin(WORD_LIST *list) {
...
static void print_alias(alias, flags) alias_t *alias;
int flags;
{
  char *value;
