...
char *posix_builtins[] = {"alias",   "bg",   "cd",        "command", "false",
                          "fc",      "fg",   "getopts",   "jobs",    "kill",
                          "newgrp",  "pwd",  "read",      "true",    "umask",
                          "unalias", "wait", (char *)NULL};
...
char **argv;
{
  ...
  while (arg_index < argc && argv[arg_index][0] == '-') {
    ...
    if (strcmp(arg, "-externfile") == 0)
      ...
      if (len && error_directory[len - 1] != '/') strcat(error_directory, "/");
  ...
  if (!only_documentation) {
    ...
    if (struct_filename) {
      ...
      if (!structfile) file_error(temp_struct_filename);
    }

    if (extern_filename) {
      externfile = fopen(extern_filename, "w");

      if (!externfile) file_error(extern_filename);
    }
  ...
  if (!only_documentation) {
    ...
    if (externfile) fclose(externfile);
  }
...
ARRAY *array_create(width) int width;
{
  ARRAY *array;
...
ARRAY *copy_string_array(array) ARRAY *array;
{
  register int i;
  ARRAY *copy;
...
HANDLER_ENTRY *find_directive(directive) char *directive;
{
  register int i;

  for (i = 0; handlers[i].directive; i++)
    if (strcmp(handlers[i].directive, directive) == 0) return (&handlers[i]);
...
FILE *structfile, *externfile;
{
  ...
  if (stat(filename, &finfo) == -1) file_error(filename);

  fd = open(filename, O_RDONLY, 0666);

  if (fd == -1) file_error(filename);

  file_size = (size_t)finfo.st_size;
  buffer = xmalloc(1 + file_size);

  if ((nr = read(fd, buffer, file_size)) < 0) file_error(filename);

  /* This is needed on WIN32, and does not hurt on Unix. */
  if (nr < file_size) file_size = nr;
  ...
  if (defs->output) fclose(defs->output);
...
static void free_builtin(builtin) BUILTIN_DESC *builtin;
{
  ...
  if (builtin->longdoc) array_free(builtin->longdoc);
...
void free_defs(defs) DEF_FILE *defs;
{
  ...
  if (defs->production) free(defs->production);

  if (defs->lines) array_free(defs->lines);
...
char *strip_whitespace(string) char *string;
{
...
char *get_arg(for_whom, defs, string) char *for_whom, *string;
DEF_FILE *defs;
{
  char *new;
  ...
  if (!*new) line_error(defs, "%s requires an argument", for_whom);
...
BUILTIN_DESC *current_builtin(directive, defs) char *directive;
DEF_FILE *defs;
{
...
char *line;
{
  ...
  if (!*line && !builtin->longdoc) return;

  if (!builtin->longdoc) builtin->longdoc = array_create(sizeof(char *));

  array_add(line, builtin->longdoc);
}

/* How to handle the $BUILTIN directive. */
int builtin_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  BUILTIN_DESC *new;
  ...
  if (!defs->builtins) defs->builtins = array_create(sizeof(BUILTIN_DESC *));
  ...
  if (is_special_builtin(name)) new->flags |= BUILTIN_FLAG_SPECIAL;
  if (is_assignment_builtin(name)) new->flags |= BUILTIN_FLAG_ASSIGNMENT;
  if (is_localvar_builtin(name)) new->flags |= BUILTIN_FLAG_LOCALVAR;
  if (is_posix_builtin(name)) new->flags |= BUILTIN_FLAG_POSIX_BUILTIN;
...
int function_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  register BUILTIN_DESC *builtin;
  ...
  if (builtin->function)
    line_error(defs, "%s already has a function (%s)", builtin->name,
               builtin->function);
  else
...
int docname_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  register BUILTIN_DESC *builtin;
  ...
  if (builtin->docname)
    line_error(defs, "%s already had a docname (%s)", builtin->name,
               builtin->docname);
  else
...
int short_doc_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  register BUILTIN_DESC *builtin;
...
int comment_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{ return (0); }

/* How to handle the $DEPENDS_ON directive. */
int depends_on_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  register BUILTIN_DESC *builtin;
...
int produces_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
  /* If just hacking documentation, don't change any of the production
     files. */
  if (only_documentation) return (0);
  ...
  if (defs->production)
    ...
    if (inhibit_production) return (0);

    defs->output = fopen(defs->production, "w");

    if (!defs->output) file_error(defs->production);
...
int end_handler(self, defs, arg) char *self;
DEF_FILE *defs;
char *arg;
{
...
static char *xmalloc(bytes) int bytes;
{
...
static char *xrealloc(pointer, bytes) char *pointer;
int bytes;
{
  char *temp;
...
BUILTIN_DESC *copy_builtin(builtin) BUILTIN_DESC *builtin;
{
  BUILTIN_DESC *new;
...
void save_builtin(builtin) BUILTIN_DESC *builtin;
{
  ...
  /* If this is the first builtin to be saved, create the array
     to hold it. */
  if (!saved_builtins) saved_builtins = array_create(sizeof(BUILTIN_DESC *));
...
char *structfile_header[] = {
    ...
    "   long-doc, short-doc.  The long-doc field contains a pointer to an "
    "array",
    "   of help lines.  The function takes a WORD_LIST *; the first word in "
    "the",
    ...
    "   (array[i].function == (sh_builtin_func_t *)NULL).  Such entries are "
    "for",
...
char *structfile_footer[] = {
    ...
    "struct builtin *current_builtin;", "", "int num_shell_builtins =",
  ...
  if (defs->builtins) {
    ...
    for (i = 0; i < defs->builtins->sindex; i++) {
      ...
      if (!only_documentation) {
        ...
        if (structfile) {
          ...
          /* Don't translate short document summaries that are identical
             to command names */
          if (builtin->shortdoc &&
              strcmp(builtin->name, builtin->shortdoc) == 0) {
      ...
      if (documentation_file) {
        ...
        write_documentation(documentation_file, builtin->longdoc->array, 0,
                            TEXINFO);
      }
    }
  }
...
int indentation, flags;
{
  ...
  base_indent = (string_array && single_longdoc_strings && filename_p == 0)
                    ? BASE_INDENT
                    : 0;

  for (i = 0, texinfo = (flags & TEXINFO);
    ...
    if (indentation)
      for (j = 0; j < indentation; j++) fprintf(stream, " ");

    /* Don't indent the first line, because of how the help builtin works. */
    if (i == 0) indentation += base_indent;
...
int write_helpfiles(builtins) ARRAY *builtins;
{
  char *helpfile, *bname;
  FILE *helpfp;
  int i, hdlen;
...
static int _find_in_table(name, name_table) char *name, *name_table[];
{
  register int i;

  for (i = 0; name_table[i]; i++)
    if (strcmp(name, name_table[i]) == 0) return 1;
  return 0;
}

static int is_special_builtin(name) char *name;
{ return (_find_in_table(name, special_builtins)); }

static int is_assignment_builtin(name) char *name;
{ return (_find_in_table(name, assignment_builtins)); }

static int is_localvar_builtin(name) char *name;
{ return (_find_in_table(name, localvar_builtins)); }

static int is_posix_builtin(name) char *name;
{ return (_find_in_table(name, posix_builtins)); }

#if !defined(HAVE_RENAME)
static int rename(from, to) char *from, *to;
{
  unlink(to);
