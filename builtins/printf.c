...
#define PF(f, func)                                                            \
  do {                                                                         \
    ...
    if (have_fieldwidth && have_precision)                                     \
      ...
      nw =                                                                     \
          vflag ? vbprintf(f, fieldwidth, func) : printf(f, fieldwidth, func); \
...
#define PRETURN(value)                        \
  do {                                        \
    ...
    if (vbsize > 4096) {                      \
      ...
      vbuf[0] = 0;                            \
    if (ferror(stdout) == 0) fflush(stdout);  \
    QUIT;                                     \
    if (ferror(stdout)) {                     \
      sh_wrerror();                           \
      clearerr(stdout);                       \
...
static int vbprintf(const char *, ...)
...
int printf_builtin(WORD_LIST *list) {
  ...
  while ((ch = internal_getopt(list, "v:")) != -1) {
    switch (ch) {
      case 'v':
        ...
        {
          vflag = 1;
          if (vbsize == 0) vbuf = xmalloc(vbsize = 16);
          vblen = 0;
          if (vbuf) vbuf[0] = 0;
        } else {
          sh_invalidid(vname);
  ...
  if (format == 0 || *format == 0) return (EXECUTION_SUCCESS);
...
/* A NULL third argument to tescape means to bypass the
   special processing for arguments to %b. */
        ...
        for (mbind = 0; mbind < mblen; mbind++) PC(mbch[mbind]);
#else
...
/* Negative precisions are allowed but treated as if the
   ...
   of asprintf/fprintf implementations get this wrong. */
      ...
      modstart = fmt;
      while (*fmt && strchr(LENMODS, *fmt)) fmt++;
      ...
      switch (convch) {
        ...
        case '(': {
          ...
          *t = '\0';
          ...
          if (timefmt[0] == '\0') {
            timefmt[0] = '%';
            timefmt[1] =
                'X'; /* locale-specific current time - should we use `+'? */
            timefmt[2] = '\0';
          }
        ...
        case 'q': /* print with shell quoting */
        {
          ...
          if (r < 0) PRETURN(EXECUTION_FAILURE);
          break;
        }
  ...
  if (conversion_error) retval = EXECUTION_FAILURE;

  PRETURN(retval);
}

static void printf_erange(s) char *s;
{ builtin_error("warning: %s: %s", s, strerror(ERANGE)); }

/* We duplicate a lot of what printf(3) does here. */
static int printstr(fmt, string, len, fieldwidth,
                    precision) char *fmt; /* format */
...
int precision;                            /* argument for precision of `*' */
{
  ...
  if (*fmt == '*') {
    ...
    mfw = *fmt++ - '0';
    while (DIGIT(*fmt)) mfw = (mfw * 10) + (*fmt++ - '0');
  ...
  if (*fmt == '.') {
    ...
    if (*fmt == '*') {
      ...
      mpr = *fmt++ - '0';
      while (DIGIT(*fmt)) mpr = (mpr * 10) + (*fmt++ - '0');
  ...
  for (; padlen > 0; padlen--) PC(' ');

  /* output NC characters from STRING */
  for (i = 0; i < nc; i++) PC(string[i]);
...
static int tescape(estart, cp, lenp, sawc) char *estart;
char *cp;
int *lenp, *sawc;
{
  register char *p;
...
static char *bexpand(string, len, sawc, lenp) char *string;
int len, *sawc, *lenp;
{
  int temp;
  char *ret, *r, *s, c;
...
#if defined(HANDLE_MULTIBYTE)
    for (mbind = 0; mbind < mblen; mbind++) *r++ = mbch[mbind];
#else
    *r++ = c;
#endif
  }
...
static char *vbadd(buf, blen) char *buf;
int blen;
{
  size_t nlen;
...
#else
    vbprintf(format, va_alist) const char *format;
va_dcl
#endif
{
  va_list args;
...
static char *mklong(str, modifiers, mlen) char *str;
char *modifiers;
size_t mlen;
{
  size_t len, slen;
...
static SHELL_VAR *bind_printf_variable(name, value, flags) char *name;
char *value;
int flags;
{
  SHELL_VAR *v;
