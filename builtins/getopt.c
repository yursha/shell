...
int sh_getopt(argc, argv, optstring) int argc;
char *const *argv;
...
char **argv;
{
  ...
  if (sh_optind < argc) {
    ...
    while (sh_optind < argc) printf("%s ", argv[sh_optind++]);
    printf("\n");
  }
