...
int trap_builtin(WORD_LIST *list) {
  ...
  if (list_signal_names)
    ...
    /* Backwards compatibility.  XXX - question about whether or not we
       ...
       32 signals).  */
    if (first_signal) operation = REVERT;
        ...
        switch (operation) {
          ...
          case REVERT:
            ...
            switch (sig) {
              ...
              case SIGTSTP:
#endif /* JOB_CONTROL */
                if (interactive) set_signal_handler(sig, SIG_IGN);
                break;
            }
            break;
...
static int display_traps(list) WORD_LIST *list;
{
  int result, i;

  if (list == 0) {
    for (i = 0; i < BASH_NSIG; i++) showtrap(i);
