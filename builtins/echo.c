...
int echo_builtin(WORD_LIST* list) {
  ...
  if (posixly_correct && xpg_echo) goto just_echo;
  ...
  for (; list && (temp = list->word->word) && *temp == '-'; list = list->next) {
    ...
    for (i = 0; temp[i]; i++) {
      if (strchr(VALID_ECHO_OPTIONS, temp[i]) == 0) break;
    }
...
just_echo:
  ...
  while (list) {
    i = len = 0;
    temp = do_v9 ? ansicstr(list->word->word, STRLEN(list->word->word), 1, &i,
                            &len)
                 : list->word->word;
    if (temp) {
      if (do_v9) {
        for (s = temp; len > 0; len--) putchar(*s++);
      } else
        printf("%s", temp);
#if defined(SunOS5)
