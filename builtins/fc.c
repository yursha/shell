...
#define FREE_RLIST()              \
  do {                            \
    for (rl = rlist; rl;) {       \
      REPL *r;                    \
      r = rl->next;               \
      if (rl->pat) free(rl->pat); \
      if (rl->rep) free(rl->rep); \
      free(rl);                   \
      rl = r;                     \
    }                             \
  } while (0)
...
static void set_verbose_flag() { echo_input_at_read = verbose_flag; }
...
int fc_builtin(WORD_LIST *list) {
  ...
  if (ename && (*ename == '-') && (ename[1] == '\0')) execute = 1;
  ...
  /* This is the second form of the command (the list-or-edit-and-rerun
     form). */
  hlist = history_list();
  if (hlist == 0) return (EXECUTION_SUCCESS);
  for (i = 0; hlist[i]; i++)
    ;
  ...
  while (hlist[real_last] == 0 && real_last > 0) real_last--;

  /* XXX */
  if (i == last_hist && hlist[last_hist] == 0)
    while (last_hist >= 0 && hlist[last_hist] == 0) last_hist--;
  if (last_hist < 0) {
  ...
  if (listing)
    ...
    if (stream == 0) {
      builtin_error("%s: cannot open temp file: %s", fn ? fn : "",
                    strerror(errno));
      FREE(fn);
      return (EXECUTION_FAILURE);
    }
  ...
  for (i = reverse ? histend : histbeg; reverse ? i >= histbeg : i <= histend;
       reverse ? i-- : i++) {
    QUIT;
    if (numbering) fprintf(stream, "%d", i + history_base);
    if (listing) {
      if (posixly_correct)
        fputs("\t", stream);
      else
  ...
  if (listing) return (sh_chkwrite(EXECUTION_SUCCESS));
...
static int fc_number(list) WORD_LIST *list;
{
  char *s;
...
static int fc_gethnum(command, hlist) char *command;
HIST_ENTRY **hlist;
{
  int sign, n, clen, rh;
  ...
  if (i == last_hist && hlist[last_hist] == 0)
    while (last_hist >= 0 && hlist[last_hist] == 0) last_hist--;
  if (last_hist < 0) return (-1);

  real_last = i;
  i = last_hist;

  /* No specification defaults to most recent command. */
  if (command == NULL) return (i);
  ...
  clen = strlen(command);
  for (j = i; j >= 0; j--) {
    if (STREQN(command, histline(j), clen)) return (j);
  }
  return (-1);
}
...
static char *fc_gethist(command, hlist) char *command;
HIST_ENTRY **hlist;
{
  int i;

  if (hlist == 0) return ((char *)NULL);
...
static char *fc_readline(stream) FILE *stream;
{
  register int c;
  ...
  if (lindex + 2 >= line_len) line = (char *)xrealloc(line, lindex + 3);
...
/* Perform the SUBS on COMMAND.
   ...
   command. */
static char *fc_dosubs(command, subs) char *command;
REPL *subs;
{
  register char *new, *t;
  register REPL *r;
...
static void fc_replhist(command) char *command;
{
  int n;
...
static void fc_addhist(line) char *line;
{
  register int n;
  ...
  if (line && *line) maybe_add_history(line); /* Obeys HISTCONTROL setting. */
}
#endif
