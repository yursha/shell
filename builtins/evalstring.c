...
int should_suppress_fork(command) COMMAND *command;
{
  return (startup_state == 2 && parse_and_execute_level == 1 &&
          running_trap == 0 && *bash_input.location.string == '\0' &&
...
void optimize_fork(command) COMMAND *command;
{
...
char *tag;
{
  ...
  if (flags & (SEVAL_NONINT | SEVAL_INTERACT)) unwind_protect_int(interactive);
...
#if defined(HISTORY)
  if (flags & SEVAL_NOHIST) bash_history_disable();
#if defined(BANG_HISTORY)
  if (flags & SEVAL_NOHISTEXP) history_expansion_inhibited = 1;
#endif /* BANG_HISTORY */
#endif /* HISTORY */
}
...
int parse_and_execute(string, from_file, flags) char *string;
...
int flags;
{
  ...
  push_stream(lreset);
  if (parser_expanding_alias()) /* push current shell_input_line */
    parser_save_alias();

  if (lreset == 0) line_number--;
  ...
  if (current_token == yacc_EOF) current_token = '\n'; /* reset_parser() ? */
  ...
  while (*(bash_input.location.string)) {
    ...
    if (code) {
      ...
      switch (code) {
        ...
        case FORCE_EOF:
        case EXITPROG:
          if (command) run_unwind_frame("pe_dispose");
    ...
    if (parse_command() == 0) {
      if ((flags & SEVAL_PARSEONLY) ||
        ...
        if (startup_state == 2 && (subshell_environment & SUBSHELL_COMSUB) &&
            *bash_input.location.string == '\0' && command->type == cm_simple &&
            !command->redirects && (command->flags & CMD_TIME_PIPELINE) == 0 &&
...
out:
  ...
  if (should_jump_to_top_level) jump_to_top_level(code);
...
int parse_string(string, from_file, flags, endp) char *string;
const char *from_file;
int flags;
char **endp;
{
  ...
  while (*(bash_input.location.string)) {
    ...
    if (code) {
      ...
      switch (code) {
        ...
        case EXITPROG:
        case DISCARD: /* XXX */
          if (command) dispose_command(command);
          /* Remember to call longjmp (top_level) after the old
             value for it is restored. */
          should_jump_to_top_level = 1;
          goto out;
    ...
    if (current_token == yacc_EOF || current_token == shell_eof_token) break;
  }
...
out:
  ...
  if (endp) *endp = bash_input.location.string;
  ...
  if (should_jump_to_top_level) {
    if (parse_and_execute_level == 0) top_level_cleanup();
    if (code == DISCARD) return -DISCARD;
...
/* Handle a $( < file ) command substitution.  This expands the filename,
   ...
   output. */
static int cat_file(r) REDIRECT *r;
{
  char *fn;
  int fd, rval;

  if (r->instruction != r_input_direction) return -1;

  /* Get the filename. */
  if (posixly_correct && !interactive_shell) disallow_filename_globbing++;
...
int evalstring(string, from_file, flags) char *string;
const char *from_file;
int flags;
{
