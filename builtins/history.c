...
int history_builtin(WORD_LIST *list) {
  ...
  if (flags & SFLAG) {
    if (list) push_history(list);
    return (EXECUTION_SUCCESS);
  }
#if defined(BANG_HISTORY)
  else if (flags & PFLAG) {
    if (list) return (expand_and_print_history(list));
...
static char *histtime(hlist, histtimefmt) HIST_ENTRY *hlist;
const char *histtimefmt;
{
  ...
  if (t)
    ...
    snprintf(
        timestr, sizeof(timestr), "%s: invalid timestamp",
...
static int display_history(list) WORD_LIST *list;
{
  register int i;
  intmax_t limit;
  HIST_ENTRY **hlist;
  ...
  if (list) {
    if (get_numeric_arg(list, 0, &limit) == 0) return (EXECUTION_FAILURE);
...
/* Remove the last entry in the history list and add each argument in
   LIST to the history. */
static void push_history(list) WORD_LIST *list;
{
  char *s;
...
/* Delete the last history entry if it was a single entry added to the
  ...
  if (remember_on_history && hist_last_line_pushed == 0 &&
      (hist_last_line_added ||
       (current_command_line_count > 0 && current_command_first_line_saved &&
...
#if defined(BANG_HISTORY)
static int expand_and_print_history(list) WORD_LIST *list;
{
  char *s;
  int r, result;
