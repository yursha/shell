...
int help_builtin(WORD_LIST *list) {
  ...
  if (glob_pattern_p(list->word->word)) {
    printf(ngettext("Shell commands matching keyword `",
    ...
    for (pass = 1, this_found = 0; pass < 3; pass++) {
      for (i = 0; name = shell_builtins[i].name; i++) {
        ...
        if (m) {
          ...
          if (sflag == 0) show_longdoc(i);
        }
      }
      if (pass == 1 && this_found == 1) break;
    }
  }
...
static int open_helpfile(name) char *name;
{
  int fd;
...
static void show_longdoc(i) int i;
{
  register int j;
  char *const *doc;
  int fd;
  ...
  if (doc && doc[0] && *doc[0] == '/' && doc[1] == (char *)NULL) {
    ...
    for (j = 0; doc[j]; j++) printf("%*s%s\n", BASE_INDENT, " ", doc[j]);
}

static void show_desc(name, i) char *name;
int i;
{
  register int j;
  char **doc, *line;
  ...
  if (usefile) free(line);
}

/* Print builtin help in pseudo-manpage format. */
static void show_manpage(name, i) char *name;
int i;
{
  register int j;
  char **doc, *line;
...
static void dispcolumn(i, buf, bufsize, width, height) int i;
char *buf;
size_t bufsize;
int width, height;
{
  ...
  dispcols = strlen(buf);
  /* two spaces */
  for (j = dispcols; j < width; j++) putc(' ', stdout);
...
static void wdispcolumn(i, buf, bufsize, width, height) int i;
char *buf;
size_t bufsize;
int width, height;
{
...
\n");
  ...
  width /= 2;
  if (width > sizeof(blurb)) width = sizeof(blurb);
  if (width <= 3) width = 40;
