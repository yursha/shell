...
int bind_builtin(WORD_LIST *list) {
  ...
  if (flags & LFLAG) rl_list_funmap_names();

  if (flags & PFLAG) rl_function_dumper(1);

  if (flags & PPFLAG) rl_function_dumper(0);

  if (flags & SFLAG) rl_macro_dumper(1);

  if (flags & SSFLAG) rl_macro_dumper(0);

  if (flags & VFLAG) rl_variable_dumper(1);

  if (flags & VVFLAG) rl_variable_dumper(0);

  if ((flags & FFLAG) && initfile) {
    if (rl_read_init_file(initfile) != 0) {
      t = printable_filename(initfile, 0);
      builtin_error("%s: cannot read: %s", t, strerror(errno));
      if (t != initfile) free(t);
      BIND_RETURN(EXECUTION_FAILURE);
    }
  }

  if ((flags & QFLAG) && fun_name) return_code = query_bindings(fun_name);

  if ((flags & UFLAG) && unbind_name) return_code = unbind_command(unbind_name);
  ...
  if (flags & XFLAG) return_code = bind_keyseq_to_unix_command(cmd_seq);

  if (flags & XXFLAG) return_code = print_unix_command_map();
...
bind_exit:
  if (saved_keymap) rl_set_keymap(saved_keymap);
...
static int query_bindings(name) char *name;
{
...
static int unbind_command(name) char *name;
{
  rl_command_func_t *function;
