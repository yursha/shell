...
int pushd_builtin(WORD_LIST *list) {
  WORD_LIST *orig_list;
  ...
  if (list == 0) {
    ...
    if (current_directory == 0) return (EXECUTION_FAILURE);
  ...
  for (flags = 0; skipopt == 0 && list; list = list->next) {
    if (ISOPTION(list->word->word, 'n')) {
      ...
      if (direction == '-') num = directory_list_offset - num;
  ...
  j = ((flags & NOCD) == 0) ? cd_builtin(skipopt ? orig_list : list)
                            : EXECUTION_SUCCESS;
  if (j == EXECUTION_SUCCESS) {
    add_dirstack_element((flags & NOCD) ? savestring(list->word->word)
                                        : current_directory);
    dirs_builtin((WORD_LIST *)NULL);
    if (flags & NOCD) free(current_directory);
...
int popd_builtin(WORD_LIST *list) {
  register int i;
  intmax_t which;
  int flags;
  char direction;
  ...
  if (which > directory_list_offset ||
  ...
  if ((direction == '+' && which == 0) ||
    ...
    i = ((flags & NOCD) == 0)
            ...
            : EXECUTION_SUCCESS;
    if (i != EXECUTION_SUCCESS) return (i);
...
int dirs_builtin(WORD_LIST *list) {
...
#define DIRSTACK_FORMAT(temp) \
  ...
  if (index_flag == 0 || (index_flag == 1 && desired_index == 0)) {
    ...
    if (temp == 0) temp = savestring("<no current directory>");
    if (vflag & 2)
...
static void pushd_error(offset, arg) int offset;
char *arg;
{
  if (offset == 0)
...
static void clear_directory_stack() {
  ...
  for (i = 0; i < directory_list_offset; i++) free(pushd_directory_list[i]);
...
/* Switch to the directory in NAME.  This uses the cd_builtin to do the work,
   ...
   been printed. */
static int cd_to_string(name) char *name;
{
  WORD_LIST *tlist;
  WORD_LIST *dir;
  int result;
...
static int change_to_temp(temp) char *temp;
{
  int tt;

  tt = temp ? cd_to_string(temp) : EXECUTION_FAILURE;

  if (tt == EXECUTION_SUCCESS) dirs_builtin((WORD_LIST *)NULL);

  return (tt);
}

static void add_dirstack_element(dir) char *dir;
{
...
static int get_dirstack_index(ind, sign, indexp) intmax_t ind;
int sign, *indexp;
{
  if (indexp) *indexp = sign > 0 ? 1 : 2;
...
char *get_dirstack_from_string(string) char *string;
{
  ...
  if (legal_number(string, &i) == 0) return ((char *)NULL);
...
char *get_dirstack_element(ind, sign) intmax_t ind;
int sign;
{
  int i;
...
void set_dirstack_element(ind, sign, value) intmax_t ind;
int sign;
char *value;
{
  int i;
  ...
  i = get_dirstack_index(ind, sign, (int *)NULL);
  if (ind == 0 || i < 0 || i > directory_list_offset) return;
  free(pushd_directory_list[i]);
  pushd_directory_list[i] = savestring(value);
}

WORD_LIST *get_directory_stack(flags) int flags;
{
  register int i;
  WORD_LIST *ret;
  char *d, *t;
...
struct builtin popd_struct = {"popd",   popd_builtin,          BUILTIN_ENABLED,
                              popd_doc, "popd [+N | -N] [-n]", 0};

struct builtin dirs_struct = {
    "dirs", dirs_builtin, BUILTIN_ENABLED, dirs_doc, "dirs [-clpv] [+N] [-N]",
    0};
