...
static int setpwd(dirname) char *dirname;
{
  int old_anm;
  SHELL_VAR *tvar;
  ...
  if (tvar && readonly_p(tvar)) return EXECUTION_FAILURE;
...
#undef tcwd
  ...
  if (tvar && readonly_p(tvar)) r = EXECUTION_FAILURE;
  ...
  if (setpwd(dirname) == EXECUTION_FAILURE) r = EXECUTION_FAILURE;
  if (dirname == 0 && eflag) r = EXECUTION_FAILURE;

  if (dirname && dirname != the_current_working_directory) free(dirname);
...
static char *resetpwd(caller) char *caller;
{
  char *tdir;
...
static int cdxattr(dir,
                   ndirp) char *dir; /* don't assume we can always free DIR */
...
int cd_builtin(WORD_LIST *list) {
  ...
  if (eflag && no_symlinks == 0) eflag = 0;
  ...
  if (change_to_directory(dirname, no_symlinks, xattrflag)) {
    if (lflag & LCD_PRINTPATH) printf("%s\n", dirname);
    return (bindpwd(no_symlinks));
  }
...
int pwd_builtin(WORD_LIST *list) {
  char *directory;
  int opt, pflag;
...
#define tcwd the_current_working_directory
  ...
  if ((tcwd && directory == 0) ||
    ...
    if (directory && directory != tcwd) free(directory);
...
#undef tcwd
  ...
  if (directory) {
    ...
    if (posixly_correct && pflag) opt = setpwd(directory);
    if (directory != the_current_working_directory) free(directory);
...
static int change_to_directory(newdir, nolinks, xattr) char *newdir;
int nolinks, xattr;
{
  char *t, *tdir, *ndir;
