...
int type_builtin(WORD_LIST *list) {
 ...
 */
int describe_command(command, dflags) char *command;
int dflags;
{
  ...
  if (((dflags & CDESC_FORCE_PATH) == 0) &&
      (i = find_reserved_word(command)) >= 0) {
    if (dflags & CDESC_TYPE)
      puts("keyword");
    else if (dflags & CDESC_SHORTDESC)
  ...
  while (1) {
    if (dflags & CDESC_STDPATH) /* command -p, all cannot be non-zero */
      ...
      full_path =
          find_in_path(command, pathlist, FS_EXEC_PREFERRED | FS_NODIRS);
      free(pathlist);
