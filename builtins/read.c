...
/* Set a flag that CHECK_ALRM can check.  This relies on zread calling
   ...
   alrmbuf. */
static sighandler sigalrm(s) int s;
{ sigalrm_seen = 1; }
...
int read_builtin(WORD_LIST *list) {
...
/* Convenience: check early whether or not the first of possibly several
   variable names is a valid identifier, and bail early if so. */
    ...
    if (list && legal_identifier(list->word->word) == 0 &&
        valid_array_reference(list->word->word, 0) == 0)
#else
  if (list && legal_identifier(list->word->word) == 0)
#endif
    {
...
#if defined(READLINE)
  if (edit) add_unwind_protect(xfree, rlbuf);
#endif
  ...
  if (tmsec > 0 || tmusec > 0) {
    /* Turn off the timeout if stdin is a regular file (e.g. from
       input redirection). */
    if ((fstat(fd, &tsb) < 0) || S_ISREG(tsb.st_mode)) tmsec = tmusec = 0;
  }

  if (tmsec > 0 || tmusec > 0) {
    code = sigsetjmp(alrmbuf, 0);
    if (code) {
      sigalrm_seen = 0;
    ...
    if (interactive_shell == 0) initialize_terminating_signals();
  ...
  for (print_ps2 = eof = retval = 0;;) {
    ...
    if (edit) {
      ...
      if (print_ps2) {
        if (ps2 == 0) ps2 = get_string_value("PS2");
	...
	interrupt_immediately--;
      ...
      if (retval <= 0) {
        if (retval < 0 && errno == EINTR) {
          lastsig = LASTSIG();
          if (lastsig == 0) lastsig = trapped_signal_received;
    ...
    if (pass_next) {
      ...
      if (c == '\n') {
        ...
        if (interactive && input_is_tty && raw == 0) print_ps2 = 1;
      } else
        goto add_char;
      continue;
    }
    ...
    if (ignore_delim == 0 && (unsigned char)c == delim) break;

    if (c == '\0' && delim != '\0') continue; /* skip NUL bytes in input */
  ...
  if (tmsec > 0 || tmusec > 0) reset_alarm();

  if (nchars > 0 || delim != '\n') {
#if defined(READLINE)
    if (edit) {
      if (nchars > 0) rl_num_chars_to_read = 0;
...
assign_vars:
  ...
  if (arrayname) {
    ...
    if (assoc_p(var)) {
      builtin_error("%s: cannot convert associative to indexed array",
                    arrayname);
      xfree(input_string);
  ...
  for (; list->next; list = list->next) {
    ...
    if (legal_identifier(varname) == 0 &&
        valid_array_reference(varname, 0) == 0)
#else
    if (legal_identifier(varname) == 0)
#endif
    {
...
/* Now assign the rest of the line to the last variable argument. */
  ...
  if (saw_escape && input_string && *input_string) {
    ...
    var =
        bind_read_variable(list->word->word, input_string ? input_string : "");
...
static SHELL_VAR *bind_read_variable(name, value) char *name, *value;
{
  SHELL_VAR *v;
...
#if defined(HANDLE_MULTIBYTE)
static int read_mbchar(fd, string, ind, ch, unbuffered) int fd;
char *string;
...
mbchar_return:
  if (i > 1) /* read a multibyte char */
    ...
    for (r = 1; r < i; r++) string[ind + r - 1] = mbchar[r];
  return i - 1;
}
#endif

static void ttyrestore(ttp) struct ttsave *ttp;
{
...
void read_tty_cleanup() {
  if (tty_modified) ttyrestore(&termsave);
}

int read_tty_modified() { return (tty_modified); }
...
static void reset_attempted_completion_function(cp) char *cp;
{
...
static int set_itext() {
  int r1, r2;

  r1 = r2 = 0;
  if (old_startup_hook) r1 = (*old_startup_hook)();
  if (deftext) {
...
static char *edit_line(p, itext) char *p;
char *itext;
{
  char *ret;
  int len;
...
static void set_eol_delim(c) int c;
{
  Keymap cmap;
...
static void reset_eol_delim(cp) char *cp;
{
  Keymap cmap;
