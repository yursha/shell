...
int declare_builtin(WORD_LIST *list) { return (declare_internal(list, 0)); }

int local_builtin(register WORD_LIST *list) {
  if (variable_context)
...
static int declare_internal(list, local_var) register WORD_LIST *list;
int local_var;
{
  ...
  flags_on = flags_off = any_failed = assign_error = pflag = nodefs = mkglobal =
      0;
  ...
  while ((opt = internal_getopt(list, DECLARE_OPTS)) != -1) {
    ...
    switch (opt) {
      ...
      case 'c':
        *flags |= att_capcase;
        if (flags == &flags_on) flags_off |= att_uppercase | att_lowercase;
        break;
#endif
      case 'l':
        *flags |= att_lowercase;
        if (flags == &flags_on) flags_off |= att_capcase | att_uppercase;
        break;
      case 'u':
        *flags |= att_uppercase;
        if (flags == &flags_on) flags_off |= att_capcase | att_lowercase;
        break;
#endif /* CASEMOD_ATTRS */
        CASE_HELPOPT;
      default:
  ...
  if (list == 0) /* declare -[aAfFirtx] */
    ...
    if (local_var) {
      ...
      if (vlist) {
        for (i = 0; vlist[i]; i++) print_assignment(vlist[i]);
    ...
    if (flags_on & att_nameref) {
      ...
      if (valid_array_reference(name, 0)) {
        ...
        if (variable_context == 0) {
          builtin_error("%s: nameref variable self references not allowed",
                        name);
          assign_error++;
          NEXT_VARIABLE();
        } else
...
#endif /* DEBUGGER */
                {
              t = nodefs ? var->name : named_function_string(
...
/* Here's what ksh93 seems to do as of the 2012 version: if we are
   ...
   modify its value (changing which variable it references). */
      ...
      /* However, if we're turning off the nameref attribute on an existing
        ...
        if (refvar && nameref_p(refvar) == 0) refvar = 0;
      ...
      if (var == 0 && (flags_on || flags_off || offset)) {
        ...
        if (refvar && var == 0) {
          ...
          if (offset) {
            ...
            if (aflags & ASS_APPEND) name[namelen++] = '+';
            name[namelen++] = '=';
            if (value && *value)
...
#endif
            {
          var = mkglobal ? bind_global_variable(name, (char *)NULL, ASS_FORCE)
      ...
      /* Can't take an existing array variable and make it a nameref */
        ...
        if (nameref_p(var) == 0 && var_isset(var) && offset == 0 &&
          ...
          builtin_error("`%s': invalid variable name for name reference",
                        value_cell(var));
          any_failed++;
          NEXT_VARIABLE();
        }
      ...
      if ((readonly_p(var) || noassign_p(var)) && offset) {
        if (readonly_p(var)) sh_readonly(name);
        assign_error++;
        NEXT_VARIABLE();
      }
      ...
      if (array_subscript_assignment && array_exists && creating_array == 0)
         ...
         * value, wflags);*/
        if ((wflags & W_COMPASSIGN) == 0 && value[0] == '(' &&
            value[vlen - 1] == ')') {
      ...
      if (flags_on & att_assoc) {
        if (assoc_p(var) == 0) var = convert_var_to_assoc(var);
      }
...
#if defined(ARRAY_VARS)
      if (array_p(var) || assoc_p(var) || (offset && compound_array_assign) ||
          simple_array_assign)
      ...
      if (offset && compound_array_assign)
        ...
        *subscript_start = '['; /* ] */
        ...
        var = assign_array_element(name, value,
                                   aflags & ASS_APPEND); /* XXX - not aflags */
        *subscript_start = '\0';
        ...
        if (assoc_p(var))
          ...
          /* bind_variable_value duplicates the essential internals of
             bind_variable() */
          if (offset) {
        if (onref || nameref_p(var)) aflags |= ASS_NAMEREF;
        v = bind_variable_value(var, value, aflags);
        if (v == 0 && (onref || nameref_p(var))) {
          if (valid_nameref_value(value, 1) == 0) sh_invalidid(value);
          assign_error++;
      ...
      if ((flags_on & (att_exported | att_readonly)) && tempvar_p(var)) {
        ...
        if (tv) {
          tvalue =
              var_isset(var) ? savestring(value_cell(var)) : savestring("");
          tv = bind_variable(var->name, tvalue, 0);
          tv->attributes |= var->attributes & ~att_tempvar;
          if (tv->context > 0) VSETATTR(tv, att_propagate);
          free(tvalue);
        }
    ...
    if (refvar) {
      if (flags_off & att_readonly) flags_off &= ~att_readonly;
