...
static char *mkdashname(name) char *name;
{
  char *ret;
...
int exec_builtin(WORD_LIST *list) {
  ...
  if (com2) {
    if (command != args[0]) free(command);
    command = com2;
  }
  ...
  if (cleanenv == 0) adjust_shell_level(-1);
...
failed_exec:
  ...
  if (args) strvec_dispose(args);

  if (env && env != export_env) strvec_dispose(env);

  initialize_traps();
  initialize_signals(1);

#if defined(JOB_CONTROL)
  if (interactive_shell || job_control) restart_job_control();
#endif /* JOB_CONTROL */
