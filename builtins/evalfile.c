...
int flags;
{
  ...
  if (flags & FEVAL_UNWINDPROT) {
    ...
    if (flags & FEVAL_NONINT) unwind_protect_int(interactive);
    unwind_protect_int(sourcelevel);
  } else {
    COPY_PROCENV(return_catch, old_return_catch);
    if (flags & FEVAL_NONINT) old_interactive = interactive;
  }

  if (flags & FEVAL_NONINT) interactive = 0;
  ...
  if (flags & FEVAL_UNWINDPROT) add_unwind_protect(restore_funcarray_state, fa);
  ...
  if ((flags & FEVAL_NOPUSHARGS) == 0) {
    ...
    if (flags & FEVAL_UNWINDPROT) add_unwind_protect(pop_args, 0);
  }
#endif
#endif
  ...
  if (flags & FEVAL_UNWINDPROT)
    ...
    if (flags & FEVAL_NONINT) interactive = old_interactive;
#if defined(ARRAY_VARS)
  ...
  if (current_token == yacc_EOF) push_token('\n'); /* XXX */
...
int force_noninteractive;
{
  ...
  if (force_noninteractive) flags |= FEVAL_NONINT;
...
int sflags;
{
  ...
  if (sflags) flags |= FEVAL_NOPUSHARGS;
