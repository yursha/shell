...
int internal_getopt(list, opts) WORD_LIST *list;
char *opts;
{
  register int c;
  register char *cp;
  ...
  if (sp == 1) {
    if (lcurrent == 0 || NOTOPT(lcurrent->word->word)) {
      ...
      return (GETOPT_HELP);
               ...
               lcurrent->word->word[1] == '-' && lcurrent->word->word[2] == 0) {
      lhead = (WORD_LIST *)NULL;
  ...
  if (c == ':' || (cp = strchr(opts, c)) == NULL) {
    ...
    list_optarg = NULL;
    if (lcurrent) loptend = lcurrent->next;
    return ('?');
  }
...
/* If the specifier is `;', don't set optarg if the next
   argument looks like another option. */
               ...
               (*cp == ':' || NOTOPT(lcurrent->next->word->word))) {
#endif
      lcurrent = lcurrent->next;
    ...
    if (lcurrent->word->word[sp + 1]) {
      ...
      if (lcurrent->next &&
          legal_number(lcurrent->next->word->word, (intmax_t *)0)) {
        lcurrent = lcurrent->next;
        list_optarg = lcurrent->word->word;
        lcurrent = lcurrent->next;
      } else {
