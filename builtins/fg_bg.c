...
int bg_builtin(WORD_LIST *list) {
  ...
  r = EXECUTION_SUCCESS;
  do {
    if (fg_bg(list, 0) == EXECUTION_FAILURE) r = EXECUTION_FAILURE;
...
static int fg_bg(list, foreground) WORD_LIST *list;
int foreground;
{
  sigset_t set, oset;
  ...
  if (INVALID_JOB(job)) {
    if (job != DUP_JOB) sh_badjob(list ? list->word->word : "current");
  ...
  if (status >= 0) {
    ...
    if (foreground == 0) last_asynchronous_pid = old_async_pid;
