...
/* Print statistics on the current state of hashed commands.  If LIST is
   ...
   commands. */
int hash_builtin(WORD_LIST *list) {
  ...
  if (expunge_hash_table) phash_flush();

  /* If someone runs `hash -r -t xyz' he will be disappointed. */
  if (list_targets) return (list_hashed_filename_targets(list, list_portably));
...
static int add_hashed_command(w, quiet) char *w;
int quiet;
{
  int rv;
  char *full_path;
...
static int print_hash_info(item) BUCKET_CONTENTS *item;
{
  printf("%4d\t%s\n", item->times_found, pathdata(item)->path);
  return 0;
}

static int print_portable_hash_info(item) BUCKET_CONTENTS *item;
{
  char *fp, *fn;
  ...
  if (fp != pathdata(item)->path) free(fp);
  if (fn != item->key) free(fn);
  return 0;
}

static int print_hashed_commands(fmt) int fmt;
{
  if (hashed_filenames == 0 || HASH_ENTRIES(hashed_filenames) == 0) return (0);
...
static int list_hashed_filename_targets(list, fmt) WORD_LIST *list;
int fmt;
{
  ...
  for (l = list; l; l = l->next) {
    ...
    if (fmt)
      ...
      if (multiple) printf("%s\t", l->word->word);
      printf("%s\n", target);
    }
    free(target);
  }
