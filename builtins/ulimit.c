...
long ulimit(cmd, newlim) int cmd;
long newlim;
{
  errno = EINVAL;
  return -1;
...
static int _findlim(opt) int opt;
{
  register int i;

  for (i = 0; limits[i].option > 0; i++)
    if (limits[i].option == opt) return i;
  return -1;
}
...
int ulimit_builtin(register WORD_LIST *list) {
  register char *s;
  ...
  while ((opt = internal_getopt(list, optstring)) != -1) {
    switch (opt) {
      ...
      default:
        if (ncmd >= cmdlistsz)
          cmdlist =
              (ULCMD *)xrealloc(cmdlist, (cmdlistsz *= 2) * sizeof(ULCMD));
...
static int ulimit_internal(cmd, cmdarg, mode, multiple) int cmd;
char *cmdarg;
int mode, multiple;
{
  ...
  setting = cmdarg != 0;
  limind = _findlim(cmd);
  if (mode == 0) mode = setting ? (LIMIT_HARD | LIMIT_SOFT) : LIMIT_SOFT;
...
static int get_limit(ind, softlim, hardlim) int ind;
  ...
  if (limits[ind].parameter >= 256) {
    switch (limits[ind].parameter) {
      case RLIMIT_FILESIZE:
        if (filesize(&value) < 0) return -1;
        break;
      case RLIMIT_PIPESIZE:
        if (pipesize(&value) < 0) return -1;
        break;
      ...
      case RLIMIT_MAXUPROC:
        if (getmaxuprc(&value) < 0) return -1;
        break;
      default:
        errno = EINVAL;
        return -1;
...
#if defined(HAVE_RESOURCE)
    if (getrlimit(limits[ind].parameter, &limit) < 0) return -1;
...
static int set_limit(ind, newlim, mode) int ind;
RLIMTYPE newlim;
int mode;
{
#if defined(HAVE_RESOURCE)
  ...
  if (limits[ind].parameter >= 256) switch (limits[ind].parameter) {
      case RLIMIT_FILESIZE:
#if !defined(HAVE_RESOURCE)
    ...
    if (getrlimit(limits[ind].parameter, &limit) < 0) return -1;
#if defined(HPUX9)
    if (limits[ind].parameter == RLIMIT_FILESIZE) newlim /= 512; /* Ugh. */
#endif                                                           /* HPUX9 */
    val = (current_user.euid != 0 && newlim == RLIM_INFINITY &&
           ...
           (limit.rlim_cur <= limit.rlim_max))
              ? limit.rlim_max
              : newlim;
    if (mode & LIMIT_SOFT) limit.rlim_cur = val;
    if (mode & LIMIT_HARD) limit.rlim_max = val;
...
static int getmaxvm(softlim, hardlim) RLIMTYPE *softlim, *hardlim;
{
#if defined(HAVE_RESOURCE)
  struct rlimit datalim, stacklim;

  if (getrlimit(RLIMIT_DATA, &datalim) < 0) return -1;

  if (getrlimit(RLIMIT_STACK, &stacklim) < 0) return -1;
...
static int filesize(valuep) RLIMTYPE *valuep;
{
#if !defined(HAVE_RESOURCE)
  long result;
...
static int pipesize(valuep) RLIMTYPE *valuep;
{
#if defined(PIPE_BUF)
...
static int getmaxuprc(valuep) RLIMTYPE *valuep;
{
  long maxchild;
...
static void print_all_limits(mode) int mode;
{
  register int i;
  RLIMTYPE softlim, hardlim;

  if (mode == 0) mode |= LIMIT_SOFT;
...
static void printone(limind, curlim, pdesc) int limind;
RLIMTYPE curlim;
int pdesc;
{
  char unitstr[64];
...
static int set_all_limits(mode, newlim) int mode;
RLIMTYPE newlim;
{
  register int i;
  int retval = 0;
  ...
  if (mode == 0) mode = LIMIT_SOFT | LIMIT_HARD;
