...
int shopt_builtin(WORD_LIST *list) {
  int opt, flags, rval;
  ...
  if ((flags & OFLAG) && ((flags & (SFLAG | UFLAG)) == 0)) /* shopt -o */
    ...
    rval =
        toggle_shopts((flags & SFLAG) ? SETOPT : UNSETOPT, list, flags & QFLAG);
...
static int find_shopt(name) char *name;
{
  int i;

  for (i = 0; shopt_vars[i].name; i++)
    if (STREQ(name, shopt_vars[i].name)) return i;
  return -1;
}

static void shopt_error(s) char *s;
{ builtin_error("%s: invalid shell option name", s); }

static int toggle_shopts(mode, list, quiet) int mode;
WORD_LIST *list;
int quiet;
{
  WORD_LIST *l;
  ...
  if (v = find_variable("BASHOPTS")) set_bashopts();
  return (rval);
}

static void print_shopt(name, val, flags) char *name;
int val, flags;
{
  if (flags & PFLAG)
    printf("shopt %s %s\n", val ? "-s" : "-u", name);
...
static int list_shopts(list, flags) WORD_LIST *list;
int flags;
{
  WORD_LIST *l;
  int i, val, rval;
  ...
  if (list == 0) {
    for (i = 0; shopt_vars[i].name; i++) {
      ...
      if ((flags & QFLAG) == 0) print_shopt(shopt_vars[i].name, val, flags);
    }
  ...
  for (l = list, rval = EXECUTION_SUCCESS; l; l = l->next) {
    ...
    val = *shopt_vars[i].value;
    if (val == 0) rval = EXECUTION_FAILURE;
    if ((flags & QFLAG) == 0) print_shopt(l->word->word, val, flags);
  }

  return (sh_chkwrite(rval));
}

static int list_some_shopts(mode, flags) int mode, flags;
{
  int val, i;
...
static int list_shopt_o_options(list, flags) WORD_LIST *list;
int flags;
{
  WORD_LIST *l;
  int val, rval;
  ...
  if (list == 0) {
    if ((flags & QFLAG) == 0) list_minus_o_opts(-1, (flags & PFLAG));
    return (sh_chkwrite(EXECUTION_SUCCESS));
  }
...
static int list_some_o_options(mode, flags) int mode, flags;
{
  if ((flags & QFLAG) == 0) list_minus_o_opts(mode, (flags & PFLAG));
  return (sh_chkwrite(EXECUTION_SUCCESS));
}

static int set_shopt_o_options(mode, list, quiet) int mode;
WORD_LIST *list;
int quiet;
{
  WORD_LIST *l;
...
static int set_shellopts_after_change(option_name, mode) char *option_name;
int mode;
{
  set_shellopts();
  return (0);
...
static int shopt_set_debug_mode(option_name, mode) char *option_name;
int mode;
{
#if defined(DEBUGGER)
...
static int shopt_enable_hostname_completion(option_name,
                                            mode) char *option_name;
int mode;
{ return (enable_hostname_completion(mode)); }

static int set_compatibility_level(option_name, mode) char *option_name;
int mode;
{
  int ind;
...
#if defined(READLINE)
static int shopt_set_complete_direxpand(option_name, mode) char *option_name;
int mode;
{
  set_directory_hook();
  return 0;
...
int set_login_shell(option_name, mode) char *option_name;
int mode;
{
...
char **get_shopt_options() {
  ...
  for (i = 0; shopt_vars[i].name; i++) ret[i] = savestring(shopt_vars[i].name);
  ret[i] = (char *)NULL;
  return ret;
}
 ...
 */
int shopt_setopt(name, mode) char *name;
int mode;
{
  WORD_LIST *wl;
  int r;
...
int shopt_listopt(name, reusable) char *name;
int reusable;
{
  int i;

  if (name == 0) return (list_shopts((WORD_LIST *)NULL, reusable ? PFLAG : 0));
...
void parse_bashopts(value) char *value;
{
  char *vname;
  int vptr, ind;
...
void initialize_bashopts(no_bashopts) int no_bashopts;
{
  char *temp;
  SHELL_VAR *var;
