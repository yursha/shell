...
int export_builtin(register WORD_LIST *list) {
  return (set_or_show_attributes(list, att_exported, 0));
}

/* For each variable name in LIST, make that variable readonly.  Given an
   empty LIST, print out all existing readonly variables. */
int readonly_builtin(register WORD_LIST *list) {
...
int set_or_show_attributes(list, attribute, nodefs) register WORD_LIST *list;
int attribute, nodefs;
{
  ...
  if (list) {
    if (attribute & att_exported) array_needs_making = 1;

    /* Cannot undo readonly status, silently disallowed. */
    if (undo && (attribute & att_readonly)) attribute &= ~att_readonly;
    ...
    while (list) {
      ...
      if (assign) /* xxx [-np] name=value */
          {
        name[assign] = '=';
        if (aflags & ASS_APPEND) name[assign - 1] = '+';
#if defined(ARRAY_VARS)
        ...
        if (arrays_only || assoc_only) {
          ...
          if (opt != EXECUTION_SUCCESS) assign_error++;
          list->next = tlist;
          dispose_word(w);
          free(nlist);
        } else
          ...
          assign_error++;
        name[assign] = '\0';
        if (aflags & ASS_APPEND) name[assign - 1] = '\0';
      }
    ...
    if (attribute & att_array) {
      arrays_only++;
      if (attribute != att_array) attribute &= ~att_array;
    } else if (attribute & att_assoc) {
      assoc_only++;
      if (attribute != att_assoc) attribute &= ~att_assoc;
    }
#endif
    ...
    if (variable_list) {
      for (i = 0; var = variable_list[i]; i++) {
        ...
        if ((var->attributes & attribute)) {
          ...
          if (any_failed = sh_chkwrite(any_failed)) break;
        }
      }
      free(variable_list);
    }
...
/* Show all variable variables (v == 1) or functions (v == 0) with
   attributes. */
int show_all_var_attributes(v, nodefs) int v, nodefs;
{
  ...
  if (variable_list == 0) return (EXECUTION_SUCCESS);
...
int var_attribute_string(var, pattr, flags) SHELL_VAR *var;
int pattr;
...
#if defined(ARRAY_VARS)
    if (array_p(var)) flags[i++] = 'a';

    if (assoc_p(var)) flags[i++] = 'A';
#endif

    if (function_p(var)) flags[i++] = 'f';

    if (integer_p(var)) flags[i++] = 'i';

    if (nameref_p(var)) flags[i++] = 'n';

    if (readonly_p(var)) flags[i++] = 'r';

    if (trace_p(var)) flags[i++] = 't';

    if (exported_p(var)) flags[i++] = 'x';

    if (capcase_p(var)) flags[i++] = 'c';

    if (lowercase_p(var)) flags[i++] = 'l';

    if (uppercase_p(var)) flags[i++] = 'u';
  } else {
#if defined(ARRAY_VARS)
    if (array_p(var)) flags[i++] = 'a';
...
int show_var_attributes(var, pattr, nodefs) SHELL_VAR *var;
int pattr, nodefs;
{
...
int show_name_attributes(name, nodefs) char *name;
int nodefs;
{
  SHELL_VAR *var;
...
int show_func_attributes(name, nodefs) char *name;
int nodefs;
{
  SHELL_VAR *var;
...
void set_var_attribute(name, attribute, undo) char *name;
int attribute, undo;
{
  ...
  if (undo)
    ...
    if (tv && tempvar_p(tv)) {
      ...
      if (var->context == 0 && (attribute & att_readonly)) {
        ...
        if (v != var) VSETATTR(tv, att_propagate);
      } else
        VSETATTR(tv, att_propagate);
      if (var->context != 0) VSETATTR(var, att_propagate);
      ...
      if (var == 0) {
        ...
        if (refvar == INVALID_NAMEREF_VALUE) return;
  ...
  if (var) SETVARATTR(var, attribute, undo);
