...
const struct {
  ...
  setopt_get_func_t *get_func;
    ...
    {"noexec", 'n', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
    {"noglob", 'f', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
#if defined(HISTORY)
    {"nolog", '\0', &dont_save_function_defs, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
#endif
#if defined(JOB_CONTROL)
    {"notify", 'b', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
#endif /* JOB_CONTROL */
    {"nounset", 'u', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
    {"onecmd", 't', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
    {"physical", 'P', (int *)NULL, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
    {"pipefail", '\0', &pipefail_opt, (setopt_set_func_t *)NULL,
     (setopt_get_func_t *)NULL},
    {"posix", '\0', &posixly_correct, set_posix_mode,
     (setopt_get_func_t *)NULL},
...
int minus_o_option_value(name) char *name;
{
  register int i;
  int *on_or_off;
...
static void print_minus_o_option(name, value, pflag) char *name;
int value, pflag;
{
  if (pflag == 0)
...
void list_minus_o_opts(mode, reusable) int mode, reusable;
{
  register int i;
  int *on_or_off, value;
  ...
  for (i = 0; o_options[i].name; i++) {
    if (o_options[i].letter) {
      value = 0;
      on_or_off = find_flag(o_options[i].letter);
      if (on_or_off == FLAG_UNKNOWN) on_or_off = &value;
...
char **get_minus_o_opts() {
  ...
  for (i = 0; o_options[i].name; i++) ret[i] = o_options[i].name;
  ret[i] = (char *)NULL;
  return ret;
}
...
void set_current_options(bitmap) const char *bitmap;
{
  int i;
  ...
  for (i = 0; o_options[i].name; i++) {
    if (o_options[i].letter)
      ...
      SET_BINARY_O_OPTION_VALUE(i, bitmap[i] ? FLAG_ON : FLAG_OFF,
                                o_options[i].name);
  }
}

static int set_ignoreeof(on_or_off, option_name) int on_or_off;
char *option_name;
{
...
static int set_posix_mode(on_or_off, option_name) int on_or_off;
char *option_name;
{
...
static int set_edit_mode(on_or_off, option_name) int on_or_off;
char *option_name;
{
  int isemacs;
  ...
  if (on_or_off == FLAG_ON) {
    ...
    if (interactive) with_input_from_stdin();
    no_line_editing = 0;
  } else {
...
static int get_edit_mode(name) char *name;
{
...
#if defined(HISTORY)
static int bash_set_history(on_or_off, option_name) int on_or_off;
char *option_name;
{
  if (on_or_off == FLAG_ON) {
    enable_history_list = 1;
    bash_history_enable();
    if (history_lines_this_session == 0) load_history();
  } else {
...
int set_minus_o_option(on_or_off, option_name) int on_or_off;
char *option_name;
{
  register int i;
    ...
    if (STREQ(option_name, o_options[i].name)) {
      if (o_options[i].letter == 0) {
        ...
        if ((previous_option_value =
                 change_flag(o_options[i].letter, on_or_off)) == FLAG_ERROR) {
...
static void print_all_shell_variables() {
  SHELL_VAR **vars;
...
void set_shellopts() {
  ...
  if (vptr) vptr--; /* cut off trailing colon */
  value[vptr] = '\0';
...
void parse_shellopts(value) char *value;
{
  char *vname;
  int vptr;
...
void initialize_shell_options(no_shellopts) int no_shellopts;
{
  char *temp;
  SHELL_VAR *var;
  ...
  if (no_shellopts == 0) {
    ...
    if (var && imported_p(var)) {
      temp = (array_p(var) || assoc_p(var)) ? (char *)NULL
...
int set_builtin(WORD_LIST *list) {
    ...
    if ((on_or_off = *arg) && (on_or_off == '-' || on_or_off == '+')) {
      while (flag_name = *++arg) {
        ...
        } else if (flag_name == 'o') /* -+o option-name */
        {
          ...
          if (option_name == 0 || *option_name == '\0' || *option_name == '-' ||
              *option_name == '+') {
            list_minus_o_opts(-1, (on_or_off == '+'));
            continue;
          }
  ...
  /* Assigning $1 ... $n */
  if (list || force_assignment) remember_args(list, 1);
  /* Set up new value of $SHELLOPTS */
  if (opts_changed) set_shellopts();
  return (rv);
}
...
int unset_builtin(WORD_LIST *list) {
  ...
  while (list) {
    ...
    if (((unset_function && posixly_correct) || !unset_function) &&
    ...
    if (var == 0 && nameref == 0 && unset_variable == 0 &&
        unset_function == 0) {
      if (var = find_function(name)) unset_function = 1;
    }

    /* Posix.2 says that unsetting readonly variables is an error. */
    if (var && readonly_p(var)) {
      builtin_error("%s: cannot unset: readonly %s", var->name,
...
/* Unless the -f option is supplied, the name refers to a variable. */
    ...
    if (var && unset_array) {
      ...
      tem = unset_function
                ? unbind_func(name)
                : (nameref ? unbind_nameref(name) : unbind_variable(name));
    ...
    if (unset_function == 0) stupidly_hack_special_variables(name);
