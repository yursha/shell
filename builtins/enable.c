...
/* Enable/disable shell commands present in LIST.  If list is not specified,
   ...
   which are disabled. */
int enable_builtin(WORD_LIST *list) {
  int result, flags;
  int opt, filter;
#if defined(HAVE_DLOPEN) && defined(HAVE_DLSYM)
  char *filename;
  ...
  if (list == 0 || (flags & PFLAG)) {
    filter = (flags & AFLAG) ? (ENABLED | DISABLED) : (flags & NFLAG) ? DISABLED
                                                                      : ENABLED;

    if (flags & SFLAG) filter |= SPECIAL;
    ...
    while (list) {
      ...
      if (opt == EXECUTION_FAILURE) result = EXECUTION_FAILURE;
      list = list->next;
    }
#if defined(PROGRAMMABLE_COMPLETION)
    set_itemlist_dirty(&it_builtins);
...
static void list_some_builtins(filter) int filter;
{
  register int i;
  ...
  for (i = 0; i < num_shell_builtins; i++) {
    ...
    if ((filter & SPECIAL) && (shell_builtins[i].flags & SPECIAL_BUILTIN) == 0)
      continue;
...
static int enable_shell_command(name, disable_p) char *name;
int disable_p;
{
  struct builtin *b;

  b = builtin_address_internal(name, 1);
  if (b == 0) return (EXECUTION_FAILURE);
...
static int dyn_load_builtin(list, flags, filename) WORD_LIST *list;
int flags;
char *filename;
{
  WORD_LIST *l;
  ...
  if (list == 0) return (EXECUTION_FAILURE);
...
#define RTLD_LAZY 1
  ...
  if (absolute_program(filename) == 0) {
    ...
    if (loadables_path) {
      load_path =
          find_in_path(filename, loadables_path, FS_NODIRS | FS_EXEC_PREFERRED);
      if (load_path) {
#if defined(_AIX)
  ...
  for (replaced = new = 0; list; list = list->next) {
    ...
    if (b == 0) {
      name = printable_filename(filename, 0);
      builtin_error("cannot find %s in shared object %s: %s", struct_name, name,
                    dlerror());
      if (name != filename) free(name);
      free(struct_name);
      continue;
    ...
    if (loadfunc) {
      r = (*loadfunc)(name);
      if (r == 0) {
        builtin_error("load function for %s returns failure (%d): not loaded",
                      name, r);
        free(funcname);
        continue;
      }
    ...
    if (flags & SPECIAL) b->flags |= SPECIAL_BUILTIN;
    b->handle = handle;
...
#if defined(HAVE_DLCLOSE)
static void delete_builtin(b) struct builtin *b;
{
  int ind, size;
...
static int local_dlclose(handle) void *handle;
{
...
static int dyn_unload_builtin(name) char *name;
{
  struct builtin *b;
  void *handle;
  char *funcname;
  ...
  for (ref = i = 0; i < num_shell_builtins; i++) {
    if (shell_builtins[i].handle == b->handle) ref++;
  }
  ...
  if (unloadfunc) (*unloadfunc)(name); /* void function */
  free(funcname);
