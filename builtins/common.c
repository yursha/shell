...
int destructive;
{
  ...
  if (destructive) set_dollar_vars_changed();
...
/* Have the dollar variables been reset to new values since we last
   checked? */
int dollar_vars_changed() { return (changed_dollar_vars); }

void set_dollar_vars_unchanged() { changed_dollar_vars = 0; }
...
int get_numeric_arg(list, fatal, count) WORD_LIST *list;
int fatal;
intmax_t *count;
{
  char *arg;
...
int get_exitstat(list) WORD_LIST *list;
{
  int status;
  intmax_t sval;
  char *arg;
...
int read_octal(string) char *string;
{
  int result, digits;
  ...
  while (*string && ISOCTAL(*string)) {
    ...
    if (result > 0777) return -1;
  }

  if (digits == 0 || *string) result = -1;
...
char *get_working_directory(for_whom) char *for_whom;
{
...
int flags;
{
  ...
  for (i = js.j_jobslots - 1; i >= 0; i--) {
    j = get_job_by_jid(i);
    if (j == 0 || ((flags & JM_STOPPED) && J_JOBSTATE(j) != JSTOPPED)) continue;
...
int get_job_spec(list) WORD_LIST *list;
{
  register char *word;
  int job, jflags;

  if (list == 0) return (js.j_current);

  word = list->word->word;

  if (*word == '\0') return (NO_JOB);
 ...
 */
int display_signal_list(list, forcecols) WORD_LIST *list;
int forcecols;
{
  ...
  if (!list) {
    ...
    if ((posixly_correct && !forcecols) || column != 0) printf("\n");
    return result;
  }
...
struct builtin *builtin_address_internal(char *name, int disabled_okay) {
  ...
  while (lo <= hi) {
    ...
    if (j == 0) j = strcmp(shell_builtins[mid].name, name);
...
sh_builtin_func_t *find_special_builtin(name) char *name;
{
  current_builtin = builtin_address_internal(name, 0);
...
static int shell_builtin_compare(sbp1, sbp2) struct builtin *sbp1, *sbp2;
{
  int result;
