...
string_to_rlimtype(s) char *s;
{
  ...
  ret = 0;
  neg = 0;
  while (s && *s && whitespace(*s)) s++;
  if (s && (*s == '-' || *s == '+')) {
    neg = *s == '-';
    s++;
  }
  for (; s && *s && DIGIT(*s); s++) ret = (ret * 10) + TODIGIT(*s);
  return (neg ? -ret : ret);
}
...
int all_digits(string) char *string;
{
  register char *s;
...
/* Return 1 if this token is a legal shell `identifier'; that is, it consists
   ...
   digit. */
int legal_identifier(name) char *name;
{
  register char *s;
  unsigned char c;

  if (!name || !(c = *name) || (legal_variable_starter(c) == 0)) return (0);
...
int valid_nameref_value(name, flags) char *name;
int flags;
{
...
int check_identifier(word, check_word) WORD_DESC *word;
int check_word;
{
...
/* Return 1 if STRING is a function name that the shell will import from
   ...
   used yet. */
int importable_function_name(string, len) char *string;
size_t len;
{
  ...
  if (*string == '\n') /* can't start with a newline */
    return 0;
  if (shellblank(*string) || shellblank(string[len - 1])) return 0;
  return (posixly_correct ? legal_identifier(string) : 1);
}

int exportable_function_name(string) char *string;
{
  if (absolute_program(string)) return 0;
...
int legal_alias_name(string, flags) char *string;
int flags;
{
  register char *s;
...
int sh_unset_nodelay_mode(fd) int fd;
{
  int flags, bflags;
...
int sh_validfd(fd) int fd;
{ return (fcntl(fd, F_GETFD, 0) >= 0); }

int fd_ispipe(fd) int fd;
{
  errno = 0;
...
int same_file(path1, path2, stp1, stp2) char *path1, *path2;
...
int move_to_high_fd(fd, check_new, maxfd) int fd, check_new, maxfd;
{
  ...
  if (maxfd < 20) {
    nfds = getdtablesize();
    if (nfds <= 0) nfds = 20;
    if (nfds > HIGH_FD_MAX) nfds = HIGH_FD_MAX; /* reasonable maximum */
  } else
    nfds = maxfd;

  for (nfds--; check_new && nfds > 3; nfds--)
    if (fcntl(nfds, F_GETFD, &ignore) == -1) break;
...
int check_binary_file(sample, sample_len) char *sample;
int sample_len;
{
  register int i;
  unsigned char c;
...
int sh_openpipe(pv) int *pv;
{
  int r;

  if ((r = pipe(pv)) < 0) return r;
...
/* Turn STRING (a pathname) into an absolute pathname, assuming that
   ...
   begin with. */
char *make_absolute(string, dot_path) char *string, *dot_path;
{
  char *result;
...
char *base_pathname(string) char *string;
{
  char *p;
  ...
  if (string[0] == '/' && string[1] == 0) return (string);
...
char *full_pathname(file) char *file;
{
  char *ret;

  file = (*file == '~') ? bash_tilde_expand(file, 0) : savestring(file);

  if (ABSPATH(file)) return (file);
...
char *polite_directory_format(name) char *name;
{
  char *home;
  int l;
...
char *trim_pathname(name, maxlen) char *name;
int maxlen;
{
  int nlen, ndirs;
  intmax_t nskip;
  ...
  if (name == 0 || (nlen = strlen(name)) == 0) return name;
  nend = name + nlen;

  v = get_string_value("PROMPT_DIRTRIM");
  if (v == 0 || *v == 0) return name;
  if (legal_number(v, &nskip) == 0 || nskip <= 0) return name;
...
char *printable_filename(fn, flags) char *fn;
int flags;
{
  char *newf;
...
char *extract_colon_unit(string, p_index) char *string;
int *p_index;
{
  int i, start, len;
  char *value;
  ...
  len = strlen(string);
  if (*p_index >= len) return ((char *)NULL);
...
/* If tilde_expand hasn't been able to expand the text, perhaps it
   ...
   directory stack. */
static char *bash_special_tilde_expansions(text) char *text;
{
  char *result;
...
int assign_p;
{
  ...
  tilde_additional_prefixes =
      assign_p == 0 ? (char **)0 : (assign_p == 2 ? bash_tilde_prefixes2
                                                  : bash_tilde_prefixes);
  if (assign_p == 2) tilde_additional_suffixes = bash_tilde_suffixes2;
...
static void initialize_group_array() {
  ...
  if (maxgroups == 0) maxgroups = getmaxgroups();

  ngroups = 0;
  group_array =
      (GETGROUPS_T *)xrealloc(group_array, maxgroups * sizeof(GETGROUPS_T));
  ...
  for (i = 0; i < ngroups; i++)
    if (current_user.gid == (gid_t)group_array[i]) break;
  if (i == ngroups && ngroups < maxgroups) {
    for (i = ngroups; i > 0; i--) group_array[i] = group_array[i - 1];
    group_array[0] = current_user.gid;
    ngroups++;
  }
  ...
  if (group_array[0] != current_user.gid) {
    for (i = 0; i < ngroups; i++)
      if (group_array[i] == current_user.gid) break;
    if (i < ngroups) {
...
#endif /* !__STDC__*/
{
  ...
  if (gid == current_user.gid || gid == current_user.egid) return (1);

#if defined(HAVE_GETGROUPS)
  if (ngroups == 0) initialize_group_array();
  ...
  for (i = 0; i < ngroups; i++)
    if (gid == (gid_t)group_array[i]) return (1);
#endif

  return (0);
}

char **get_group_list(ngp) int *ngp;
{
  ...
  for (i = 0; i < ngroups; i++) group_vector[i] = itos(group_array[i]);

  if (ngp) *ngp = ngroups;
  return group_vector;
}

int *get_group_array(ngp) int *ngp;
{
  int i;
  ...
  for (i = 0; i < ngroups; i++) group_iarray[i] = (int)group_array[i];
