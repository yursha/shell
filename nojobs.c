...
static void alloc_pid_list() {
  ...
  for (i = old; i < pid_list_size; i++) pid_list[i].pid = NO_PID;
}
...
static int find_proc_slot(pid) pid_t pid;
{
  register int i;

  for (i = 0; i < pid_list_size; i++)
    if (pid_list[i].pid == NO_PID || pid_list[i].pid == pid) return (i);

  if (i == pid_list_size) alloc_pid_list();
...
static int find_index_by_pid(pid) pid_t pid;
{
  register int i;

  for (i = 0; i < pid_list_size; i++)
    if (pid_list[i].pid == pid) return (i);
...
static int find_status_by_pid(pid) pid_t pid;
{
  int i;

  i = find_index_by_pid(pid);
  if (i == NO_PID) return (PROC_BAD);
  if (pid_list[i].flags & PROC_RUNNING) return (PROC_STILL_ALIVE);
...
/* Return the status of PID as looked up in the PID_LIST array.  A
   return value of PROC_BAD indicates that PID wasn't found. */
static int find_termsig_by_pid(pid) pid_t pid;
{
  int i;

  i = find_index_by_pid(pid);
  if (i == NO_PID) return (0);
...
static int get_termsig(status) WAIT status;
{
...
static int get_pid_flags(pid) pid_t pid;
{
  int slot;

  slot = find_index_by_pid(pid);
  if (slot == NO_PID) return 0;
...
int async;
{
  ...
  pid_list[slot].flags = PROC_RUNNING;
  if (async) pid_list[slot].flags |= PROC_ASYNC;
}
...
static void mark_dead_jobs_as_notified(force) int force;
{
  ...
  for (i = ndead = 0; force == 0 && i < pid_list_size; i++) {
    if (pid_list[i].pid == NO_PID) continue;
    if (((pid_list[i].flags & PROC_RUNNING) == 0) &&
        (pid_list[i].flags & PROC_ASYNC))
      ndead++;
  }

  if (child_max < 0) child_max = getmaxchild();
  if (child_max < 0) child_max = DEFAULT_CHILD_MAX;

  if (force == 0 && ndead <= child_max) return;
  ...
  for (i = 0; i < pid_list_size; i++) {
    ...
    if (((pid_list[i].flags & PROC_RUNNING) == 0) &&
      ...
      if (force == 0 && (pid_list[i].flags & PROC_ASYNC) &&
          --ndead <= child_max)
        break;
    }
  }
...
void initialize_job_signals() {
  ...
  /* If this is a login shell we don't wish to be disturbed by
     stop signals. */
  if (login_shell) ignore_tty_job_signals();
}
...
static int siginterrupt(sig, flag) int sig, flag;
{
...
int wait_for_single_pid(pid) pid_t pid;
{
  ...
  if (pstatus != PROC_STILL_ALIVE) {
    if (pstatus > 128) last_command_exit_signal = find_termsig_by_pid(pid);
    return (pstatus);
  }
...
int wait_for(pid) pid_t pid;
{
  ...
  if (got_pid > 0) set_pid_status(got_pid, status);

#if defined(HAVE_WAITPID)
  if (got_pid >= 0) reap_zombie_children();
#endif /* HAVE_WAITPID */
...
int kill_pid(pid, signal, group) pid_t pid;
int signal, group;
{
  int result;
...
/* Stop a pipeline. */
int stop_pipeline(async, ignore) int async;
COMMAND *ignore;
{
...
void start_pipeline() { already_making_children = 1; }
...
int freeze_jobs_list() { return 0; }

void unfreeze_jobs_list() {}

int count_all_jobs() { return 0; }
