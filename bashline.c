...
{
  if (on_or_off) rl_variable_bind("comment-begin", "#");
#if defined(VI_MODE)
  rl_bind_key_in_map(CTRL('I'), on_or_off ? rl_insert : rl_complete,
                     vi_insertion_keymap);
#endif
}
...
int enable_hostname_completion(on_or_off) int on_or_off;
{
  int old_value;
  char *at, *nv, *nval;
...
void initialize_readline() {
  ...
  if (bash_readline_initialized) return;
  ...
  if (RL_ISSTATE(RL_STATE_INITIALIZED) == 0) rl_initialize();
...
void bashline_set_event_hook() { rl_signal_event_hook = bash_event_hook; }

void bashline_reset_event_hook() { rl_signal_event_hook = 0; }
...
/* Call this to set the initial text for the next line to read
   from readline. */
int bash_re_edit(line) char *line;
{
  FREE(push_to_readline);
...
static void initialize_hostname_list() {
  ...
  if (temp == 0) temp = get_string_value("hostname_completion_file");
  if (temp == 0) temp = DEFAULT_HOSTS_FILE;

  snarf_hosts_from_file(temp);

  if (hostname_list) hostname_list_initialized++;
}
...
char **get_hostname_list() {
  if (hostname_list_initialized == 0) initialize_hostname_list();
...
void clear_hostname_list() {
  ...
  if (hostname_list_initialized == 0) return;
  for (i = 0; i < hostname_list_length; i++) free(hostname_list[i]);
...
static char **hostnames_matching(text) char *text;
{
  ...
  if (hostname_list_initialized == 0) return ((char **)NULL);

  /* Special case.  If TEXT consists of nothing, then the whole list is
     what is desired. */
  if (*text == '\0') {
    result = strvec_create(1 + hostname_list_length);
    for (i = 0; i < hostname_list_length; i++) result[i] = hostname_list[i];
    result[i] = (char *)NULL;
    return (result);
  }
  ...
  for (i = nmatch = rsize = 0; i < hostname_list_length; i++) {
    if (STREQN(text, hostname_list[i], len) == 0) continue;
  ...
  if (nmatch) result[nmatch] = (char *)NULL;
  return (result);
}
...
static int set_saved_history() {
  /* XXX - compensate for assumption that history was `shuffled' if it was
     actually not. */
  if (HISTORY_FULL() && hist_last_line_added == 0 &&
...
static int operate_and_get_next(count, c) int count, c;
{
  int where;
...
static int edit_and_execute_command(count, c, editing_mode,
                                    edit_command) int count,
    c, editing_mode;
char *edit_command;
{
  ...
  if (rl_prep_term_function) (*rl_prep_term_function)(metaflag);
...
#if defined(VI_MODE)
static int vi_edit_and_execute_command(count, c) int count, c;
{
  if (posixly_correct)
    ...
    return (
        edit_and_execute_command(count, c, VI_EDITING_MODE, VI_EDIT_COMMAND));
}
#endif /* VI_MODE */

static int emacs_edit_and_execute_command(count, c) int count, c;
{
...
#if defined(ALIAS)
static int posix_edit_macros(count, key) int count, key;
{
  int c;
...
static int bash_forward_shellword(count, key) int count, key;
{
  size_t slen;
  int sindex, c, p;
  DECLARE_MBSTATE;

  if (count < 0) return (bash_backward_shellword(-count, key));
...
static int bash_backward_shellword(count, key) int count, key;
{
  size_t slen;
  int sindex, c, p;
  DECLARE_MBSTATE;

  if (count < 0) return (bash_forward_shellword(-count, key));
  ...
  while (count) {
    ...
    while (p > 0) {
      c = rl_line_buffer[p];
      if (WORDDELIM(c) && char_is_quoted(rl_line_buffer, p) == 0) break;
      BACKUP_CHAR(rl_line_buffer, slen, p);
    }
...
static int bash_kill_shellword(count, key) int count, key;
{
  int p;

  if (count < 0) return (bash_backward_kill_shellword(-count, key));

  p = rl_point;
  bash_forward_shellword(count, key);

  if (rl_point != p) rl_kill_text(p, rl_point);
...
static int bash_backward_kill_shellword(count, key) int count, key;
{
  int p;

  if (count < 0) return (bash_kill_shellword(-count, key));
...
/* check for redirections and other character combinations that are not
   command separators */
static int check_redir(ti) int ti;
{
 ...
 */
static int find_cmd_start(start) int start;
{
  ...
  while (((s = skip_to_delim(rl_line_buffer, os, COMMAND_SEPARATORS,
    ...
    if (rl_line_buffer[s] == '|' && rl_line_buffer[s - 1] == '>') {
      ns = skip_to_delim(rl_line_buffer, s + 1, COMMAND_SEPARATORS,
                         SD_NOJMP | SD_COMPLETE /*|SD_NOSKIPCMD*/);
      if (ns > start || rl_line_buffer[ns] == 0) return os;
      os = ns + 1;
      continue;
    }
    os = s + 1;
...
static int find_cmd_end(end) int end;
{
  register int e;
...
static char *find_cmd_name(start, sp, ep) int start;
int *sp, *ep;
{
  char *name;
  register int s, e;
...
int ind;
{
  ...
  pind = ind - 1;
  while (pind > 0 && whitespace(rl_line_buffer[pind])) pind--;
...
int start, end;
{
  ...
  while ((ti > -1) && (whitespace(rl_line_buffer[ti]))) ti--;
  ...
  if (ti < 0) {
    ...
    if (check_redir(ti) == 1) in_command_position = 0;
  } else {
  ...
  if (matches == 0 && (in_command_position == 0 || text[0] == '\0') &&
    ...
    if (prog_complete_matches) free(prog_complete_matches);
    ...
    if (start == 0 && end == 0 && e != 0 &&
      ...
      prog_complete_matches =
          programmable_completions("_EmptycmD_", text, s, e, &foundcs);
      ...
      foundcs = 0; /* whitespace before command name */
             ...
             rl_line_buffer[e] == 0 && whitespace(rl_line_buffer[e - 1]) == 0) {
    ...
    if (foundcs) {
      ...
      if (matches || ((foundcs & COPT_BASHDEFAULT) == 0)) return (matches);
    }
  }
#endif
  ...
  if (matches == 0) {
    dflags = 0;
    if (in_command_position) dflags |= DEFCOMP_CMDPOS;
    ...
    if (qc != '\'' && text[1] == '(') /* ) */
      ...
      if (matches && matches[0] && matches[1] == 0) {
        ...
        /* doesn't use test_for_directory because that performs tilde
           expansion */
        if (file_isdir(t)) rl_completion_append_character = '/';
        free(t);
      }
    }
  }
...
static int bash_command_name_stat_hook(name) char **name;
{
  char *cname, *result;

  /* If it's not something we're going to look up in $PATH, just call the
     normal filename stat hook. */
  if (absolute_program(*name)) return (bash_filename_stat_hook(name));
...
int state;
{
  ...
  if (state == 0) {
    ...
    if (dequoted_hint && dequoted_hint != hint) free(dequoted_hint);
    if (hint) free(hint);
    ...
    if (globpat || absolute_program(hint_text)) {
      ...
      if (filename_hint) free(filename_hint);
  ...
  switch (mapping_over) {
    case 0: /* Aliases come first. */
      ...
      while (alias_list && alias_list[local_index]) {
        ...
        if (STREQN(alias, hint, hint_len)) return (savestring(alias));
      }
#endif /* ALIAS */
      local_index = 0;
      mapping_over++;
    ...
    case 2: /* Then function names. */
      while (varlist && varlist[local_index]) {
        ...
        if (STREQN(varname, hint, hint_len)) return (savestring(varname));
      }
      local_index = 0;
      mapping_over++;
...
outer:
  ...
  if (istate == 0) {
    ...
    if (current_path[0] == '.' && current_path[1] == '\0') dot_in_path = 1;

    if (fnhint && fnhint != filename_hint) free(fnhint);
    if (filename_hint) free(filename_hint);
...
inner:
  ...
  if (mapping_over == 4 && dircomplete_expand) set_directory_hook();
  ...
  if (val == 0) {
    ...
    if (absolute_program(hint)) return ((char *)NULL);
...
/* If we have found a match, and it is an executable file, return it.
   ...
   relative pathnames.  */
    ...
    if (match &&
      ...
      if (cval != val) free(cval);
      free(val);
      val = ""; /* So it won't be NULL. */
      return (temp);
    } else {
      if (freetemp) free(temp);
      if (cval != val) free(cval);
      free(val);
      goto inner;
    }
...
int state;
{
  ...
  if (state == 0) {
    if (filename_text) free(filename_text);
    orig_start = text;
    if (*text == '`')
      text++;
    else if (*text == '$' && text[1] == '(') /* ) */
    ...
    for (value = filename_text + strlen(filename_text) - 1;
      ...
      if (whitespace(*value) || member(*value, COMMAND_SEPARATORS)) break;
    if (value <= filename_text)
      ...
      if (whitespace(value[-1]))
        ...
        matches =
            rl_completion_matches(value, command_word_completion_function);
    }
...
int state;
{
  ...
  if (!state) {
    ...
    if (first_char == '$') first_char_loc++;

    if (text[first_char_loc] == '{') first_char_loc++;

    varname = savestring(text + first_char_loc);

    namelen = strlen(varname);
    if (varlist) strvec_dispose(varlist);
  ...
  if (!varlist || !varlist[varlist_index]) {
    ...
    if (first_char_loc) {
      value[0] = first_char;
      if (first_char_loc == 2) value[1] = '{';
    }

    strcpy(value + first_char_loc, varlist[varlist_index]);
    if (first_char_loc == 2) strcat(value, "}");
...
int state;
{
  ...
  while (srvent = getservent()) {
    afound = 0;
    if (snamelen == 0 || (STREQN(sname, srvent->s_name, snamelen))) break;
...
int state;
{
  ...
  while (grent = getgrent()) {
    if (gnamelen == 0 || (STREQN(gname, grent->gr_name, gnamelen))) break;
  }
...
static char *history_expand_line_internal(line) char *line;
{
  char *new_line;
  int old_verify;
...
static void cleanup_expansion_error() {
  ...
  hist_verify = old_verify;
#endif
  if (to_free != rl_line_buffer) FREE(to_free);
  putc('\r', rl_outstream);
  rl_forced_update_display();
}
...
static void set_up_new_line(new_line) char *new_line;
{
  ...
  if (at_end)
    ...
    rl_point = old_point;
    if (!whitespace(rl_line_buffer[rl_point])) rl_forward_word(1, 0);
  }
}

#if defined(ALIAS)
/* Expand aliases in the current readline line. */
static int alias_expand_line(count, ignore) int count, ignore;
{
  char *new_line;
...
static int history_expand_line(count, ignore) int count, ignore;
{
  char *new_line;
...
static int tcsh_magic_space(count, ignore) int count, ignore;
{
...
static int history_and_alias_expand_line(count, ignore) int count, ignore;
{
  char *new_line;
...
/* History and alias expand the line, then perform the shell word
   ...
   set of operations. */
static int shell_expand_line(count, ignore) int count, ignore;
{
  char *new_line;
...
sh_ignore_func_t *name_func;
{
  ...
  /* Copy the acceptable names back to NAMES, set the new array end,
     and return. */
  for (nidx = 1; newnames[nidx]; nidx++) names[nidx] = newnames[nidx];
...
static int filename_completion_ignore(names) char **names;
{
#if 0
  ...
  if (fignore.num_ignores == 0) return 0;
...
static int bash_ignore_filenames(names) char **names;
{
...
static int bash_ignore_everything(names) char **names;
{
...
static char *restore_tilde(val, directory_part) char *val, *directory_part;
{
  int l, vl, dl2, xl;
  char *dh2, *expdir, *ret;
...
static char *maybe_restore_tilde(val, directory_part) char *val,
    *directory_part;
{
  rl_icppfunc_t *save;
  char *ret;
  ...
  if (save) restore_directory_hook(save);
  return ret;
}
...
static char *bash_filename_rewrite_hook(fname, fnlen) char *fname;
int fnlen;
{
  char *conv;

  conv = fnx_fromfs(fname, fnlen);
  if (conv != fname) conv = savestring(conv);
  return conv;
}
...
/* Check whether not the (dequoted) version of DIRNAME, with any trailing slash
   removed, exists. */
static int directory_exists(dirname) const char *dirname;
{
  char *new_dirname;
  int dirlen, r;
  struct stat sb;
  ...
  if (new_dirname[dirlen - 1] == '/') new_dirname[dirlen - 1] = '\0';
...
static int bash_filename_stat_hook(dirname) char **dirname;
{
  ...
  if (should_expand_dirname) {
    ...
    wl = expand_prompt_string(new_dirname, 0,
                              W_NOCOMSUB); /* does the right thing */
...
static int bash_directory_completion_hook(dirname) char **dirname;
{
  char *local_dirname, *new_dirname, *t;
  ...
  if (should_expand_dirname) {
    ...
    new_dirname =
        bash_dequote_filename(local_dirname, rl_completion_quote_character);
...
static char *history_completion_generator(hint_text, state) const
    char *hint_text;
int state;
{
...
static int dynamic_complete_history(count, key) int count, key;
{
  int r;
...
static int bash_dabbrev_expand(count, key) int count, key;
{
  ...
  if (rl_last_func == bash_dabbrev_expand) rl_last_func = rl_menu_complete;
...
#if defined(SPECIFIC_COMPLETION_FUNCTIONS)
static int bash_complete_username(ignore, ignore2) int ignore, ignore2;
{
  return bash_complete_username_internal(
      rl_completion_mode(bash_complete_username));
}

static int bash_possible_username_completions(ignore, ignore2) int ignore,
    ignore2;
{ return bash_complete_username_internal('?'); }

static int bash_complete_username_internal(what_to_do) int what_to_do;
{
  return bash_specific_completion(what_to_do, rl_username_completion_function);
}

static int bash_complete_filename(ignore, ignore2) int ignore, ignore2;
{
  return bash_complete_filename_internal(
      rl_completion_mode(bash_complete_filename));
}

static int bash_possible_filename_completions(ignore, ignore2) int ignore,
    ignore2;
{ return bash_complete_filename_internal('?'); }

static int bash_complete_filename_internal(what_to_do) int what_to_do;
{
  rl_compentry_func_t *orig_func;
  rl_completion_func_t *orig_attempt_func;
...
static int bash_complete_hostname(ignore, ignore2) int ignore, ignore2;
{
  return bash_complete_hostname_internal(
      rl_completion_mode(bash_complete_hostname));
}

static int bash_possible_hostname_completions(ignore, ignore2) int ignore,
    ignore2;
{ return bash_complete_hostname_internal('?'); }

static int bash_complete_variable(ignore, ignore2) int ignore, ignore2;
{
  return bash_complete_variable_internal(
      rl_completion_mode(bash_complete_variable));
}

static int bash_possible_variable_completions(ignore, ignore2) int ignore,
    ignore2;
{ return bash_complete_variable_internal('?'); }

static int bash_complete_command(ignore, ignore2) int ignore, ignore2;
{
  return bash_complete_command_internal(
      rl_completion_mode(bash_complete_command));
}

static int bash_possible_command_completions(ignore, ignore2) int ignore,
    ignore2;
{ return bash_complete_command_internal('?'); }

static int bash_complete_hostname_internal(what_to_do) int what_to_do;
{ return bash_specific_completion(what_to_do, hostname_completion_function); }

static int bash_complete_variable_internal(what_to_do) int what_to_do;
{ return bash_specific_completion(what_to_do, variable_completion_function); }

static int bash_complete_command_internal(what_to_do) int what_to_do;
{
...
int state;
{
  ...
  if (state == 0) {
    ...
    FREE(matches);
    if (globorig != globtext) FREE(globorig);
    FREE(globtext);
    ...
    if (ttext != text) free(ttext);

    matches = shell_glob_filename(globtext);
    if (GLOB_FAILED(matches)) matches = (char **)NULL;
    ind = 0;
  }
...
static int bash_glob_completion_internal(what_to_do) int what_to_do;
...
static char *bash_glob_quote_filename(s, rtype, qcp) char *s;
int rtype;
char *qcp;
{
...
static int bash_glob_complete_word(count, key) int count, key;
{
  int r;
...
static int bash_glob_expand_word(count, key) int count, key;
{ return bash_glob_completion_internal('*'); }

static int bash_glob_list_expansions(count, key) int count, key;
{ return bash_glob_completion_internal('?'); }

static int bash_specific_completion(what_to_do, generator) int what_to_do;
...
static int bash_vi_complete(count, key) int count, key;
{
#if defined(SPECIFIC_COMPLETION_FUNCTIONS)
  int p, r;
  char *t;
  ...
  if ((rl_point < rl_end) && (!whitespace(rl_line_buffer[rl_point]))) {
    if (!whitespace(rl_line_buffer[rl_point + 1])) rl_vi_end_word(1, 'E');
    rl_point++;
  }
  ...
  if (key == '*' || key == '\\') rl_vi_start_inserting(key, 1, 1);
...
static char *bash_dequote_filename(text, quote_char) char *text;
int quote_char;
{
  char *ret, *p, *r;
  int l, quoted;
  ...
  for (quoted = quote_char, p = text, r = ret; p && *p; p++) {
    ...
    if (*p == '\\') {
      ...
      *r++ = *++p;
      if (*p == '\0') return ret; /* XXX - was break; */
      continue;
    }
    /* Close quote. */
    if (quoted && *p == quoted) {
...
static char *quote_word_break_chars(text) char *text;
{
  char *ret, *r, *s;
  int l;
...
static void set_filename_bstab(string) const char *string;
{
  ...
  for (s = string; s && *s; s++) filename_bstab[*s] = 1;
}
...
/* Quote a filename using double quotes, single quotes, or backslashes
   ...
   memory. */
static char *bash_quote_filename(s, rtype, qcp) char *s;
int rtype;
char *qcp;
{
  ...
  if (rtype == MULT_MATCH && cs != COMPLETE_BSQUOTE) ret[rlen - 1] = '\0';
  free(rtext);
  return ret;
}
...
int key;
{
  ...
  l1 = v ? value_cell(v) : 0;
  if (l1 != l) maybe_make_readline_line(value_cell(v));
...
static void init_unix_command_map() { cmd_xmap = rl_make_bare_keymap(); }

static int isolate_sequence(string, ind, need_dquote, startp) char *string;
...
int bind_keyseq_to_unix_command(line) char *line;
{
  Keymap kmap;
  char *kseq, *value;
  int i, kstart;

  if (cmd_xmap == 0) init_unix_command_map();
...
char **bash_directory_completion_matches(text) const char *text;
{
  ...
  if (dfn != text) free(dfn);

  if (m1 == 0 || m1[0] == 0) return m1;
