...
static char *xmalloc(size) unsigned size;
{
...
static char *xrealloc(ptr, size) char *ptr;
unsigned size;
{
...
static char *find_capability(bp, cap) register char *bp, *cap;
{
  for (; *bp; bp++)
...
__private_extern__ int tgetnum(cap) char *cap;
{
...
__private_extern__ int tgetflag(cap) char *cap;
{
  register char *ptr = find_capability(term_entry, cap);
  return ptr && ptr[-1] == ':';
}
...
__private_extern__ char *tgetstr(cap, area) char *cap;
char **area;
{
  register char *ptr = find_capability(term_entry, cap);
  if (!ptr || (ptr[-1] != '=' && ptr[-1] != '~')) return NULL;
...
static char *tgetst1(ptr, area) char *ptr;
char **area;
{
  register char *p, *r;
  register int c;
...
__private_extern__ int tputs(str, nlines, outfun) register char *str;
int nlines;
register int (*outfun)();
{
  ...
  if (speed > 10000) speed = -speed / 100;
#else
  if (ospeed == 0)
...
static int valid_filename_p(fn) char *fn;
{
...
__private_extern__ int tgetent(bp, name) char *bp, *name;
{
  ...
  if (termcap_name && *termcap_name == '\0') termcap_name = NULL;
#if 0
  ...
  if (!termcap_name || !filep) termcap_name = TERMCAP_FILE;
...
/* Here we know we must search a file and termcap_name has its name.  */
  ...
  while (term) {
    ...
    if (scan_file(term, fd, &buf) == 0) {
      close(fd);
      free(buf.beg);
      if (malloc_size) free(bp);
      return 0;
    }

    /* Free old `term' if appropriate.  */
    if (term != name) free(term);
  ...
  if (malloc_size) bp = (char *)xrealloc(bp, bp1 - bp + 1);
...
static int scan_file(str, fd, bufp) char *str;
int fd;
register struct buffer *bufp;
{
  ...
  *bufp->ptr = '\0';
  ...
  while (!bufp->ateof) {
    ...
    if (*bufp->ptr != '#' && name_match(bufp->ptr, str)) return 1;
...
static int name_match(line, name) char *line, *name;
{
  register char *tem;

  if (!compare_contin(line, name)) return 1;
  /* This line starts an entry.  Is it the right one?  */
  for (tem = line; *tem && *tem != '\n' && *tem != ':'; tem++)
    if (*tem == '|' && !compare_contin(tem + 1, name)) return 1;

  return 0;
}

static int compare_contin(str1, str2) register char *str1, *str2;
{
  register int c1, c2;
  while (1) {
    c1 = *str1++;
    ...
    if (c2 == '\0') {
      ...
      if (c1 == '|' || c1 == ':') /* If end of name in data base, we win.  */
        return 0;
      else
        return 1;
    } else if (c1 != c2)
...
static char *gobble_line(fd, bufp, append_end) int fd;
register struct buffer *bufp;
char *append_end;
{
  register char *end;
  ...
  if (!append_end) append_end = bufp->ptr;

  while (1) {
    end = append_end;
    while (*end && *end != '\n') end++;
    if (*end) break;
    if (bufp->ateof) return buf + bufp->full;
    if (bufp->ptr == buf) {
