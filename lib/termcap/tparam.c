...
/* VARARGS 2 */
char *tparam(string, outstring, len, arg0, arg1, arg2, arg3) char *string;
char *outstring;
int len;
...
__private_extern__ char *tgoto(cm, hpos, vpos) char *cm;
int hpos, vpos;
{
  int args[2];
  if (!cm) return NULL;
...
static char *tparam1(string, outstring, len, up, left, argp) char *string;
char *outstring;
int len;
char *up, *left;
register int *argp;
{
  ...
  *op = 0;
  while (doup-- > 0) strcat(op, up);
  while (doleft-- > 0) strcat(op, left);
  return outstring;
}
