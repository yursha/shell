...
static int rangecmp(c1, c2, forcecoll) int c1, c2;
int forcecoll;
{
  static char s1[2] = {' ', '\0'};
  static char s2[2] = {' ', '\0'};
  ...
  if (forcecoll == 0 && glob_asciirange) return (c1 - c2);
...
#if defined(HAVE_STRCOLL)
static int collequiv(c1, c2) int c1, c2;
...
static int collsym(s, len) CHAR *s;
int len;
{
  ...
  for (csp = posix_collsyms; csp->name; csp++) {
    if (STREQN(csp->name, x, len) && csp->name[len] == '\0') return (csp->code);
  }
  if (len == 1) return s[0];
  return INVALID;
}
...
static int is_cclass(c, name) int c;
const char *name;
{
...
#define FOLD(c) \
  ((flags & FNM_CASEFOLD) ? TOLOWER((unsigned char)c) : ((unsigned char)c))
...
static int rangecmp_wc(c1, c2, forcecoll) wint_t c1, c2;
int forcecoll;
{
...
static int collequiv_wc(c, equiv) wint_t c, equiv;
{ return (c == equiv); }
...
static wint_t collwcsym(s, len) wchar_t *s;
int len;
{
...
static int is_wcclass(wc, name) wint_t wc;
wchar_t *name;
{
  char *mbs;
  mbstate_t state;
...
int xstrmatch(pattern, string, flags) char *pattern;
char *string;
int flags;
{
#if HANDLE_MULTIBYTE
