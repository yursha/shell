...
static int extglob_skipname(pat, dname, flags) char *pat, *dname;
int flags;
{
  char *pp, *pe, *t, *se;
  int n, r, negate;
...
static int skipname(pat, dname, flags) char *pat;
char *dname;
int flags;
{
#if EXTENDED_GLOB
...
static int wchkname(pat_wc, dn_wc) wchar_t *pat_wc, *dn_wc;
{
...
static int wextglob_skipname(pat, dname, flags) wchar_t *pat, *dname;
int flags;
{
#if EXTENDED_GLOB
...
static int mbskipname(pat, dname, flags) char *pat, *dname;
int flags;
{
  int ret, ext;
  ...
  if (pat_n != (size_t)-1) dn_n = xdupmbstowcs(&dn_wc, NULL, dname);

  ret = 0;
  if (pat_n != (size_t)-1 && dn_n != (size_t)-1)
    ret =
        ext ? wextglob_skipname(pat_wc, dn_wc, flags) : wchkname(pat_wc, dn_wc);
  else
...
static void udequote_pathname(pathname) char *pathname;
{
  register int i, j;
  ...
  for (i = j = 0; pathname && pathname[i];) {
    ...
    if (pathname[i - 1] == 0) break;
  }
  if (pathname) pathname[j] = '\0';
}
...
static void wdequote_pathname(pathname) char *pathname;
{
  ...
  for (i = j = 0; wpathname && wpathname[i];) {
    ...
    if (wpathname[i - 1] == L'\0') break;
  }
  if (wpathname) wpathname[j] = L'\0';
...
static int glob_testdir(dir, flags) char *dir;
int flags;
{
  struct stat finfo;
  int r;
...
/*itrace("glob_testdir: testing %s" flags = %d, dir, flags);*/
  ...
  if (S_ISDIR(finfo.st_mode) == 0) return (-1);
...
static struct globval *finddirs(pat, sdir, flags, ep, np) char *pat;
char *sdir;
int flags;
struct globval **ep;
int *np;
{
  ...
  if (r == 0 || r[0] == 0) {
    if (np) *np = 0;
    if (ep) *ep = 0;
    if (r && r != &glob_error_return) free(r);
...
char **glob_vector(pat, dir, flags) char *pat;
char *dir;
int flags;
{
  DIR *d;
  ...
  if (pat == 0 || *pat == '\0') {
    if (glob_testdir(dir, 0) < 0) return ((char **)&glob_error_return);

    nextlink = (struct globval *)alloca(sizeof(struct globval));
    if (nextlink == NULL) return ((char **)NULL);
...
/* Open the directory, punting immediately if we cannot.  If opendir
   ...
   that DIR is a directory and punt if it's not. */
    ...
    d = opendir(dir);
    if (d == NULL) return ((char **)&glob_error_return);
...
#ifdef FNM_CASEFOLD
    if (glob_ignore_case) mflags |= FNM_CASEFOLD;
#endif

    if (extended_glob) mflags |= FNM_EXTMATCH;

    add_current =
        ((flags & (GX_ALLDIRS | GX_ADDCURDIR)) == (GX_ALLDIRS | GX_ADDCURDIR));
    ...
    while (1) {
      ...
      if (REAL_DIR_ENTRY(dp) == 0) continue;
      ...
      if (flags & (GX_MATCHDIRS | GX_ALLDIRS)) {
        ...
        if (flags & GX_NULLDIR) pflags |= MP_IGNDOT;
      ...
      if (flags & GX_ALLDIRS) {
        if (isdir == 0) {
          ...
          if (ndirs) /* add recursive directories to list */
              {
            if (firstmalloc == 0) firstmalloc = e;
            e->next = lastlink;
            lastlink = dirlist;
            count += ndirs;
          }
  ...
  if (lose) {
    ...
    while (lastlink) {
      ...
      if (firstmalloc) {
        if (lastlink == firstmalloc) firstmalloc = 0;
        tmplink = lastlink;
      } else
        tmplink = 0;
      free(lastlink->name);
...
static char **glob_dir_to_array(dir, array, flags) char *dir, **array;
int flags;
{
  ...
  for (i = 0; array[i] != NULL; i++) {
    ...
    if (result[i] == NULL) {
      int ind;
      for (ind = 0; ind < i; ind++) free(result[ind]);
      free(result);
      return (NULL);
    }

    strcpy(result[i], dir);
    if (add_slash) result[i][l] = '/';
...
char **glob_filename(pathname, flags) char *pathname;
int flags;
{
  char **result;
  ...
  if (directory_len > 0 && glob_pattern_p(directory_name)) {
    ...
    if ((flags & GX_GLOBSTAR) && all_starstar == 0) {
      ...
      while (dl >= 4 && d[dl - 1] == '/' && d[dl - 2] == '*' &&
             d[dl - 3] == '*' && d[dl - 4] == '/')
        prev = dl, dl -= 3;
      if (dl != directory_len) last_starstar = 1;
      directory_len = prev;
    }
    ...
    if (d[directory_len - 1] == '/') d[directory_len - 1] = '\0';
    ...
    for (i = 0; directories[i] != NULL; ++i) {
      ...
      if (dname[0] == '\0' && filename[0]) {
        dflags |= GX_NULLDIR;
        dname = "."; /* treat null directory name and non-null filename as
                        current directory */
      }
...
#undef NULL_PLACEHOLDER
        ...
        if (result == NULL) {
          for (l = 0; array[l]; ++l) free(array[l]);
          free((char *)array);
          goto memory_error;
        }

        for (l = 0; array[l] != NULL; ++l) result[result_size++ - 1] = array[l];
    ...
    for (i = 0; directories[i]; i++) free(directories[i]);
...
only_filename:
  ...
  if (*filename == '\0') {
    ...
    if (result == NULL) {
      if (free_dirname) free(directory_name);
      return (NULL);
    }
    /* Handle GX_MARKDIRS here. */
    result[0] = (char *)malloc(directory_len + 1);
    if (result[0] == NULL) goto memory_error;
    ...
    if (directory_len > 0) dequote_pathname(directory_name);
    ...
    dflags = flags & ~GX_MARKDIRS;
    if (directory_len == 0) dflags |= GX_NULLDIR;
    if ((flags & GX_GLOBSTAR) && filename[0] == '*' && filename[1] == '*' &&
        filename[2] == '\0') {
      dflags |= GX_ALLDIRS | GX_ADDCURDIR;
#if 0
...
memory_error:
  if (result != NULL) {
    register unsigned int i;
    for (i = 0; result[i] != NULL; ++i) free(result[i]);
    free((char *)result);
  }

  if (free_dirname && directory_name) free(directory_name);
...
char **argv;
{
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (value == NULL)
      ...
      for (i = 0; value[i] != NULL; i++) puts(value[i]);
  }
