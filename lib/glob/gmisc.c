...
int match_pattern_wchar(wpat, wstring, flags) wchar_t *wpat, *wstring;
int flags;
{
  wchar_t wc;
...
int wmatchlen(wpat, wmax) wchar_t *wpat;
size_t wmax;
{
  wchar_t wc;
      ...
      case L'[':
        ...
        do {
          if (wc == 0) {
            ...
            bracklen++;
            if (*wpat ==
                L']') /* right bracket can appear as collating symbol */
                {
              wpat++;
              bracklen++;
            }
            ...
            bracklen++;
            if (*wpat ==
                L']') /* right bracket can appear as equivalence class */
                {
              wpat++;
              bracklen++;
            }
...
int extglob_pattern_p(pat) char *pat;
{
  switch (pat[0]) {
    case '*':
    case '+':
...
/* Return 1 of the first character of STRING could match the first
   ...
   matching for now. */
int match_pattern_char(pat, string, flags) char *pat, *string;
int flags;
{
  char c;
...
int umatchlen(pat, max) char *pat;
size_t max;
{
  char c;
  int matlen, bracklen, t, in_cclass, in_collsym, in_equiv;
...
/* Skip characters in PAT and return the final occurrence of DIRSEP.  This
   ...
   patterns x(...) */
char *glob_dirscan(pat, dirsep) char *pat;
int dirsep;
{
  char *p, *d, *pe, *se;
