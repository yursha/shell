...
int FCT(pattern, string, flags) CHAR *pattern;
CHAR *string;
int flags;
{
  CHAR *se, *pe;
...
static int GMATCH(string, se, pattern, pe, flags) CHAR *string, *se;
CHAR *pattern, *pe;
int flags;
{
  ...
  while (p < pe) {
    ...
    switch (c) {
      ...
      case L('\\'): /* backslash escape removes special meaning */
        ...
        if (FOLD(sc) != (U_CHAR)c) return FNM_NOMATCH;
        break;
   ...
   is something that matches the empty string, we can succeed. */
        ...
        /* General case, use recursion. */
        {
          ...
          for (--p; n < endp; ++n) {
            ...
            if (GMATCH(n, se, p, pe, flags & ~FNM_PERIOD) == 0) return (0);
          }
          return FNM_NOMATCH;
        }

      case L('['): {
        if (sc == L('\0') || n == se) return FNM_NOMATCH;
        ...
        if (p == 0) return FNM_NOMATCH;
      } break;

      default:
        if ((U_CHAR)c != FOLD(sc)) return (FNM_NOMATCH);
    }
...
static CHAR *PARSE_COLLSYM(p, vp) CHAR *p;
INT *vp;
{
  register int pc;
  INT val;
...
#else
    BRACKMATCH(p, test, flags) CHAR *p;
U_CHAR test;
int flags;
#endif
{
  ...
  for (;;) {
    ...
    if (c == L('[') && *p == L('=') && p[2] == L('=') && p[3] == L(']')) {
      ...
      if (COLLEQUIV(test, pc)) {
        ...
        c = *p++;
        if (c == L('\0')) return ((test == L('[')) ? savep : (CHAR *)0); /*]*/
        c = FOLD(c);
        continue;
      }
    }
    ...
    if (c == L('-') && *p != L(']')) {
      cend = *p++;
      if (!(flags & FNM_NOESCAPE) && cend == L('\\')) cend = *p++;
      if (cend == L('\0')) return (CHAR *)0;
      if (cend == L('[') && *p == L('.')) {
        p = PARSE_COLLSYM(p, &pc);
    ...
    if (RANGECMP(test, cstart, forcecoll) >= 0 &&
...
/* Scan a pattern starting at STRING and ending at END, keeping track of
   ...
   empty or invalid. */
/*static*/ CHAR *PATSCAN(string, end, delim) CHAR *string, *end;
INT delim;
{
  ...
  for (s = string; c = *s; s++) {
    ...
    switch (c) {
      ...
      case L('|'):
        if (bnest == 0 && pnest == 0 && delim == L('|')) return ++s;
        break;
    }
  }
...
static int STRCOMPARE(p, pe, s, se) CHAR *p, *pe, *s, *se;
{
  int ret;
  CHAR c1, c2;
  int l1, l2;
  ...
  if (l1 != l2) return (FNM_NOMATCH); /* unequal lengths, can't be identical */
...
/* Match a ksh extended pattern specifier.  Return FNM_NOMATCH on failure or
   ...
   gmatch recursively. */
static int EXTMATCH(xc, s, se, p, pe,
                    flags) INT xc; /* select which operation */
CHAR *s, *se;
CHAR *p, *pe;
int flags;
{
  ...
  switch (xc) {
    ...
    case L('*'): /* match zero or more occurrences */
                 /* If we can get away with no matches, don't even bother.  Just
                    ...
                    it succeeds. */
      if (xc == L('*') && (GMATCH(s, se, prest, pe, flags) == 0)) return 0;
    ...
    case '!': /* match anything *except* one of the patterns */
      for (srest = s; srest <= se; srest++) {
        ...
        for (psub = p + 1;; psub = pnext) {
          ...
          if (m1 = (GMATCH(s, srest, psub, pnext - 1, flags) == 0)) break;
          if (pnext == prest) break;
        }
        /* if srest > s, we are not at start of string */
        xflags = (srest > s) ? (flags & ~FNM_PERIOD) : flags;
        if (m1 == 0 && GMATCH(srest, se, prest, pe, xflags) == 0) return (0);
      }
      return (FNM_NOMATCH);
  }
