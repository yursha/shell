...
size_t xmbsrtowcs(dest, src, len, pstate) wchar_t *dest;
const char **src;
size_t len;
mbstate_t *pstate;
{
  ...
  for (wclength = 0; wclength < len; wclength++, dest++) {
    ...
    if (mblength == (size_t)-1 || mblength == (size_t)-2) return (size_t)-1;
...
const char *src;     /* Multibyte character string */
{
  ...
  do {
    ...
    if (wsbuf_size <
        wcnum + wcslength + 1) /* 1 for the L'\0' or the potential L'\\' */
        {
      wchar_t *wstmp;
...
const char *src;     /* Multibyte character string */
{
  ...
  if (src == NULL || destp == NULL) {
    if (destp) *destp = NULL;
    if (indicesp) *indicesp = NULL;
    return (size_t)-1;
  }

#if HAVE_MBSNRTOWCS
  if (indicesp == NULL) return (xdupmbstowcs2(destp, src));
#endif
  ...
  do {
    ...
    wsbuf[wcnum - 1] = wc;
    if (indices) indices[wcnum - 1] = (char *)p;
    p += mblength;
  } while (MB_NULLWCH(wc) == 0);

  /* Return the length of the wide character string, not including `\0'. */
  *destp = wsbuf;
  if (indicesp != NULL) *indicesp = indices;
