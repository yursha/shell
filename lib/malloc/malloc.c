...
static const unsigned long binsizes[NBUCKETS] = {
    8UL,         16UL,        32UL,         64UL,         128UL,
    256UL,       512UL,       1024UL,       2048UL,       4096UL,
    8192UL,      16384UL,     32768UL,      65536UL,      131072UL,
    262144UL,    524288UL,    1048576UL,    2097152UL,    4194304UL,
    8388608UL,   16777216UL,  33554432UL,   67108864UL,   134217728UL,
    268435456UL, 536870912UL, 1073741824UL, 2147483648UL, 4294967295UL};
...
static void bcoalesce(nu) register int nu;
{
  ...
  nbuck = nu - 1;
  if (nextf[nbuck] == 0 || busy[nbuck]) return;
...
static void bsplit(nu) register int nu;
{
  ...
  if (nu >= SPLIT_MID) {
    for (nbuck = split_max; nbuck > nu; nbuck--) {
      if (busy[nbuck] || nextf[nbuck] == 0) continue;
      break;
    }
  } else {
  ...
  if (nbuck > split_max || nbuck <= nu) return;
...
int nu;
{
  ...
  nbuck = nu - 1;
  while (nbuck >= SPLIT_MIN && busy[nbuck]) nbuck--;
  if (nbuck < SPLIT_MIN) return;
...
static void morecore(nu) register int nu; /* size index to get more of  */
{
  ...
  if (siz < 0) goto morecore_done; /* oops */
  ...
  if ((long)mp == -1) goto morecore_done;
...
static int pagealign() {
  ...
  if (sbrk_needed < 0) sbrk_needed += pagesz;
  ...
  for (nunits = 7; nunits < NBUCKETS; nunits++)
    if (pagesz <= binsize(nunits)) break;
  pagebucket = nunits;
...
int line, flags;
{
  ...
  if (pagesz == 0)
    if (pagealign() < 0) return ((PTR_T)NULL);
  ...
  for (; nunits < NBUCKETS; nunits++)
    if (nbytes <= binsize(nunits)) break;

  /* Silently reject too-large requests. */
  if (nunits >= NBUCKETS) return ((PTR_T)NULL);
...
/* In case this is reentrant use of malloc from signal handler,
  ...
  if (nunits > maxbuck) maxbuck = nunits;

  /* If there are no blocks of the appropriate size, go get some */
  if (nextf[nunits] == 0) morecore(nunits);
  ...
  if (p->mh_alloc != ISFREE || p->mh_index != nunits)
    xbotch((PTR_T)(p + 1), 0, _("malloc: block on free list clobbered"), file,
           line);
...
#ifdef MALLOC_WATCH
  if (_malloc_nwatch > 0) _malloc_ckwatch(p + 1, file, line, W_ALLOC, n);
#endif
...
#ifdef MEMSCRAMBLE
  if (p->mh_nbytes) MALLOC_MEMSET(mem, 0xcf, p->mh_nbytes);
#endif
...
#ifdef MALLOC_WATCH
  if (_malloc_nwatch > 0) _malloc_ckwatch(mem, file, line, W_FREE, ubytes);
#endif
}

static PTR_T internal_realloc(mem, n, file, line, flags) PTR_T mem;
register size_t n;
const char *file;
int line, flags;
{
  ...
  *z++ = *m++, *z++ = *m++, *z++ = *m++, *z++ = *m++;
  ...
  if (n < tocopy) tocopy = n;

#ifdef MALLOC_STATS
  _mstats.nrcopy++;
#endif

  if ((m = internal_malloc(n, file, line, MALLOC_INTERNAL | MALLOC_NOTRACE |
                                              MALLOC_NOREG)) == 0)
    return 0;
...
#ifdef MALLOC_WATCH
  if (_malloc_nwatch > 0) _malloc_ckwatch(m, file, line, W_RESIZED, n);
#endif

  return m;
}

static PTR_T internal_memalign(alignment, size, file, line,
...
int line, flags;
{
  ...
  if (((long)ptr & (alignment - 1)) == 0) return ptr;
...
static PTR_T internal_valloc(size, file, line, flags) size_t size;
const char *file;
int line, flags;
{
  return internal_memalign(getpagesize(), size, file, line,
                           flags | MALLOC_INTERNAL);
}
#endif /* !NO_VALLOC */

#ifndef NO_CALLOC
static PTR_T internal_calloc(n, s, file, line, flags) size_t n, s;
const char *file;
int line, flags;
{
  size_t total;
...
#ifdef MALLOC_STATS
int malloc_free_blocks(size) int size;
{
  int nfree;
  register union mhead *p;

  nfree = 0;
  for (p = nextf[size]; p; p = CHAIN(p)) nfree++;
