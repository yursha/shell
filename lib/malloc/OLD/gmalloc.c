...
/* Aligned allocation.  */
static genptr_t align(size) size_t size;
{
  genptr_t result;
  ...
  adj =
      (unsigned long int)((unsigned long int)((char *)result - (char *)NULL)) %
      BLOCKSIZE;
  if (adj != 0) {
    genptr_t new;
    adj = BLOCKSIZE - adj;
...
static genptr_t get_contiguous_space(size, position) ptrdiff_t size;
genptr_t position;
{
  genptr_t before;
  genptr_t after;
...
inline static void register_heapinfo() {
  size_t block, blocks;
  ...
  while (--blocks > 0) _heapinfo[block + blocks].busy.info.size = -blocks;
}

/* Set everything up and remember that we have.  */
static int malloc_initialize() {
...
static genptr_t default_morecore(increment) ptrdiff_t increment;
{
  genptr_t result;
...
static genptr_t morecore(size) size_t size;
{
  genptr_t result;
  ...
  if ((size_t)BLOCK((char *)result + size) > heapsize) {
    ...
    if (_heaplimit != 0) {
      ...
      newinfo =
          (malloc_info *)irealloc(_heapinfo, newsize * sizeof(malloc_info));
    ...
    while (1) {
      ...
      /* Is it big enough to record status for its own space?
         If so, we win.  */
      if ((size_t)BLOCK((char *)newinfo + newsize * sizeof(malloc_info)) <
          newsize)
        break;
...
static genptr_t imalloc(size) size_t size;
{
  genptr_t result;
    ...
    if (next != NULL) {
      ...
      if (--_heapinfo[block].busy.info.frag.nfree != 0)
        _heapinfo[block].busy.info.frag.first =
            (unsigned long int)((unsigned long int)((char *)next->next -
                                                    (char *)NULL) %
                                BLOCKSIZE) >>
            log;
...
genptr_t malloc(size) size_t size;
{
#ifdef RCHECK
  struct hdr *hdr;
#endif
  ...
  if (malloc_initialized == 0 && malloc_initialize() == 0) return NULL;
...
static void ifree(ptr) genptr_t ptr;
{
  int type;
  size_t block, blocks;
  register size_t i;
  ...
  switch (type) {
    case 0:
      ...
      i = _heapindex;
      if (i > block)
        while (i > block) i = _heapinfo[i].free.prev;
      else {
        do
      ...
      if (_heaplimit != 0 && curbrk == ADDRESS(_heaplimit)) {
        ...
        if (/* Win if this block being freed is last in core, the info table
            ...
            (block + blocks == _heaplimit &&
             info_block + info_blocks == block && prev_block != 0 &&
             prev_block + prev_blocks == info_block &&
    ...
    default:
      ...
      if (_heapinfo[block].busy.info.frag.nfree == (BLOCKSIZE >> type) - 1) {
        ...
        next = prev;
        for (i = 1; i < (size_t)(BLOCKSIZE >> type); ++i) next = next->next;
        prev->prev->next = next;
        if (next != NULL) next->prev = prev->prev;
        _heapinfo[block].busy.type = 0;
        ...
        _heapinfo[block].busy.info.frag.first =
            (unsigned long int)((unsigned long int)((char *)ptr -
                                                    (char *)NULL) %
                                    BLOCKSIZE >>
                                type);
...
static void malloc_safe_bcopy(afrom, ato, size) genptr_t afrom;
genptr_t ato;
size_t size;
{
  char *from, *to;
  ...
  if (to < from || from + size <= to) bcopy(from, to, size);
...
static genptr_t irealloc(ptr, size) genptr_t ptr;
size_t size;
{
  genptr_t result;
  int type;
  ...
  switch (type) {
    case 0:
      ...
      if (blocks < _heapinfo[block].busy.info.size) {
        ...
        ifree(ptr);
        result = imalloc(size);
        if (_heaplimit == 0) _heaplimit = oldlimit;
        if (result == NULL) {
        ...
        if (ptr != result) memmove(result, ptr, blocks * BLOCKSIZE);
      }
      break;
...
genptr_t realloc(ptr, size) genptr_t ptr;
size_t size;
{
#ifdef RCHECK
  struct hdr *hdr;
  ...
  checkhdr(hdr);
  if (size < osize) zmemset((char *)ptr + size, FREEFLOOD, osize - size);
...
genptr_t calloc(nmemb, size) register size_t nmemb;
register size_t size;
{
  register genptr_t result;

  result = malloc(nmemb * size);
  if (result != NULL) (void)memset(result, 0, nmemb * size);

  return result;
}

/* Define the `cfree' alias for `free'.  */
void cfree(ptr) genptr_t ptr;
{ free(ptr); }

genptr_t memalign(alignment, size) size_t alignment;
size_t size;
{
  genptr_t result;
  ...
  if (adj != 0) {
    ...
    struct alignlist *l;
    for (l = _aligned_blocks; l != NULL; l = l->next)
      if (l->aligned == NULL) /* This slot is free.  Use it.  */
        break;
    if (l == NULL) {
...
genptr_t _malloc(size) size_t size;
{ return malloc(size); }

void _free(ptr) genptr_t ptr;
{ free(ptr); }

genptr_t _realloc(ptr, size) genptr_t ptr;
size_t size;
...
static void zmemset(ptr, val, size) genptr_t ptr;
int val;
size_t size;
{
  char *cp = ptr;
...
static enum mcheck_status checkhdr(hdr) const struct hdr *hdr;
{
  ...
  if (status != MCHECK_OK) mabort(status);
  return status;
}
...
static void mabort(status) enum mcheck_status status;
{
  const char *msg;
...
enum mcheck_status mprobe(ptr) genptr_t ptr;
