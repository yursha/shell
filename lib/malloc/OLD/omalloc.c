...
void malloc_init(start, warnfun) char *start;
void (*warnfun)();
{
  if (start) data_space_start = start;
  lim_data = 0;
  warnlevel = 0;
...
int malloc_usable_size(mem) char *mem;
{
...
/* Block all signals in case we are executed from a signal handler. */
  ...
  if (warnfunction) switch (warnlevel) {
      case 0:
        if (siz > (lim_data / 4) * 3) {
          warnlevel++;
  ...
  /* Take at least 2k, and figure out how many blocks of the desired size
     we're about to get */
  nblks = 1;
  if ((siz = nu) < 8) nblks = 1 << ((siz = 8) - nu);

  if ((cp = sbrk(1 << (siz + 3))) == (char *)-1) return; /* no more room! */
...
static void getpool() {
  ...
  if (_malloc_base == 0) _malloc_base = cp;
...
static char *zmemset(s, c, n) char *s;
int c;
register int n;
{
  register char *sp;
...
register unsigned int n;
{
  ...
  if ((p = (struct mhead *)mem) == 0) return malloc(n);
  p--;
  nunits = p->mh_index;
...
char *memalign(alignment, size) unsigned int alignment, size;
{
  register char *ptr;
  ...
  if (((int)ptr & (alignment - 1)) == 0) return ptr;
...
char *calloc(n, s) size_t n, s;
{
  ...
  total = n * s;
  result = malloc(total);
  if (result) zmemset(result, 0, total);
  return result;
}
...
struct mstats_value malloc_stats(size) int size;
{
  ...
  for (p = nextf[size]; p; p = CHAIN(p)) v.nfree++;
