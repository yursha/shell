    ...
    size_t n;
{
  ...
  if (pagesz == 0) {
    ...
    sbrk_needed =
        pagesz - ((long)sbrk(0) & (pagesz - 1)); /* sbrk(0) % pagesz */
...
#endif
      if ((long)sbrk(sbrk_needed) == -1) return (NULL);
    }
    nunits = 0;
    nbytes = 8;
    while (pagesz > nbytes) {
...
register size_t n;
{
  ...
  if ((m = malloc(n)) == 0) return 0;
  FASTCOPY(mem, m, tocopy);
  free(mem);
  return m;
}

char *memalign(alignment, size) unsigned int alignment;
size_t size;
{
  register char *ptr;
