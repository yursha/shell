...
#else
#define which_bucket(mem) \
  (mt_hash((unsigned char *)(mem)) & (REG_TABLE_SIZE - 1));

#define next_bucket() ((table_bucket_index + 1) & (REG_TABLE_SIZE - 1))
#define next_entry(mem) \
  ((mem == mem_table + REG_TABLE_SIZE - 1) ? mem_table : ++mem)

#define prev_bucket() \
  (table_bucket_index == 0 ? REG_TABLE_SIZE - 1 : table_bucket_index - 1)
#define prev_entry(mem) \
...
int flags;
{
  ...
  if (mem_overflow.mem == mem) return (&mem_overflow);
...
int line;
{
  ...
  if (tentry != &mem_overflow) table_allocated++;
...
static char *_entry_flags(x) int x;
{
  if (x & MT_FREE)
    return "free";
  else if (x & MT_ALLOC)
  ...
  for (i = 0; i < REG_TABLE_SIZE; i++) {
    ...
    if (entry.mem)
      fprintf(fp, "%s[%d] %p:%zu:%s:%s:%s:%d:%d:%d\n",
              (i == table_bucket_index) ? "*" : "", i, entry.mem, entry.size,
              _entry_flags(entry.flags), entry.func ? entry.func : "unknown",
              entry.file ? entry.file : "unknown", entry.line, entry.nalloc,
              entry.nfree);
  }
}

void mregister_dump_table() { _register_dump_table(stderr); }
...
int line;
{
  ...
  for (tp = mlocation_table; tp <= endp; tp++) {
    if (tp->line == line && STREQ(file, tp->file)) return tp;
  }
...
static void _location_dump_table(fp) FILE *fp;
{
  ...
  for (tp = mlocation_table; tp < endp; tp++)
    fprintf(fp, "%s:%d\t%d\n", tp->file ? tp->file : "unknown",
            tp->line ? tp->line : 0, tp->nalloc);
}

void mlocation_dump_table() { _location_dump_table(stderr); }
...
void mlocation_write_table() {
  ...
  fp = _imalloc_fopen((char *)NULL, (char *)NULL, LOCROOT, defname,
                      sizeof(defname));
  if (fp == 0) return; /* XXX - no error message yet */
...
int malloc_set_register(n) int n;
{
  int old;
