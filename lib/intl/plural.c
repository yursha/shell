...
static struct expression *new_exp(nargs, op, args) int nargs;
enum operator op;
struct expression *const *args;
{
  int i;
  ...
  if (newp != NULL) {
    newp->nargs = nargs;
    newp->operation = op;
    for (i = nargs - 1; i >= 0; i--) newp->val.args[i] = args[i];
    return newp;
  }

fail:
  for (i = nargs - 1; i >= 0; i--) FREE_EXPRESSION(args[i]);
...
static inline struct expression *new_exp_3(op, bexp, tbranch, fbranch) enum
operator op;
struct expression *bexp;
struct expression *tbranch;
struct expression *fbranch;
...
/* YYPRHS[YYN] -- Index of the first RHS symbol of rule number YYN in
   YYRHS.  */
static const unsigned char yyprhs[] = {0,  0,  3,  5,  11, 15, 19,
                                       23, 27, 31, 35, 38, 40, 42};

/* YYRHS -- A `-1'-separated list of the rules' RHS. */
static const yysigned_char yyrhs[] = {
    17, 0,  -1, 18, -1, 18, 3,  18, 12, 18, -1, 18, 4,  18, -1, 18,
...
static const unsigned short int yytoknum[] = {
    0, 256, 257, 63, 124, 38, 258, 259, 260, 261, 33, 262, 58, 110, 40, 41};
#endif
...
static const yysigned_char yypact[] = {-9,  -9, -10, -10, -9, 8,  36,  -10, 13,
                                       -10, -9, -9,  -9,  -9, -9, -9,  -9,  -10,
...
static const unsigned char yystos[] = {0,  10, 11, 13, 14, 17, 18, 18, 18,
                                       0,  3,  4,  5,  6,  7,  8,  9,  15,
...
void internal_function FREE_EXPRESSION(exp) struct expression *exp;
{
  if (exp == NULL) return;
...
static int yylex(lval, pexp) YYSTYPE *lval;
const char **pexp;
{
  const char *exp = *pexp;
  int result;
