...
static SigHandler *rl_set_sighandler PARAMS((int, SigHandler *,
                                             sighandler_cxt *));
static void rl_maybe_set_sighandler PARAMS((int, SigHandler *,
                                            sighandler_cxt *));
...
int volatile _rl_caught_signal =
    0; /* should be sig_atomic_t, but that requires including <signal.h>
          everywhere */
...
static RETSIGTYPE rl_signal_handler(sig) int sig;
{
...
static RETSIGTYPE _rl_handle_signal(sig) int sig;
{
#if defined(HAVE_POSIX_SIGNALS)
  sigset_t set;
...
#if defined(SIGWINCH)
static RETSIGTYPE rl_sigwinch_handler(sig) int sig;
{
  SigHandler *oh;
...
#if !defined(HAVE_POSIX_SIGNALS)
static int rl_sigaction(sig, nh, oh) int sig;
...
static void rl_maybe_set_sighandler(sig, handler, ohandler) int sig;
SigHandler *handler;
sighandler_cxt *ohandler;
{
  ...
  if (oh == (SigHandler *)SIG_IGN) rl_sigaction(sig, ohandler, &dummy);
}
...
static void rl_maybe_restore_sighandler(sig, handler) int sig;
sighandler_cxt *handler;
{
  ...
  sigemptyset(&dummy.sa_mask);
  dummy.sa_flags = 0;
  if (handler->sa_handler != SIG_IGN) rl_sigaction(sig, handler, &dummy);
}
...
int rl_set_signals() {
  ...
  if (rl_catch_signals && signals_set_flag == 0) {
    ...
    if (oh == (SigHandler *)SIG_IGN) rl_sigaction(SIGALRM, &old_alrm, &dummy);
...
int rl_pending_signal() { return (_rl_caught_signal); }
...
/*								    */
/* **************************************************************** */
void rl_echo_signal_char(sig) int sig;
{
  char cstr[3];
  int cslen, c;
