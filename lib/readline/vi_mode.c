...
void _rl_vi_initialize_line() {
  ...
  for (i = 0; i < n; i++) vi_mark_chars[i] = -1;
...
void _rl_vi_set_last(key, repeat, sign) int key, repeat, sign;
{
...
void rl_vi_start_inserting(key, repeat, sign) int key, repeat, sign;
{
...
int _rl_vi_textmod_command(c) int c;
{ return (member(c, vi_textmod)); }

int _rl_vi_motion_command(c) int c;
{ return (member(c, vi_motion)); }

static void _rl_vi_replace_insert(count) int count;
{
  int nchars;
...
static void _rl_vi_stuff_insert(count) int count;
{
  rl_begin_undo_group();
  while (count--) rl_insert_text(vi_insert_buffer);
  rl_end_undo_group();
}
...
int rl_vi_redo(count, c) int count, c;
{
  int r;
...
int rl_vi_undo(count, key) int count, key;
{ return (rl_undo_command(count, key)); }

/* Yank the nth arg from the previous line into this line at point. */
int rl_vi_yank_arg(count, key) int count, key;
{
...
int rl_vi_fetch_history(count, c) int count, c;
{
  int wanted;
...
int rl_vi_search_again(count, key) int count, key;
{
  switch (key) {
    case 'n':
...
int rl_vi_search(count, key) int count, key;
{
  switch (key) {
    case '?':
...
int rl_vi_complete(ignore, key) int ignore, key;
{
  ...
  if (key == '*' || key == '\\') rl_vi_start_inserting(key, 1, rl_arg_sign);

  return (0);
}

/* Tilde expansion for vi mode. */
int rl_vi_tilde_expand(ignore, key) int ignore, key;
{
...
int rl_vi_prev_word(count, key) int count, key;
{
  if (count < 0) return (rl_vi_next_word(-count, key));
...
int rl_vi_next_word(count, key) int count, key;
{
  if (count < 0) return (rl_vi_prev_word(-count, key));
...
int rl_vi_end_word(count, key) int count, key;
{
  if (count < 0) {
    rl_ding();
    return 1;
...
int rl_vi_fWord(count, ignore) int count, ignore;
{
...
int rl_vi_bWord(count, ignore) int count, ignore;
{
  while (count-- && rl_point > 0) {
    ...
    while (rl_point > 0 && whitespace(rl_line_buffer[rl_point])) rl_point--;
...
int rl_vi_eWord(count, ignore) int count, ignore;
{
  while (count-- && rl_point < (rl_end - 1)) {
    if (!whitespace(rl_line_buffer[rl_point])) rl_point++;
...
int rl_vi_insert_beg(count, key) int count, key;
{
...
static void _rl_vi_append_forward(key) int key;
{
  int point;
...
int rl_vi_append_mode(count, key) int count, key;
{
...
int rl_vi_append_eol(count, key) int count, key;
{
...
int rl_vi_eof_maybe(count, c) int count, c;
{ return (rl_newline(1, '\n')); }
...
/* Switching from one mode to the other really just involves
   switching keymaps. */
int rl_vi_insertion_mode(count, key) int count, key;
{
  _rl_keymap = vi_insertion_keymap;
  _rl_vi_last_key_before_insert = key;
  if (_rl_show_mode_in_prompt) _rl_reset_prompt();
  return (0);
}

int rl_vi_insert_mode(count, key) int count, key;
{
  rl_vi_start_inserting(key, 1, rl_arg_sign);
  return (0);
}

static void vi_save_insert_buffer(start, len) int start, len;
{
...
static void _rl_vi_save_replace() {
  int len, start, end;
  UNDO_LIST *up;

  up = rl_undo_list;
  if (up == 0 || up->what != UNDO_END || vi_replace_count <= 0) {
    if (vi_insert_buffer_size >= 1) vi_insert_buffer[0] = '\0';
    return;
  }
...
static void _rl_vi_save_insert(up) UNDO_LIST *up;
{
  int len, start, end;
...
void _rl_vi_done_inserting() {
  if (_rl_vi_doing_insert) {
    ...
    /* XXX - Other keys probably need to be checked. */
      ...
      rl_end_undo_group();
    while (_rl_undo_group_level > 0) rl_end_undo_group();
    vi_continued_command = 0;
  }
}

int rl_vi_movement_mode(count, key) int count, key;
{
  if (rl_point > 0) rl_backward_char(1, key);
  ...
  if (RL_ISSTATE(RL_STATE_VICMDONCE) == 0) rl_free_undo_list();
...
int rl_vi_arg_digit(count, c) int count, c;
{
...
#if defined(HANDLE_MULTIBYTE)
static int _rl_vi_change_mbchar_case(count) int count;
{
  wchar_t wc;
  ...
  if (_rl_adjust_point(rl_line_buffer, rl_point, &ps) > 0) count--;
...
int rl_vi_change_case(count, ignore) int count, ignore;
{
  int c, p;
...
int rl_vi_put(count, key) int count, key;
{
  if (!_rl_uppercase_p(key) && (rl_point + 1 <= rl_end))
    rl_point =
        _rl_find_next_mbchar(rl_line_buffer, rl_point, 1, MB_FIND_NONZERO);

  while (count--) rl_yank(1, key);

  rl_backward_char(1, key);
  return (0);
}

static void _rl_vi_backup() {
  if (MB_CUR_MAX > 1 && rl_byte_oriented == 0)
...
int rl_vi_column(count, key) int count, key;
{
  if (count > rl_end)
    rl_end_of_line(1, key);
  else
...
static int _rl_vi_arg_dispatch(c) int c;
{
  int key;
...
static void _rl_mvcxt_init(m, op, key) _rl_vimotion_cxt *m;
int op, key;
{
  m->op = op;
  m->state = m->flags = 0;
...
static _rl_vimotion_cxt *_rl_mvcxt_alloc(op, key) int op, key;
{
  _rl_vimotion_cxt *m;
...
static void _rl_mvcxt_dispose(m) _rl_vimotion_cxt *m;
{ xfree(m); }

static int rl_domove_motion_callback(m) _rl_vimotion_cxt *m;
{
  int c;
...
int _rl_vi_domove_motion_cleanup(c, m) int c;
_rl_vimotion_cxt *m;
{
  int r;
  ...
  if (rl_mark < rl_point) SWAP(rl_point, rl_mark);
...
static int rl_domove_read_callback(m) _rl_vimotion_cxt *m;
{
  int c, save;
...
static int rl_vi_domove_getchar(m) _rl_vimotion_cxt *m;
{
  int c;
...
#if defined(READLINE_CALLBACKS)
int _rl_vi_domove_callback(m) _rl_vimotion_cxt *m;
{
  int c, r;
...
/* This code path taken when not in callback mode. */
int rl_vi_domove(x, ignore) int x, *ignore;
{
  int r;
  _rl_vimotion_cxt *m;
...
static int vi_delete_dispatch(m) _rl_vimotion_cxt *m;
{
...
int rl_vi_delete_to(count, key) int count, key;
{
  int c, r;
...
static int vi_change_dispatch(m) _rl_vimotion_cxt *m;
{
  /* These are the motion commands that do not require adjusting the
  ...
  if (_rl_vi_redoing) {
    if (vi_insert_buffer && *vi_insert_buffer) rl_begin_undo_group();
    ...
    if (_rl_uppercase_p(m->key) == 0) _rl_vi_doing_insert = 1;
...
int rl_vi_change_to(count, key) int count, key;
{
  int c, r;
...
static int vi_yank_dispatch(m) _rl_vimotion_cxt *m;
{
  /* These are the motion commands that do not require adjusting the
     mark. */
...
int rl_vi_yank_to(count, key) int count, key;
{
  int c, r;
...
static int vidomove_dispatch(m) _rl_vimotion_cxt *m;
{
  int r;
...
int rl_vi_rubout(count, key) int count, key;
{
  int opoint;

  if (count < 0) return (rl_vi_delete(-count, key));
...
int rl_vi_delete(count, key) int count, key;
{
  int end;

  if (count < 0) return (rl_vi_rubout(-count, key));
  ...
  if (MB_CUR_MAX > 1 && rl_byte_oriented == 0)
        ...
        _rl_find_next_mbchar(rl_line_buffer, rl_point, count, MB_FIND_NONZERO);
  else
    end = rl_point + count;

  if (end >= rl_end) end = rl_end;

  rl_kill_text(rl_point, end);

  if (rl_point > 0 && rl_point == rl_end) rl_backward_char(1, key);
...
int rl_vi_unix_word_rubout(count, key) int count, key;
{
  int orig_point;
...
int rl_vi_back_to_indent(count, key) int count, key;
{
  rl_beg_of_line(1, key);
  while (rl_point < rl_end && whitespace(rl_line_buffer[rl_point])) rl_point++;
  return (0);
}

int rl_vi_first_print(count, key) int count, key;
{ return (rl_vi_back_to_indent(1, key)); }

static int _rl_cs_dir, _rl_cs_orig_dir;

#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_char_search(data) _rl_callback_generic_arg *data;
{
  int c;
#if defined(HANDLE_MULTIBYTE)
...
int rl_vi_char_search(count, key) int count, key;
{
  int c;
#if defined(HANDLE_MULTIBYTE)
  static char *target;
  ...
  if (key == ';' || key == ',') {
    if (_rl_cs_orig_dir == 0) return 1;
#if defined(HANDLE_MULTIBYTE)
    if (_rl_vi_last_search_mblen == 0) return 1;
#else
    if (_rl_vi_last_search_char == 0) return 1;
#endif
...
/* Match brackets */
int rl_vi_match(ignore, key) int ignore, key;
{
...
static int _rl_vi_change_char(count, c, mb) int count, c;
char *mb;
{
  int p;

  if (c == '\033' || c == CTRL('C')) return -1;
...
static int _rl_vi_callback_getchar(mb, mlen) char *mb;
int mlen;
{
  int c;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_change_char(data) _rl_callback_generic_arg *data;
{
  int c;
  char mb[MB_LEN_MAX];
...
int rl_vi_change_char(count, key) int count, key;
{
  int c;
  char mb[MB_LEN_MAX];
...
int rl_vi_subst(count, key) int count, key;
{
...
int rl_vi_overstrike(count, key) int count, key;
{
...
int rl_vi_overstrike_delete(count, key) int count, key;
{
  int i, s;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (rl_do_undo()) vi_replace_count--;

    if (rl_point == s) rl_backward_char(1, key);
  }
...
int rl_vi_replace(count, key) int count, key;
{
  int i;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_set_mark(data) _rl_callback_generic_arg *data;
{
  _rl_callback_func = 0;
  _rl_want_redisplay = 1;
...
int rl_vi_set_mark(count, key) int count, key;
{
#if defined(READLINE_CALLBACKS)
  if (RL_ISSTATE(RL_STATE_CALLBACK)) {
    _rl_callback_data = 0;
...
#if defined(READLINE_CALLBACKS)
static int _rl_vi_callback_goto_mark(data) _rl_callback_generic_arg *data;
{
  _rl_callback_func = 0;
  _rl_want_redisplay = 1;
...
int rl_vi_goto_mark(count, key) int count, key;
{
#if defined(READLINE_CALLBACKS)
  if (RL_ISSTATE(RL_STATE_CALLBACK)) {
    _rl_callback_data = 0;
