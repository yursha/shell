...
static void _rl_callback_newline() {
  rl_initialize();
  ...
  if (in_handler == 0) {
    ...
    if (rl_persistent_signal_handlers) rl_set_signals();
#endif
  }
...
void rl_callback_handler_install(prompt, linefunc) const char *prompt;
...
#define CALLBACK_READ_RETURN()                                  \
  do {                                                          \
    if (rl_persistent_signal_handlers == 0) rl_clear_signals(); \
    return;                                                     \
  } while (0)
#else
...
void rl_callback_read_char() {
  ...
  if (rl_persistent_signal_handlers == 0) rl_set_signals();
#endif
  ...
  do {
    ...
    /* States that can occur while in state VIMOTION have to be checked
      ...
      if (RL_ISSTATE(RL_STATE_NUMERICARG) == 0) _rl_internal_char_cleanup();
    ...
    if (rl_done) {
      ...
      if (rl_line_buffer[0]) _rl_init_line_state();

      /* Redisplay the prompt if readline_handler_{install,remove}
         not called. */
      if (in_handler == 0 && rl_linefunc) _rl_callback_newline();
    }
...
_rl_callback_generic_arg *_rl_callback_data_alloc(count) int count;
{
...
void _rl_callback_data_dispose(arg) _rl_callback_generic_arg *arg;
{ xfree(arg); }

/* Make sure that this agrees with cases in rl_callback_read_char */
void rl_callback_sigcleanup() {
  if (RL_ISSTATE(RL_STATE_CALLBACK) == 0) return;
  ...
  if (RL_ISSTATE(RL_STATE_CHARSEARCH)) RL_UNSETSTATE(RL_STATE_CHARSEARCH);
