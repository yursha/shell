...
static char *prompt_modestr(lenp) int *lenp;
{
  if (rl_editing_mode == emacs_mode) {
    if (lenp)
      *lenp =
          _rl_emacs_mode_str ? _rl_emacs_modestr_len : RL_EMACS_MODESTR_DEFLEN;
...
static char *expand_prompt(pmt, flags, lp, lip, niflp, vlp) char *pmt;
int flags;
int *lp, *lip, *niflp, *vlp;
{
  ...
  if (rl < _rl_screenwidth) invfl = ninvis;

  *r = '\0';
  if (lp) *lp = rl;
  if (lip) *lip = last;
  if (niflp) *niflp = invfl;
  if (vlp) *vlp = physchars;

  if (nprompt != pmt) free(nprompt);
...
char *_rl_strip_prompt(pmt) char *pmt;
{
  char *ret;

  ret =
      expand_prompt(pmt, 0, (int *)NULL, (int *)NULL, (int *)NULL, (int *)NULL);
  return ret;
}
 ...
 */
int rl_expand_prompt(prompt) char *prompt;
{
  char *p, *t;
  int c;
  ...
  if (p == 0) {
    ...
    local_prompt =
        expand_prompt(prompt, 0, &prompt_visible_length, &prompt_last_invisible,
    ...
    local_prompt = expand_prompt(
        p, PMT_MULTILINE, &prompt_visible_length, &prompt_last_invisible,
        &prompt_invis_chars_first_line, &prompt_physical_chars);
    c = *t;
    *t = '\0';
    ...
    local_prompt_prefix =
        expand_prompt(prompt, PMT_MULTILINE, &prompt_prefix_length, (int *)NULL,
                      (int *)NULL, (int *)NULL);
    *t = c;
    local_prompt_len = local_prompt ? strlen(local_prompt) : 0;
...
static void init_line_structures(minsize) int minsize;
{
  register int n;

  if (invisible_line == 0) /* initialize it */
      {
    if (line_size < minsize) line_size = minsize;
...
void rl_redisplay() {
  ...
  if (!rl_display_prompt) rl_display_prompt = "";
  ...
  if (visible_line[0] != invisible_line[0]) rl_display_fixed = 0;
  ...
  if (rl_display_prompt == rl_prompt || local_prompt) {
    ...
    if (!prompt_this_line)
      ...
      if (forced_display) {
        ...
        if (pmtlen < 2 || prompt_this_line[-2] != '\r') cr();
      }
    }
...
#define CHECK_LPOS()                                                          \
  ...
  while (lpos >= _rl_screenwidth) {
    ...
    wadjust = (newlines == 0) ? prompt_invis_chars_first_line
...
/* fix from Darin Johnson <darin@acuson.com> for prompt string with
  ...
  /* Draw the rest of the line (after the prompt) into invisible_line, keeping
     track of where the cursor is (cpos_buffer_position), the number of the line
     containing
...
#endif
  {
...
#endif
    {
      ...
      if (mb_cur_max > 1 && rl_byte_oriented == 0) {
        ...
        for (i = in; i < in + wc_bytes; i++) line[out++] = rl_line_buffer[i];
        for (i = 0; i < wc_width; i++) CHECK_LPOS();
      } else {
        line[out++] = c;
        CHECK_LPOS();
      }
...
#define INVIS_FIRST()                                                      \
  (prompt_physical_chars > _rl_screenwidth ? prompt_invis_chars_first_line \
                                           : wrap_offset)
   ...
   _rl_last_c_pos > wrap_offset && o_cpos < prompt_last_invisible)
      ...
      for (linenum = 0; linenum <= inv_botlin; linenum++) {
        ...
        if (linenum == 0 && inv_botlin == 0 && _rl_last_c_pos == out &&
          ...
          if (nleft) _rl_clear_to_eol(nleft);
        }
#if 0
      ...
      nleft = prompt_visible_length + wrap_offset;
        ...
        putc('\r', rl_outstream);
#else
        if (_rl_term_cr) tputs(_rl_term_cr, 1, _rl_output_character_function);
#endif
        if (modmark) _rl_output_some_chars("*", 1);
...
#define M_OFFSET(margin, offset) ((margin) == 0 ? offset : 0)
    ...
    if (phys_c_pos > _rl_screenwidth - 2) {
      ...
      if (wrap_offset && lmargin > 0 && lmargin < nleft) lmargin = nleft;
    ...
    if (rl_display_fixed == 0 || forced_display || lmargin != last_lmargin) {
      ...
      o_cpos = _rl_last_c_pos;
      cpos_adjusted = 0;
      update_line(&visible_line[last_lmargin], &invisible_line[lmargin], 0,
...
static void update_line(old, new, current_line, omax, nmax,
                        inv_botlin) register char *old,
    *new;
  ...
  if (temp == _rl_screenwidth && _rl_term_autowrap &&
    ...
    if (mb_cur_max > 1 && rl_byte_oriented == 0) {
      ...
      if (tempwidth > 0) {
        int count, i;
        bytes = ret;
        for (count = 0; count < bytes; count++) putc(new[count], rl_outstream);
        _rl_last_c_pos = tempwidth;
        _rl_last_v_pos++;
        memset(&ps, 0, sizeof(mbstate_t));
        ret = mbrtowc(&wc, old, mb_cur_max, &ps);
        if (ret != 0 && bytes != 0) {
          if (MB_INVALIDCH(ret)) ret = 1;
...
/* Find first difference. */
  ...
  if (mb_cur_max > 1 && rl_byte_oriented == 0) {
    ...
    if (memcmp(old, new, temp) == 0) /* adding at the end */
      ...
      if (omax == nmax && STREQN(new, old, omax)) {
        ...
        for (ofd = old, nfd = new;
             (ofd - old < omax) &&
             *ofd &&_rl_compare_chars(old, old_offset, &ps_old, new, new_offset,
                                      &ps_new);) {
  ...
  if (mb_cur_max > 1 && rl_byte_oriented == 0) {
    ...
    while ((ols > ofd) && (nls > nfd)) {
      ...
      if (_rl_compare_chars(old, ols - old, &ps_old, new, nls - new, &ps_new) ==
          0)
        break;
  ...
  if (current_line == 0 && !_rl_horizontal_scroll_mode && _rl_term_cr &&
    ...
    if (mb_cur_max > 1 && rl_byte_oriented == 0) {
      ...
      _rl_last_c_pos =
          _rl_col_width(local_prompt, 0, lendiff, 1) - wrap_offset + modmark;
      cpos_adjusted = 1;
    } else
  ...
  if (mb_cur_max > 1 && rl_byte_oriented == 0)
    col_lendiff = _rl_col_width(new, nfd - new, nls - new, 1) -
  ...
  if (col_lendiff > 0) /* XXX - was lendiff */
    ...
    /* Sometimes it is cheaper to print the characters rather than
      ...
      if (*ols &&
          ...
          (((mb_cur_max > 1 && rl_byte_oriented == 0) && current_line == 0 &&
            wrap_offset && ((nfd - new) <= prompt_last_invisible) &&
      ...
      if (bytes_to_insert > 0) {
        ...
        if (mb_cur_max > 1 && rl_byte_oriented == 0) {
          ...
          if (current_line == 0 && wrap_offset &&
              displaying_prompt_first_line && _rl_last_c_pos > wrap_offset &&
              ((nfd - new) <= prompt_last_invisible)) {
            _rl_last_c_pos -= wrap_offset;
            cpos_adjusted = 1;
          }
...
int rl_on_new_line() {
  if (visible_line) visible_line[0] = '\0';

  _rl_last_c_pos = _rl_last_v_pos = 0;
  _rl_vis_botlin = last_lmargin = 0;
  if (vis_lbreaks) vis_lbreaks[0] = vis_lbreaks[1] = 0;
  visible_wrap_offset = 0;
  return 0;
}
...
int rl_on_new_line_with_prompt() {
  ...
  if (!prompt_last_line) prompt_last_line = rl_prompt;
  ...
  if (l > 0 && (l % real_screenwidth) == 0) _rl_output_some_chars("\n", 1);
  last_lmargin = 0;
...
void _rl_move_cursor_relative(new, data) int new;
const char *data;
{
  register int i;
  ...
  if (mb_cur_max > 1 && rl_byte_oriented == 0) {
    ...
    if (displaying_prompt_first_line == 0) adjust = 0;
    ...
    if (adjust &&
        ((new > prompt_last_invisible) || /* XXX - don't use woff here */
         ...
         (prompt_physical_chars >= _rl_screenwidth &&    /* visible line */
          ...
          dpos >= woff &&
          new > (prompt_last_invisible - (vis_lbreaks[_rl_last_v_pos]) -
                 wrap_offset))))
  ...
  if (cpos < dpos) {
    ...
    if (mb_cur_max > 1 && rl_byte_oriented == 0) {
      if (_rl_term_forward_char) {
        ...
        tputs(_rl_term_cr, 1, _rl_output_character_function);
        for (i = 0; i < new; i++) putc(data[i], rl_outstream);
      }
    } else
      for (i = cpos; i < new; i++) putc(data[i], rl_outstream);
  }
...
void _rl_move_vert(to) int to;
{
  register int delta, i;

  if (_rl_last_v_pos == to || to > _rl_screenheight) return;
...
int rl_character_len(c, pos) register int c, pos;
{
  unsigned char uc;

  uc = (unsigned char)c;

  if (META_CHAR(uc)) return ((_rl_output_meta_chars == 0) ? 4 : 1);
  ...
  if (CTRL_CHAR(c) || c == RUBOUT) return (2);
...
#endif
{
  ...
  if (msg_buf == 0) msg_buf = xmalloc(msg_bufsiz = 128);
  ...
  local_prompt =
      expand_prompt(msg_buf, 0, &prompt_visible_length, &prompt_last_invisible,
                    &prompt_invis_chars_first_line, &prompt_physical_chars);
...
char *_rl_make_prompt_for_search(pchar) int pchar;
{
  int len;
  char *pmt, *p;
...
void _rl_erase_at_end_of_line(l) int l;
{
  register int i;

  _rl_backspace(l);
  for (i = 0; i < l; i++) putc(' ', rl_outstream);
  _rl_backspace(l);
  for (i = 0; i < l; i++) visible_line[--_rl_last_c_pos] = '\0';
  rl_display_fixed++;
}

/* Clear to the end of the line.  COUNT is the minimum
   number of character spaces to clear, */
void _rl_clear_to_eol(count) int count;
{
#ifndef __MSDOS__
  if (_rl_term_clreol)
...
/* Clear to the end of the line using spaces.  COUNT is the minimum
   number of character spaces to clear, */
static void space_to_eol(count) int count;
{
  register int i;

  for (i = 0; i < count; i++) putc(' ', rl_outstream);
...
static void insert_some_chars(string, count, col) char *string;
int count, col;
{
...
static void open_some_spaces(col) int col;
{
  ...
  if (_rl_term_IC) {
    ...
    for (i = col; i--;) _rl_output_character_function(' ');
    ...
    for (i = col; i--;) tputs(_rl_term_ic, 1, _rl_output_character_function);
  }
#endif /* !__MSDOS__ && (!__MINGW32__ || NCURSES_VERSION)*/
}

/* Delete COUNT characters from the display line. */
static void delete_chars(count) int count;
{
  ...
  if (_rl_term_DC && *_rl_term_DC) {
    ...
    if (_rl_term_dc && *_rl_term_dc)
      while (count--) tputs(_rl_term_dc, 1, _rl_output_character_function);
  }
#endif /* !__MSDOS__ && (!__MINGW32__ || NCURSES_VERSION)*/
}
...
static void redraw_prompt(t) char *t;
{
  char *oldp;
  ...
  rl_display_prompt = t;
  local_prompt = expand_prompt(
      t, PMT_MULTILINE, &prompt_visible_length, &prompt_last_invisible,
      &prompt_invis_chars_first_line, &prompt_physical_chars);
  local_prompt_prefix = (char *)NULL;
  local_prompt_len = local_prompt ? strlen(local_prompt) : 0;
...
#endif
    if (_rl_last_v_pos > 0) _rl_move_vert(0);
  } else
    rl_crlf();
...
void _rl_ttyflush() { fflush(rl_outstream); }

/* return the `current display line' of the cursor -- the number of lines to
   move up to get to the first screen line of the current readline line. */
int _rl_current_display_line() {
  int ret, nleft;
...
static int _rl_col_width(str, start, end, flags) const char *str;
int start, end, flags;
{
  wchar_t wc;
  mbstate_t ps;
