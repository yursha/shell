...
int rl_set_prompt(prompt) const char *prompt;
{
  FREE(rl_prompt);
...
char *readline(prompt) const char *prompt;
{
  char *value;
#if 0
  int in_callback;
...
STATIC_CALLBACK void readline_internal_setup() {
  char *nprompt;
  ...
  if (_rl_enable_meta & RL_ISSTATE(RL_STATE_TERMPREPPED)) _rl_enable_meta_key();
...
STATIC_CALLBACK char *readline_internal_teardown(eof) int eof;
{
  char *temp;
  HIST_ENTRY *entry;
  ...
  if (_rl_revert_all_at_newline) _rl_revert_all_lines();

  /* At any rate, it is highly likely that this line has an undo list.  Get
     rid of it now. */
  if (rl_undo_list) rl_free_undo_list();
...
return (eof_found);
...
#if defined(READLINE_CALLBACKS)
  static int readline_internal_charloop() {
    int eof = 1;
  ...
  void _rl_set_the_line() { the_line = rl_line_buffer; }

#if defined(READLINE_CALLBACKS)
  _rl_keyseq_cxt *_rl_keyseq_cxt_alloc() {
    _rl_keyseq_cxt *cxt;
  ...
  void _rl_keyseq_cxt_dispose(cxt) _rl_keyseq_cxt * cxt;
  { xfree(cxt); }
  ...
  static int _rl_subseq_getchar(key) int key;
  {
    int k;

    if (key == ESC) RL_SETSTATE(RL_STATE_METANEXT);
    RL_SETSTATE(RL_STATE_MOREINPUT);
    k = rl_read_key();
    RL_UNSETSTATE(RL_STATE_MOREINPUT);
    if (key == ESC) RL_UNSETSTATE(RL_STATE_METANEXT);

    return k;
  }

#if defined(READLINE_CALLBACKS)
  int _rl_dispatch_callback(cxt) _rl_keyseq_cxt * cxt;
  {
    int nkey, r;
    ...
    if (r >= 0 ||
        (r == -1 && (cxt->flags & KSEQ_SUBSEQ) == 0)) /* success! or failure! */
        {
    ...
    if (r != -3) /* magic value that says we added to the chain */
      _rl_kscxt = cxt->ocxt;
    if (_rl_kscxt) _rl_kscxt->childval = r;
    if (r != -3) _rl_keyseq_cxt_dispose(cxt);
  ...
  int _rl_dispatch(key, map) register int key;
  Keymap map;
  {
  ...
  int _rl_dispatch_subseq(key, map, got_subseq) register int key;
  Keymap map;
  int got_subseq;
  {
    int r, newkey;
    ...
    if (META_CHAR(key) && _rl_convert_meta_chars_to_ascii) {
      if (map[ESC].type == ISKMAP) {
        if (RL_ISSTATE(RL_STATE_MACRODEF)) _rl_add_macro_char(ESC);
    ...
    if (RL_ISSTATE(RL_STATE_MACRODEF)) _rl_add_macro_char(key);
...
/* Allocate new context here.  Use linked contexts (linked through
   cxt->ocxt) to simulate recursion */
          ...
          /* If we're redoing a vi mode command and we know there is a shadowed
             function corresponding to this key, just call it -- all the
             redoable
             vi mode commands already have all the input they need, and
             rl_vi_redo
          ...
          if (RL_ISSTATE(RL_STATE_CALLBACK)) {
            ...
            if (got_subseq) cxt->flags |= KSEQ_SUBSEQ;
            cxt->okey = key;
            cxt->oldmap = map;
...
#if defined(VI_MODE)
    if (rl_editing_mode == vi_mode && _rl_keymap == vi_movement_keymap &&
        key != ANYOTHERKEY && _rl_dispatching_keymap == vi_movement_keymap &&
  ...
  static int _rl_subseq_result(r, map, key, got_subseq) int r;
  Keymap map;
  int key, got_subseq;
  {
    Keymap m;
...
#if defined(VI_MODE)
    if (rl_editing_mode == vi_mode) _rl_vi_initialize_line();
#endif
...
_emx_build_environ ()
{
  ...
  *tp = 0;
  ...
  running_in_emacs = sh_get_env_value ("EMACS") != (char *)0;
    ...
    if (rl_terminal_name == 0) rl_terminal_name = sh_get_env_value("TERM");
...
#if defined(COLOR_SUPPORT)
    if (_rl_colored_stats || _rl_colored_completion_prefix) _rl_parse_colors();
#endif

    rl_executing_keyseq = malloc(_rl_executing_keyseq_size = 16);
    if (rl_executing_keyseq) rl_executing_keyseq[0] = '\0';
  }
  ...
  static void readline_default_bindings() {
    if (_rl_bind_stty_chars) rl_tty_set_default_bindings(_rl_keymap);
  }
  ...
  static void bind_arrow_keys_internal(map) Keymap map;
  {
    Keymap xkeymap;
  ...
  static void bind_arrow_keys() {
    ...
    if (vi_movement_keymap[ESC].type == ISKMAP)
      rl_bind_keyseq_in_map("\033", (rl_command_func_t *)NULL,
                            vi_movement_keymap);
  ...
  int rl_save_state(sp) struct readline_state * sp;
  {
    if (sp == 0) return -1;
  ...
  int rl_restore_state(sp) struct readline_state * sp;
  {
    if (sp == 0) return -1;
