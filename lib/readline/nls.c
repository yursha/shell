...
static char *legal_lang_values[] = {
    ...
    "iso88597", "iso88598", "iso88599", "iso885910", "koi8r",    0};
...
static char *_rl_get_locale_var(v) const char *v;
{
  char *lspec;

  lspec = sh_get_env_value("LC_ALL");
  if (lspec == 0 || *lspec == 0) lspec = sh_get_env_value(v);
  if (lspec == 0 || *lspec == 0) lspec = sh_get_env_value("LANG");

  return lspec;
}

static int utf8locale(lspec) char *lspec;
{
  char *cp;
  size_t len;
...
/* If we have setlocale(3), just check the current LC_CTYPE category
  ...
  /* Since _rl_get_locale_var queries the right environment variables,
     ...
     environment. */
  if (lspec == 0 || *lspec == 0) lspec = setlocale(LC_CTYPE, (char *)NULL);
  if (lspec == 0) lspec = "";
  t = setlocale(LC_CTYPE, lspec);

  if (t && *t) _rl_utf8locale = utf8locale(t);
  ...
  if (lspec == 0 || (t = normalize_codeset(lspec)) == 0) return (0);
...
#if !defined(HAVE_SETLOCALE)
static char *normalize_codeset(codeset) char *codeset;
{
  size_t namelen, i;
  int len, all_digits;
  char *wp, *retval;
...
static char *find_codeset(name, lenp) char *name;
size_t *lenp;
{
