...
static UNDO_LIST *alloc_undo_entry(what, start, end, text) enum undo_code what;
int start, end;
char *text;
{
  UNDO_LIST *temp;
...
/* Remember how to undo something.  Concatenate some undos if that
   seems right. */
void rl_add_undo(what, start, end, text) enum undo_code what;
int start, end;
char *text;
{
  UNDO_LIST *temp;
...
/* Free an UNDO_LIST */
void _rl_free_undo_list(ul) UNDO_LIST *ul;
{
  UNDO_LIST *release;
  ...
  while (ul) {
    ...
    if (release->what == UNDO_DELETE) xfree(release->text);
...
UNDO_LIST *_rl_copy_undo_entry(entry) UNDO_LIST *entry;
{
  UNDO_LIST *new;
...
UNDO_LIST *_rl_copy_undo_list(head) UNDO_LIST *head;
{
...
#define TRANS(i) ((i) == -1 ? rl_point : ((i) == -2 ? rl_end : (i)))
  ...
  do {
    ...
    _hs_replace_history_data(-1, (histdata_t *)release,
                             (histdata_t *)rl_undo_list);
...
int _rl_fix_last_undo_of_type(type, start, end) int type, start, end;
{
  UNDO_LIST *rl;
...
int rl_modifying(start, end) int start, end;
{
  if (start > end) {
    SWAP(start, end);
  }
...
int rl_revert_line(count, key) int count, key;
{
  if (rl_undo_list == 0)
    rl_ding();
  else {
...
int rl_undo_command(count, key) int count, key;
{
