...
#if defined(_WIN32)
  if (home == 0) home = sh_get_env_value("APPDATA");
#endif
...
int from, to;
{
  ...
  if ((file < 0) || (fstat(file, &finfo) == -1)) goto error_and_exit;
  ...
  for (line_start = line_end = buffer; line_end < bufend && current_line < from;
       line_end++)
    if (*line_end == '\n') {
      p = line_end + 1;
      /* If we see something we think is a timestamp, continue with this
         line.  We should check more extensively here... */
      if (HIST_TIMESTAMP_START(p) == 0) current_line++;
      line_start = p;
    }
  ...
  if (reset_comment_char) history_comment_char = '\0';
...
int lines;
{
  ...
  for (bp1 = bp = buffer + chars_read - 1; lines && bp > buffer; bp--) {
    if (*bp == '\n' && HIST_TIMESTAMP_START(bp1) == 0) lines--;
    bp1 = bp;
  }
  ...
  if ((file = open(tempname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600)) !=
      -1) {
    if (write(file, bp, chars_read - (bp - buffer)) < 0) rv = errno;

    if (close(file) < 0 && rv == 0) rv = errno;
  } else
    rv = errno;
...
truncate_exit:
  ...
  /* Make sure the new filename is owned by the same user as the old.  If one
     ...
     owned by root. */
  if (rv == 0 && exists) r = chown(filename, finfo.st_uid, finfo.st_gid);
...
int nelements, overwrite;
{
  ...
  if (nelements > history_length) nelements = history_length;
...
/* Allocate the buffer, and fill it. */
#ifdef HISTORY_USE_MMAP
      if (ftruncate(file, buffer_size + cursize) == -1) goto mmap_error;
    ...
    if (msync(buffer, buffer_size, MS_ASYNC) != 0 ||
      ...
      rv = errno;
#else
    if (write(file, buffer, buffer_size) < 0) rv = errno;
    xfree(buffer);
#endif
  }
  ...
  /* Make sure the new filename is owned by the same user as the old.  If one
     ...
     with a shared history file, we don't want to leave the history file
     owned by root. */
  if (rv == 0 && exists) mode = chown(histname, finfo.st_uid, finfo.st_gid);
