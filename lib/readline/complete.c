...
int rl_complete(ignore, invoking_key) int ignore, invoking_key;
{
...
int rl_possible_completions(ignore, invoking_key) int ignore, invoking_key;
{
  rl_completion_invoking_key = invoking_key;
  return (rl_complete_internal('?'));
}

int rl_insert_completions(ignore, invoking_key) int ignore, invoking_key;
{
  rl_completion_invoking_key = invoking_key;
  return (rl_complete_internal('*'));
}
...
int rl_completion_mode(cfunc) rl_command_func_t *cfunc;
{
...
static void _rl_complete_sigcleanup(sig, ptr) int sig;
void *ptr;
{
...
static void set_completion_defaults(what_to_do) int what_to_do;
{
...
static int get_y_or_n(for_pager) int for_pager;
{
  int c;
...
/* For now, disable pager in callback mode, until we later convert to state
  ...
  if (RL_ISSTATE(RL_STATE_CALLBACK)) return 1;
#endif
  ...
  for (;;) {
    ...
    if (c == 'y' || c == 'Y' || c == ' ') return (1);
    if (c == 'n' || c == 'N' || c == RUBOUT) return (0);
    if (c == ABORT_CHAR || c < 0) _rl_abort_internal();
    if (for_pager && (c == NEWLINE || c == RETURN)) return (2);
    if (for_pager && (c == 'q' || c == 'Q')) return (0);
    rl_ding();
  }
}

static int _rl_internal_pager(lines) int lines;
{
  int i;
...
static int path_isdir(filename) const char *filename;
{
  struct stat finfo;
...
static int stat_char(filename) char *filename;
{
  struct stat finfo;
  int character, r;
  char *f;
...
/* Short-circuit a //server on cygwin, since that will always behave as
  ...
  character = 0;
  if (S_ISDIR(finfo.st_mode)) character = '/';
#if defined(S_ISCHR)
    ...
    if (ext &&
      ...
      character = '*';
#else
    if (access(filename, X_OK) == 0) character = '*';
#endif
  }
...
#if defined(COLOR_SUPPORT)
static int colored_stat_start(filename) const char *filename;
{
  _rl_set_normal_color();
  return (_rl_print_color_indicator(filename));
}

static void colored_stat_end() {
  _rl_prep_non_filename_text();
  _rl_put_indicator(&_rl_color_indicator[C_CLR_TO_EOL]);
}

static int colored_prefix_start() {
  _rl_set_normal_color();
  return (_rl_print_prefix_color());
}

static void colored_prefix_end() { colored_stat_end(); /* for now */ }
#endif
...
static char *printable_part(pathname) char *pathname;
{
  char *temp, *x;
...
static int fnwidth(string) const char *string;
{
  int width, pos;
...
static int fnprint(to_print, prefix_bytes, real_pathname) const char *to_print;
int prefix_bytes;
const char *real_pathname;
{
  ...
  if (prefix_bytes && _rl_completion_prefix_display_length > 0) {
    ...
    for (w = 0; w < ELLIPSIS_LEN; w++) putc(ellipsis, rl_outstream);
  ...
  if (_rl_colored_stats) colored_stat_end();
#endif
...
static int print_filename(to_print, full_pathname, prefix_bytes) char *to_print,
    *full_pathname;
int prefix_bytes;
{
  ...
  if (rl_filename_completion_desired && (
    ...
    if (to_print != full_pathname) {
      ...
      if (full_pathname == 0 || *full_pathname == 0)
        ...
        dn = full_pathname;
      s = tilde_expand(dn);
      if (rl_directory_completion_hook) (*rl_directory_completion_hook)(&s);
      ...
      if (rl_visible_stats)
        ...
        if (path_isdir(new_full_pathname)) extension_char = '/';
      }
...
/* Move colored-stats code inside fnprint() */
#if defined(COLOR_SUPPORT)
      if (_rl_colored_stats) printed_len = fnprint(to_print, prefix_bytes, s);
#endif
    }
...
static char *rl_quote_filename(s, rtype, qcp) char *s;
int rtype;
char *qcp;
{
  char *r;
  ...
  *r = *rl_completer_quote_characters;
  strcpy(r + 1, s);
  if (qcp) *qcp = *rl_completer_quote_characters;
  return r;
}
...
char _rl_find_completion_word(fp, dp) int *fp, *dp;
{
  ...
  if (brkchars == 0) brkchars = rl_completer_word_break_characters;
  ...
  if (rl_point == end && quote_char == '\0') {
    ...
    while (rl_point = MB_PREVCHAR(rl_line_buffer, rl_point, MB_FIND_ANY)) {
      ...
      if (strchr(brkchars, scan) == 0) continue;
  ...
  if (fp) *fp = found_quote;
  if (dp) *dp = delimiter;

  return (quote_char);
}

static char **gen_completion_matches(text, start, end, our_func, found_quote,
                                     quote_char) char *text;
int start, end;
...
/* Filter out duplicates in MATCHES.  This frees up the strings in
   MATCHES. */
static char **remove_duplicate_matches(matches) char **matches;
{
  char *lowest_common;
  int i, j, newlen;
  char dead_slot;
  ...
  for (i = j = 1; matches[i]; i++) {
    if (matches[i] != (char *)&dead_slot) temp_array[j++] = matches[i];
  }
  temp_array[j] = (char *)NULL;

  if (matches[0] != (char *)&dead_slot) xfree(matches[0]);
...
/* Find the common prefix of the list of matches, and put it into
   matches[0]. */
static int compute_lcd_of_matches(match_list, matches, text) char **match_list;
int matches;
const char *text;
{
    ...
    if (_rl_completion_case_fold) {
...
#if defined(HANDLE_MULTIBYTE)
        if (MB_CUR_MAX > 1 && rl_byte_oriented == 0) {
          v1 = mbrtowc(&wc1, match_list[i] + si, strlen(match_list[i] + si),
                       &ps1);
          v2 = mbrtowc(&wc2, match_list[i + 1] + si,
      ...
      for (si = 0; (c1 = match_list[i][si]) && (c2 = match_list[i + 1][si]);
        ...
        if (MB_CUR_MAX > 1 && rl_byte_oriented == 0) {
          mbstate_t ps_back;
          ps_back = ps1;
          if (!_rl_compare_chars(match_list[i], si, &ps1, match_list[i + 1], si,
                                 &ps2))
            break;
  ...
  if (low == 0 && text && *text) {
    ...
    if (_rl_completion_case_fold) {
      ...
      if (i > matches) strncpy(match_list[0], match_list[1], low);
...
static int postprocess_matches(matchesp, matching_filenames) char ***matchesp;
...
static int complete_get_screenwidth() {
  int cols;
  char *envcols;

  cols = _rl_completion_columns;
  if (cols >= 0 && cols <= _rl_screenwidth) return cols;
  envcols = getenv("COLUMNS");
  if (envcols && *envcols) cols = atoi(envcols);
  if (cols >= 0 && cols <= _rl_screenwidth) return cols;
...
void rl_display_match_list(matches, len, max) char **matches;
int len, max;
{
  ...
  if (_rl_completion_prefix_display_length > 0) {
    ...
    if (common_length > max || sind > max) common_length = sind = 0;
  ...
  max += 2;
  limit = cols / max;
  if (limit != 1 && (limit * max == cols)) limit--;

  /* If cols == 0, limit will end up -1 */
  if (cols < _rl_screenwidth && limit < 0) limit = 1;
        ...
        if (l > len || matches[l] == 0)
          ...
          if (j + 1 < limit) {
            if (max <= printed_len)
              ...
              for (k = 0; k < max - printed_len; k++) putc(' ', rl_outstream);
          }
        }
        l += count;
      }
...
static void display_matches(matches) char **matches;
{
  int len, max, i;
  char *temp;
...
static char *make_quoted_replacement(match, mtype, qc) char *match;
int mtype;
char *qc; /* Pointer to quoting character, if any */
{
  ...
  if (should_quote)
    should_quote =
        should_quote &&
        (!qc || !*qc || (rl_completer_quote_characters &&
...
static void insert_match(match, start, mtype, qc) char *match;
int start, mtype;
char *qc;
{
  char *replacement, *r;
  ...
  if (replacement) {
    ...
    if (_rl_skip_completed_text) {
      ...
      if (start <= end || *r) _rl_replace_text(r, start, end);
      rl_point = start + strlen(r);
    } else
      _rl_replace_text(replacement, start, end);
    if (replacement != match) xfree(replacement);
  }
}
...
static int append_to_match(text, delimiter, quote_char,
                           nontrivial_match) char *text;
int delimiter, quote_char, nontrivial_match;
{
  ...
  if (rl_filename_completion_desired) {
    ...
    /* Don't add anything if the filename is a symlink and resolves to a
      ...
      if (rl_point == rl_end && temp_string_index) rl_insert_text(temp_string);
    }
    xfree(filename);
  } else {
    if (rl_point == rl_end && temp_string_index) rl_insert_text(temp_string);
...
static void insert_all_matches(matches, point, qc) char **matches;
int point;
char *qc;
{
  int i;
  ...
  /* remove any opening quote character; make_quoted_replacement will add
     it back. */
  if (qc && *qc && point && rl_line_buffer[point - 1] == *qc) point--;
  ...
  if (matches[1]) {
    for (i = 1; matches[i]; i++) {
      ...
      rl_insert_text(rp);
      rl_insert_text(" ");
      if (rp != matches[i]) xfree(rp);
    }
  } else {
    rp = make_quoted_replacement(matches[0], SINGLE_MATCH, qc);
    rl_insert_text(rp);
    rl_insert_text(" ");
    if (rp != matches[0]) xfree(rp);
  }
  rl_end_undo_group();
}

void _rl_free_match_list(matches) char **matches;
{
  register int i;

  if (matches == 0) return;

  for (i = 0; matches[i]; i++) xfree(matches[i]);
  xfree(matches);
}
...
int rl_complete_internal(what_to_do) int what_to_do;
{
  char **matches;
  rl_compentry_func_t *our_func;
  ...
  if (what_to_do == '!' || what_to_do == '@') tlen = strlen(text);
  xfree(text);
    ...
    case '@':
      ...
      if (matches[1]) {
        if (what_to_do == '!') {
          ...
          if (nontrivial_lcd == 0) display_matches(matches);
          break;
 ...
 */
char **rl_completion_matches(text, entry_function) const char *text;
  ...
  while (string = (*entry_function)(text, matches)) {
    if (RL_SIG_RECEIVED()) {
      ...
      if (entry_function == rl_filename_completion_function) {
        for (i = 1; match_list[i]; i++) xfree(match_list[i]);
      }
      xfree(match_list);
      match_list = 0;
      match_list_size = 0;
    ...
    if (match_list == 0) return (match_list);
...
/* A completion function for usernames.
   ...
   character (usually `~').  */
char *rl_username_completion_function(text, state) const char *text;
int state;
{
  ...
  while (entry = getpwent()) {
    ...
    if (namelen == 0 || (STREQN(username, entry->pw_name, namelen))) break;
  }
#endif
  ...
  if (entry == 0) {
    ...
    *value = *text;
    ...
    if (first_char == '~') rl_filename_completion_desired = 1;
...
static int complete_fncmp(convfn, convlen, filename, filename_len) const
    char *convfn;
int convlen;
const char *filename;
int filename_len;
{
  ...
  if (filename_len == 0) return 1;
  if (convlen < filename_len) return 0;
...
char *rl_filename_completion_function(text, state) const char *text;
int state;
{
  ...
  if (state == 0) {
    ...
    if (*filename && rl_completion_found_quote &&
        rl_filename_dequoting_function) {
      /* delete single and double quotes */
      temp = (*rl_filename_dequoting_function)(filename,
  ...
  while (directory && (entry = readdir(directory))) {
    ...
    if (filename_len == 0) {
      if (_rl_match_hidden_files == 0 && HIDDEN_FILE(convfn)) continue;

      if (convfn[0] != '.' || (convfn[1] && (convfn[1] != '.' || convfn[2])))
        break;
    } else {
      if (complete_fncmp(convfn, convlen, filename, filename_len)) break;
    }
  }
  ...
  if (entry == 0) {
    ...
    if (dirname && (dirname[0] != '.' || dirname[1])) {
      if (rl_complete_with_tilde_expansion && *users_dirname == '~') {
        ...
        if (users_dirname[dirlen - 1] != '/') temp[dirlen++] = '/';
      }
...
int rl_old_menu_complete(count, invoking_key) int count, invoking_key;
{
  rl_compentry_func_t *our_func;
  ...
  if (rl_last_func != rl_old_menu_complete) {
    ...
    FREE(orig_text);
    if (matches) _rl_free_match_list(matches);
    ...
    matches = gen_completion_matches(orig_text, orig_start, orig_end, our_func,
    ...
    if (match_list_size > 1 && _rl_complete_show_all) display_matches(matches);
  }
  ...
  if (match_list_index < 0) {
    while (match_list_index < 0) match_list_index += match_list_size;
  } else
...
int rl_menu_complete(count, ignore) int count, ignore;
{
  rl_compentry_func_t *our_func;
  int matching_filenames, found_quote;
  ...
  if ((rl_last_func != rl_menu_complete &&
    ...
    /* matches[0] is lcd if match_list_size > 1, but the circular buffer
       code below should take care of it. */
    if (*matches[0]) {
      insert_match(matches[0], orig_start,
                   matches[1] ? MULT_MATCH : SINGLE_MATCH, &quote_char);
...
int rl_backward_menu_complete(count, key) int count, key;
{
