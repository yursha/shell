...
_rl_search_cxt *_rl_scxt_alloc(type, flags) int type, flags;
{
  _rl_search_cxt *cxt;
...
void _rl_scxt_dispose(cxt, flags) _rl_search_cxt *cxt;
int flags;
{
...
int rl_reverse_search_history(sign, key) int sign, key;
{ return (rl_search_history(-sign, key)); }

/* Search forwards through the history looking for a string which is typed
   interactively.  Start with the current line. */
int rl_forward_search_history(sign, key) int sign, key;
{ return (rl_search_history(sign, key)); }
...
static void rl_display_search(search_string, flags, where) char *search_string;
int flags, where;
{
  char *message;
...
static _rl_search_cxt *_rl_isearch_init(direction) int direction;
{
  _rl_search_cxt *cxt;
  register int i;
  HIST_ENTRY **hlist;
  ...
  if (direction < 0) cxt->sflags |= SF_REVERSE;
  ...
  for (i = 0; i < cxt->hlen; i++) cxt->lines[i] = hlist[i]->line;
...
static void _rl_isearch_fini(cxt) _rl_search_cxt *cxt;
{
...
int _rl_search_getchar(cxt) _rl_search_cxt *cxt;
{
  int c;
...
int _rl_isearch_dispatch(cxt, c) _rl_search_cxt *cxt;
int c;
{
  ...
  switch (cxt->lastc) {
    ...
    case -1:
      if (cxt->search_string_index == 0) {
        if (last_isearch_string) {
          ...
          cxt->search_string =
              (char *)xrealloc(cxt->search_string, cxt->search_string_size);
    ...
    case -3: /* C-H, DEL */
      ...
      if (cxt->search_string_index == 0)
        ...
        wstart = _rl_find_prev_mbchar(
            cxt->search_string, cxt->search_string_index, MB_FIND_NONZERO);
        if (wstart >= 0)
      ...
      if (cxt->search_string_index == 0) rl_ding();
    ...
    case -5: /* C-W */
      ...
      while (n < rl_end) {
        ...
        if (_rl_walphabetic(cval) == 0) break;
...
int _rl_isearch_cleanup(cxt, r) _rl_search_cxt *cxt;
int r;
{
  if (r >= 0) _rl_isearch_fini(cxt);
...
/* Search through the history looking for an interactively typed string.
   ...
   backwards. */
static int rl_search_history(direction, invoking_key) int direction,
    invoking_key;
{
...
int _rl_isearch_callback(cxt) _rl_search_cxt *cxt;
{
  int c, r;
