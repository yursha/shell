...
int _rl_arg_dispatch(cxt, c) _rl_arg_cxt cxt;
int c;
{
  int key, r;
  ...
  if (_rl_digit_p(c)) {
    ...
    if (RL_ISSTATE(RL_STATE_CALLBACK)) {
      ...
      if (rl_done == 0) (*rl_redisplay_function)();
      r = 0;
    }
    return r;
  }
...
static int rl_digit_loop() {
  ...
  while (1) {
    ...
    if (r <= 0 || (RL_ISSTATE(RL_STATE_NUMERICARG) == 0)) break;
  }
...
int rl_digit_argument(ignore, key) int ignore, key;
{
  _rl_arg_init();
  if (RL_ISSTATE(RL_STATE_CALLBACK)) {
...
int rl_universal_argument(count, key) int count, key;
{
  _rl_arg_init();
  rl_numeric_arg *= 4;
...
int _rl_arg_callback(cxt) _rl_arg_cxt cxt;
{
  int c, r;
...
void _rl_free_history_entry(entry) HIST_ENTRY *entry;
{
  if (entry == 0) return;
...
int _rl_free_saved_history_line() {
  if (_rl_saved_line_for_history) {
    _rl_free_history_entry(_rl_saved_line_for_history);
    _rl_saved_line_for_history = (HIST_ENTRY *)NULL;
  }
...
void rl_replace_from_history(entry, flags) HIST_ENTRY *entry;
...
void _rl_revert_all_lines() {
  ...
  while (entry) {
    if (ul = (UNDO_LIST *)entry->data) {
      if (ul == saved_undo_list) saved_undo_list = 0;
      /* Set up rl_line_buffer and other variables from history entry */
      rl_replace_from_history(entry, 0); /* entry->line is now current */
      entry->data = 0; /* entry->data is now current undo list */
      /* Undo all changes to this history entry */
      while (rl_undo_list) rl_do_undo();
...
int rl_beginning_of_history(count, key) int count, key;
{ return (rl_get_previous_history(1 + where_history(), key)); }

/* Meta-> goes to the end of the history.  (The current line). */
int rl_end_of_history(count, key) int count, key;
{
...
int rl_get_next_history(count, key) int count, key;
{
  HIST_ENTRY *temp;
...
int rl_get_previous_history(count, key) int count, key;
{
  ...
  if (!temp && old_temp) temp = old_temp;
...
int rl_vi_editing_mode(count, key) int count, key;
{
#if defined(VI_MODE)
...
int rl_emacs_editing_mode(count, key) int count, key;
{
...
void _rl_set_insert_mode(im, force) int im, force;
{
#ifdef CURSOR_MODE
...
int rl_overwrite_mode(count, key) int count, key;
{
