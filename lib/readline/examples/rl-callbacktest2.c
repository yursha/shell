...
static char *cb_readline() {
  ...
  if (RL_ISSTATE(RL_STATE_ISEARCH))
    fprintf(stderr, "cb_readline: after handler install, state (ISEARCH) = %d",
            rl_readline_state);
  else if (RL_ISSTATE(RL_STATE_NSEARCH))
    fprintf(stderr, "cb_readline: after handler install, state (NSEARCH) = %d",
            rl_readline_state);
  ...
  while (input_string == not_done) {
    ...
    if (saw_signal) sigint_handler(saw_signal);
...
void sigint_sighandler(s) int s;
{ saw_signal = s; }

int sigint_handler(s) int s;
{
  rl_free_line_state();
