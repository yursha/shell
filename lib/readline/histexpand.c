...
int delimiting_quote;
{
  ...
  if (string[i] != history_expansion_char) return ((char *)NULL);
...
#define RETURN_ENTRY(e, w) \
  ...
  if (_rl_digit_p(string[i])) {
    ...
    *caller_index = i;
    ...
    if (sign < 0) which = (history_length + history_base) - which;
  ...
  if (which) strncpy(temp, string + local_index, which);
  temp[which] = '\0';
...
static void postproc_subst_rhs() {
  ...
  for (i = j = 0; i < subst_rhs_len; i++) {
    if (subst_rhs[i] == '&') {
      ...
      if (j >= new_size) new = (char *)xrealloc(new, new_size *= 2);
      new[j++] = subst_rhs[i];
    }
  }
  new[j] = '\0';
...
char *current_line; /* for !# */
{
    ...
    switch (c) {
      ...
      case 's': {
        ...
        if (c == 's') {
          ...
          FREE(subst_rhs);
          subst_rhs =
              get_subst_pattern(string, &i, delimiter, 1, &subst_rhs_len);

          /* If `&' appears in the rhs, it's supposed to be replaced
             with the lhs. */
          if (member('&', subst_rhs)) postproc_subst_rhs();
        } else
          i += 2;
        ...
        if (failed == 0) continue; /* don't want to increment i */
  ...
  n = strlen(temp);
  if (n >= result_len) result = (char *)xrealloc(result, n + 2);
  strcpy(result, temp);
  xfree(temp);
...
#define ADD_STRING(s)                                \
  do {                                               \
    ...
    j += sl;                                         \
    if (j >= result_len) {                           \
      while (j >= result_len) result_len += 128;     \
  ...
  if (hstring[0] == history_subst_char) {
    ...
    for (i = dquote = squote = 0; string[i]; i++) {
      ...
      if (history_comment_char && string[i] == history_comment_char &&
        ...
        if (cc == 0 || member(cc, history_no_expand_chars)) continue;
...
const char *string;
{
  ...
  if ((list = history_tokenize(string)) == NULL) return ((char *)NULL);
  ...
  if (first >= len || last > len || first < 0 || last < 0 || first > last)
    ...
    for (size = 0, i = first; i < last; i++) size += strlen(list[i]) + 1;
...
int ind;
{
  ...
  if (isdigit(string[i])) {
    j = i;
    while (string[j] && isdigit(string[j])) j++;
...
int start;
{
  ...
  for (i = start; words[i]; i++) xfree(words[i]);
}
