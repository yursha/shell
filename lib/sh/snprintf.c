...
#define GETSIGNED(p)                                                          \
  (((p)->flags & PF_LONGINT) ? GETARG(long) : (((p)->flags & PF_SHORTINT)     \
                                                   ? (long)(short)GETARG(int) \
                                                   : (long)GETARG(int)))
...
#define SET_SIZE_FLAGS(p, type)                             \
  if (sizeof(type) > sizeof(int)) (p)->flags |= PF_LONGINT; \
  if (sizeof(type) > sizeof(long)) (p)->flags |= PF_LONGLONG;
...
#define isnan(x)                                                               \
  (sizeof(x) == sizeof(LONGDOUBLE) ? isnan_ld(x) : sizeof(x) == sizeof(double) \
                                                       ? isnan_d(x)            \
                                                       : isnan_f(x))
#endif
...
#define isinf(x)                                                               \
  (sizeof(x) == sizeof(LONGDOUBLE) ? isinf_ld(x) : sizeof(x) == sizeof(double) \
                                                       ? isinf_d(x)            \
                                                       : isinf_f(x))
#endif
...
#define PUT_CHAR(c, p)                                                     \
  do {                                                                     \
    ...
    if ((p)->counter < (p)->length) *(p)->holder++ = (c);                  \
    (p)->counter++;                                                        \
  } while (0)
...
#define PUT_PLUS(d, p, zero) \
  if (((p)->flags & PF_PLUS) && (d) > zero) PUT_CHAR('+', p)

#define PUT_SPACE(d, p, zero) \
  if (((p)->flags & PF_SPACE) && (d) > zero) PUT_CHAR(' ', p)

/* pad right */
#define PAD_RIGHT(p)                          \
  if ((p)->width > 0 && (p)->justify != LEFT) \
    for (; (p)->width > 0; (p)->width--) PUT_CHAR((p)->pad, p)
...
#define PAD_ZERO(p)       \
  if ((p)->precision > 0) \
    for (; (p)->precision > 0; (p)->precision--) PUT_CHAR('0', p)
 ...
 */
static double integral(real, ip) double real;
double *ip;
{
  int j;
  double i, s, p;
 ...
 */
static char *numtoa(number, base, precision, fract) double number;
int base, precision;
char **fract;
{
  register int i, j;
  ...
  if (number == 0.) {
    ...
    for (ch = 0; ch < precision - 1; ch++) fraction_part[ch] = '0';
    fraction_part[ch] = '0';
    fraction_part[ch + 1] = '\0';
    if (fract) *fract = fraction_part;
  ...
  if (number != 0.)
    for (i = 0; i < digits; ++i) integral_part[i] = '9';

  /* put the sign ? */
  if (sign == '-') integral_part[i++] = '-';
  ...
  if (fract != (char **)0) *fract = fraction_part;
...
int base;
{
  ...
  switch (base) {
    case 10:
      ...
      break;
    case 8:
      if (p->flags & PF_ALTFORM) PUT_CHAR('0', p);
      break;
    case 16:
...
wint_t wc;
{
  ...
  if (len == (size_t)-1) /* conversion failed; bail now. */
    return;
  p->width -= len;
  l = lbuf;
  PUT_STRING(l, len, p);
...
static int chkinfnan(p, d, mode) struct DATA *p;
double d;
...
double d;
{
  ...
  t = 0;
  if ((p->flags & PF_THOUSANDS) && grouping && (t = groupnum(tmp))) tmp = t;

  if ((*p->pf == 'g' || *p->pf == 'G') && (p->flags & PF_ALTFORM) == 0) {
    /* smash the trailing zeros unless altform */
    for (i = strlen(tmp2) - 1; i >= 0 && tmp2[i] == '0'; i--) tmp2[i] = '\0';
    if (tmp2[0] == '\0') p->precision = 0;
  }
...
/* XXX - should this be d>0. && (p->flags & PF_PLUS) ? */
  ...
  for (; *tmp2; tmp2++) PUT_CHAR(*tmp2, p); /* the fraction */
...
#else
    vsnprintf_internal(data, string, length, format, args) struct DATA *data;
char *string;
size_t length;
const char *format;
va_list args;
...
/* fmtend = data->pf */
      ...
      switch (c) {
        ...
        case '-':
          ...
          continue;
        case ' ':
          if ((data->flags & PF_PLUS) == 0) data->flags |= PF_SPACE;
          continue;
        case '+':
          if ((data->flags & PF_DOT) == 0) {
            data->flags |= PF_PLUS;
            if ((data->flags & PF_LADJUST) == 0) data->justify = RIGHT;
          }
          continue;
        case '\'':
...
/* Conversion specifiers */
        ...
        case 'G':
          ...
          if (-4 < i && i < data->precision) {
            ...
            if (data->precision > 0) data->precision--;
            exponent(data, d);
          }
          state = 0;
          break;
...
/* FALLTHROUGH */
  ...
  if (data->length >= 0) *data->holder = '\0'; /* the end ye ! */
...
long double ld;
{
  ...
  for (x = obuf; *x; x++) PUT_CHAR(*x, data);
  xfree(obuf);
}
#endif /* FLOATING_POINT && HAVE_LONG_DOUBLE */
...
#else
    vsnprintf(string, length, format, args) char *string;
size_t length;
const char *format;
va_list args;
#endif
...
#else
    snprintf(string, length, format, va_alist) char *string;
size_t length;
const char *format;
va_dcl
#endif
...
#else
    vasprintf(stringp, format, args) char **stringp;
const char *format;
va_list args;
#endif
{
...
#else
    asprintf(stringp, format, va_alist) char **stringp;
const char *format;
va_dcl
#endif
{
...
static void *xmalloc(bytes) size_t bytes;
{
  void *ret;

  ret = malloc(bytes);
  if (ret == 0) memory_error_and_abort();
  return ret;
}
