...
#undef fpurge
  ...
  if (result == 0)
    ...
    if ((fp_->_flags & __SRD) != 0) fp_->_w = 0;
#endif
  return result;
...
/* Most systems provide FILE as a struct and the necessary bitmask in
  ...
  /* Avoid memory leak when there is an active ungetc buffer.  */
  if (fp_ub._base != NULL) {
    if (fp_ub._base != fp_->_ubuf) free(fp_ub._base);
    fp_ub._base = NULL;
  }
  return 0;
  ...
  return 0;
#elif defined _IOERR || \
    defined __TANDEM     /* AIX, HP-UX, IRIX, OSF/1, Solaris, OpenServer, mingw */
  fp->_ptr = fp->_base;
  ...
  fp->_Rsave = NULL;
  if (fp->_Mode & 0x2000 /* _MWRITE */) /* fp->_Buf <= fp->_Next <= fp->_Wend */
    fp->_Next = fp->_Buf;
  else
  ...
  if (fp->__target != -1) fp->__target += fp->__bufp - fp->__buffer;
