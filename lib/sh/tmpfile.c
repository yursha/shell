...
static char *get_sys_tmpdir() {
  if (sys_tmpdir) return sys_tmpdir;
...
static char *get_tmpdir(flags) int flags;
{
  char *tdir;

  tdir = (flags & MT_USETMPDIR) ? get_string_value("TMPDIR") : (char *)NULL;
  if (tdir && (file_iswdir(tdir) == 0 || strlen(tdir) > PATH_MAX)) tdir = 0;

  if (tdir == 0) tdir = get_sys_tmpdir();

#if defined(HAVE_PATHCONF) && defined(_PC_NAME_MAX)
  if (tmpnamelen == -1) tmpnamelen = pathconf(tdir, _PC_NAME_MAX);
#else
  tmpnamelen = 0;
#endif
...
char *sh_mktmpname(nameroot, flags) char *nameroot;
int flags;
{
...
int sh_mktmpfd(nameroot, flags, namep) char *nameroot;
int flags;
char **namep;
{
  char *filename, *tdir, *lroot;
...
FILE *sh_mktmpfp(nameroot, flags, namep) char *nameroot;
int flags;
char **namep;
{
  int fd;
