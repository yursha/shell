...
ssize_t zread(fd, buf, len) int fd;
char *buf;
size_t len;
{
  ssize_t r;
...
ssize_t zreadretry(fd, buf, len) int fd;
char *buf;
size_t len;
{
  ssize_t r;
...
ssize_t zreadintr(fd, buf, len) int fd;
char *buf;
size_t len;
...
ssize_t zreadcintr(fd, cp) int fd;
char *cp;
{
  ssize_t nr;
...
/* Like zreadc, but read a specified number of characters at a time.  Used
   for `read -N'. */
ssize_t zreadn(fd, cp, len) int fd;
char *cp;
size_t len;
{
  ssize_t nr;
  ...
  if (lind == lused || lused == 0) {
    if (len > sizeof(lbuf)) len = sizeof(lbuf);
...
void zsyncfd(fd) int fd;
{
  ...
  off = lused - lind;
  r = 0;
  if (off > 0) r = lseek(fd, -off, SEEK_CUR);
