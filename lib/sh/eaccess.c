...
static int sh_stataccess(path, mode) char *path;
int mode;
{
  struct stat st;

  if (sh_stat(path, &st) < 0) return (-1);
...
static int sh_euidaccess(path, mode) char *path;
int mode;
{
  int r, e;
...
int sh_eaccess(path, mode) char *path;
int mode;
{
  int ret;

  if (path_is_devfd(path)) return (sh_stataccess(path, mode));
...
#else
  if (mode == F_OK) return (sh_stataccess(path, mode));
