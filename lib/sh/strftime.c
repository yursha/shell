...
#if !defined(OS2) && !defined(MSDOS) && !defined(__CYGWIN__) && \
    defined(HAVE_TZNAME)
...
size_t strftime(char *s, size_t maxsize, const char *format,
  ...
  if (s == NULL || format == NULL || timeptr == NULL || maxsize == 0) return 0;
 ...
 */
      ...
      case 'v': /* date as dd-bbb-YYYY */
        ...
        for (i = 3; i < 6; i++)
          if (islower(tbuf[i])) tbuf[i] = toupper(tbuf[i]);
        break;
#endif
