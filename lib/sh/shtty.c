...
TTYSTRUCT *ttattr(fd) int fd;
{
  if (ttsaved == 0) return ((TTYSTRUCT *)0);
  if (fd == 0)
    return &ttin;
  else if (fd == 1)
    return &ttout;
 ...
 */
int tt_setonechar(ttp) TTYSTRUCT *ttp;
{
...
int ttfd_onechar(fd, ttp) int fd;
TTYSTRUCT *ttp;
{
  if (tt_setonechar(ttp) < 0) return -1;
 ...
 */
int tt_setnoecho(ttp) TTYSTRUCT *ttp;
{
#if defined(TERMIOS_TTY_DRIVER) || defined(TERMIO_TTY_DRIVER)
...
int ttfd_noecho(fd, ttp) int fd;
TTYSTRUCT *ttp;
{
  if (tt_setnoecho(ttp) < 0) return -1;
  return (ttsetattr(fd, ttp));
}
 ...
 */
int tt_seteightbit(ttp) TTYSTRUCT *ttp;
{
#if defined(TERMIOS_TTY_DRIVER) || defined(TERMIO_TTY_DRIVER)
  ttp->c_iflag &= ~ISTRIP;
  ttp->c_cflag |= CS8;
...
int ttfd_eightbit(fd, ttp) int fd;
TTYSTRUCT *ttp;
{
  if (tt_seteightbit(ttp) < 0) return -1;
  return (ttsetattr(fd, ttp));
}
 ...
 */
int tt_setnocanon(ttp) TTYSTRUCT *ttp;
{
#if defined(TERMIOS_TTY_DRIVER) || defined(TERMIO_TTY_DRIVER)
  ttp->c_lflag &= ~ICANON;
#endif
...
int ttfd_nocanon(fd, ttp) int fd;
TTYSTRUCT *ttp;
{
  if (tt_setnocanon(ttp) < 0) return -1;
  return (ttsetattr(fd, ttp));
}
 ...
 */
int tt_setcbreak(ttp) TTYSTRUCT *ttp;
{
  if (tt_setonechar(ttp) < 0) return -1;
...
int ttfd_cbreak(fd, ttp) int fd;
TTYSTRUCT *ttp;
{
  if (tt_setcbreak(ttp) < 0) return -1;
  return (ttsetattr(fd, ttp));
}
