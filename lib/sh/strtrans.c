...
char *ansicstr(string, len, flags, sawc, rlen) char *string;
int len, flags, *sawc, *rlen;
{
  ...
  temp = 4 * len + 1;
  if (temp < 12) temp = 12; /* ensure enough for eventual u32cesc */
...
/*FALLTHROUGH*/
        ...
        case '0':
          /* If (FLAGS & 1), we're translating a string for echo -e (or
             ...
             a leading `0'. */
          temp = 2 + ((flags & 1) && (c == '0'));
          for (c -= '0'; ISOCTAL(*s) && temp--; s++) c = (c * 8) + OCTVALUE(*s);
          c &= 0xFF;
          break;
      ...
      if ((flags & 2) && (c == CTLESC || c == CTLNUL)) *r++ = CTLESC;
      *r++ = c;
    }
  }
  *r = '\0';
...
char *ansic_quote(str, flags, rlen) char *str;
int flags, *rlen;
{
  char *r, *ret, *s;
  int l, rsize;
  ...
  for (s = str; c = *s; s++) {
    ...
    if (clen == 1)
      *r++ = c;
      ...
      for (b = 0; b < (int)clen; b++) *r++ = (unsigned char)s[b];
...
int ansic_shouldquote(string) const char *string;
{
  ...
  for (s = string; c = *s; s++) {
#if defined(HANDLE_MULTIBYTE)
    if (is_basic(c) == 0) return (ansic_wshouldquote(s));
#endif
...
char *ansiexpand(string, start, end, lenp) char *string;
int start, end, *lenp;
{
  char *temp, *t;
  int len, tlen;
