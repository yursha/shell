...
char **strvec_create(n) int n;
{ return ((char **)xmalloc((n) * sizeof(char *))); }

/* Allocate an array of strings with room for N members. */
char **strvec_mcreate(n) int n;
{ return ((char **)malloc((n) * sizeof(char *))); }

char **strvec_resize(array, nsize) char **array;
int nsize;
{ return ((char **)xrealloc(array, nsize * sizeof(char *))); }

char **strvec_mresize(array, nsize) char **array;
int nsize;
{ return ((char **)realloc(array, nsize * sizeof(char *))); }

/* Return the length of ARRAY, a NULL terminated array of char *. */
int strvec_len(array) char **array;
{
  register int i;
...
int strvec_remove(array, name) char **array, *name;
{
  register int i, j;
  char *x;
...
int strvec_search(array, name) char **array, *name;
{
  int i;

  for (i = 0; array[i]; i++)
    if (STREQ(name, array[i])) return (i);
...
char **strvec_copy(array) char **array;
{
  register int i;
  int len;
  char **ret;
  ...
  for (i = 0; array[i]; i++) ret[i] = savestring(array[i]);
  ret[i] = (char *)NULL;
...
int strvec_strcmp(s1, s2) register char **s1, **s2;
{
  ...
  if ((result = **s1 - **s2) == 0) result = strcmp(*s1, *s2);
...
void strvec_sort(array) char **array;
...
char **strvec_from_word_list(list, alloc, starting_index, ip) WORD_LIST *list;
int alloc, starting_index, *ip;
{
  ...
  for (count = 0; count < starting_index; count++) array[count] = (char *)NULL;
...
WORD_LIST *strvec_to_word_list(array, alloc, starting_index) char **array;
