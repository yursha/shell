...
static int _getaddr(host, ap) char *host;
struct in_addr *ap;
{
  struct hostent *h;
  int r;
...
static int _getserv(serv, proto, pp) char *serv;
int proto;
unsigned short *pp;
{
  intmax_t l;
  ...
  if (legal_number(serv, &l)) {
    ...
    if (se == 0) return 0;
    if (pp) *pp = se->s_port; /* ports returned in network byte order */
    return 1;
  }
 ...
 */
static int _netopen4(host, serv, typ) char *host, *serv;
int typ;
{
  struct in_addr ina;
 ...
 * on error.
 */
static int _netopen6(host, serv, typ) char *host, *serv;
int typ;
{
  int s, e;
 ...
 */
static int _netopen(host, serv, typ) char *host, *serv;
int typ;
{
#ifdef HAVE_GETADDRINFO
