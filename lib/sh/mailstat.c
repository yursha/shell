...
struct stat *st;
{
  ...
  if ((i = stat(path, st)) != 0 || S_ISDIR(st->st_mode) == 0) return i;
  ...
  if (stat(dir, &st_tmp) || S_ISDIR(st_tmp.st_mode) == 0) return 0;
  ...
  for (i = 0; i < 2; i++) {
    ...
    while ((fn = readdir(dd)) != NULL) {
      ...
      if (st_tmp.st_mtime > mtime) mtime = st_tmp.st_mtime;
    }
    closedir(dd);
  }
