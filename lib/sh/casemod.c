...
#define CASE_USEWORDS \
  0x1000 /* modify behavior to act on words in passed string */
...
#if defined(HANDLE_MULTIBYTE)
static wchar_t cval(s, i) char *s;
int i;
{
  size_t tmp;
  wchar_t wc;
  ...
  if (MB_CUR_MAX == 1 || is_basic(s[i])) return ((wchar_t)s[i]);
  l = strlen(s);
  if (i >= (l - 1)) return ((wchar_t)s[i]);
  memset(&mps, 0, sizeof(mbstate_t));
  tmp = mbrtowc(&wc, s + i, l - i, &mps);
  if (MB_INVALIDCH(tmp) || MB_NULLWCH(tmp)) return ((wchar_t)s[i]);
  return wc;
}
#endif
