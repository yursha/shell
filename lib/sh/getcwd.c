...
#if !defined(D_FILENO_AVAILABLE)
static int _path_checkino(dotp, name, thisino) char *dotp;
char *name;
ino_t thisino;
{
  char *fullpath;
  ...
  while (!(thisdev == rootdev && thisino == rootino)) {
    ...
    while ((d = readdir(dirstream)) != NULL) {
...
#endif
      {
        ...
        if (st.st_dev == thisdev && st.st_ino == thisino) break;
      }
    }
    if (d == NULL) {
#if 0
