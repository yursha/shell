...
/* u32toascii ? */
int u32tochar(x, s) unsigned long x;
char *s;
{
  int l;
...
int u32tocesc(wc, s) u_bits32_t wc;
char *s;
{
  int l;
...
int u32toutf8(wc, s) u_bits32_t wc;
char *s;
{
  int l;
...
int u32toutf16(c, s) u_bits32_t c;
unsigned short *s;
{
  int l;
...
/* convert a single unicode-32 character into a multibyte string and put the
   ...
   */
int u32cconv(c, s) unsigned long c;
char *s;
{
  wchar_t wc;
  wchar_t ws[3];
  ...
  /* If the conversion is not supported, even the ASCII requested above, we
     ...
     u32tocesc(). */
  if (localconv == (iconv_t)-1) return n;
  ...
  if (iconv(localconv, (ICONV_CONST char **)&iptr, &sn, &optr, &obytesleft) ==
      (size_t)-1) {
#if 1
