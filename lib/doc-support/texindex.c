...
char **argv;
{
  ...
  for (i = 0; i < num_infiles; i++) {
    ...
    if (ptr < MAX_IN_CORE_SORT) /* Sort a small amount of data. */
...
void decode_command(argc, argv) int argc;
char **argv;
{
  int optc;
  char **ip;
  ...
  *ip = 0;
  if (num_infiles == 0) usage();
}

/* Return a name for a temporary file. */

char *maketempname(count) int count;
{
  char tempsuffix[10];
...
void flush_tempfiles(to_count) int to_count;
{
...
int compare_full(line1, line2) char **line1, **line2;
{
  int i;
  ...
  for (i = 0; i < num_keyfields; i++) {
    ...
    if (tem) {
      if (keyfields[i].reverse) return -tem;
      return tem;
    }
  }
...
int compare_prepared(line1, line2) struct lineinfo *line1, *line2;
{
  int i;
  int tem;
  char *text1, *text2;
  ...
  if (tem) {
    if (keyfields->reverse) return -tem;
    return tem;
  }
...
int compare_general(str1, str2, pos1, pos2, use_keyfields) char *str1, *str2;
long pos1, pos2;
int use_keyfields;
{
  int i;
...
char *find_field(keyfield, str, lengthptr) struct keyfield *keyfield;
char *str;
long *lengthptr;
{
  char *start;
...
char *find_pos(str, words, chars, ignore_blanks) char *str;
int words, chars;
int ignore_blanks;
{
  int i;
...
char *find_braced_pos(str, words, chars, ignore_blanks) char *str;
int words, chars;
int ignore_blanks;
{
  int i;
...
char *find_braced_end(str) char *str;
{
  int bracelevel;
  char *p = str;
  char c;
...
long find_value(start, length) char *start;
long length;
{
  while (length != 0L) {
    if (isdigit(*start)) return atol(start);
    length--;
    start++;
  }
  return 0l;
...
void init_char_order() {
  int i;
  for (i = 1; i < 256; i++) char_order[i] = i;

  for (i = '0'; i <= '9'; i++) char_order[i] += 512;
...
int compare_field(keyfield, start1, length1, pos1, start2, length2,
                  pos2) struct keyfield *keyfield;
char *start1;
long length1;
long pos1;
char *start2;
...
long pos2;
{
  ...
  if (keyfield->numeric) {
    ...
    while (1) {
      ...
      if (c1 != c2) /* Reverse sign here so upper case comes out last.  */
        return c2 - c1;
      if (!c1) break;
    }
...
void initbuffer(linebuffer) struct linebuffer *linebuffer;
{
  linebuffer->size = 200;
...
long readline(linebuffer, stream) struct linebuffer *linebuffer;
FILE *stream;
{
...
void sort_offline(infile, nfiles, total, outfile) char *infile;
int nfiles;
long total;
char *outfile;
{
...
fail:
  ...
  for (i = 0; i < ntemps; i++) {
    ...
    if (!keep_tempfiles) unlink(tempfiles[i]);
    tempfiles[i] = newtemp;
  }
...
void sort_in_core(infile, total, outfile) char *infile;
long total;
char *outfile;
{
  char **nextline;
  ...
  if (desc < 0) fatal("failure reopening %s", infile);
  for (file_size = 0;;) {
  ...
  lineinfo = (struct lineinfo *)malloc((nextline - linearray) *
  ...
  if (lineinfo) {
    ...
    for (lp = lineinfo, p = linearray; p != nextline; lp++, p++) *p = lp->text;
...
char **parsefile(filename, nextline, data, size) char *filename;
char **nextline;
char *data;
long size;
{
...
void indexify(line, ostream) char *line;
FILE *ostream;
{
  ...
  if (*p == '{') {
    ...
    if (initial1[0] >= 'a' && initial1[0] <= 'z') initial1[0] -= 040;
  }

  pagenumber = find_braced_pos(line, 1, 0, 0);
  pagelength = find_braced_end(pagenumber) - pagenumber;
  if (pagelength == 0) abort();
  ...
  if (!nosecondary) secondarylength = find_braced_end(secondary) - secondary;
  ...
  if (strncmp(primary, lastprimary, primarylength)) {
    ...
    if (lastprimarylength < primarylength) {
      ...
      lastprimary = (char *)xrealloc(lastprimary, 1 + lastprimarylength);
    }
  ...
  if (!nosecondary && strncmp(secondary, lastsecondary, secondarylength)) {
    ...
    if (lastsecondarylength < secondarylength) {
      ...
      lastsecondary = (char *)xrealloc(lastsecondary, 1 + lastsecondarylength);
    }
...
void finish_index(ostream) FILE *ostream;
{
  if (pending) fputs("}\n", ostream);
  free(lastprimary);
  free(lastsecondary);
...
void writelines(linearray, nlines, ostream) char **linearray;
int nlines;
FILE *ostream;
{
...
int merge_files(infiles, nfiles, outfile) char **infiles;
int nfiles;
char *outfile;
{
  char **tempfiles;
  ...
  if (nfiles <= MAX_DIRECT_MERGE) return merge_direct(infiles, nfiles, outfile);
  ...
  for (i = 0; i < ntemps; i++) {
    ...
    if (i + 1 == ntemps) nf = nfiles - i * MAX_DIRECT_MERGE;
...
int merge_direct(infiles, nfiles, outfile) char **infiles;
int nfiles;
char *outfile;
{
  ...
  thisline =
      (struct linebuffer **)xmalloc(nfiles * sizeof(struct linebuffer *));
  ...
  for (i = 0; i < nfiles; i++) {
    ...
    if (!streams[i]) pfatal_with_name(infiles[i]);
...
char *concat(s1, s2, s3) char *s1, *s2, *s3;
{
...
void *xmalloc(nbytes) int nbytes;
{
...
void *xrealloc(pointer, nbytes) void *pointer;
int nbytes;
{
  void *temp;
...
memory_error(callers_name, bytes_wanted) char *callers_name;
int bytes_wanted;
{
