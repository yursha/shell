...
static WORD_LIST *call_expand_word_internal(WORD_DESC *, int, int, int *,
                                            int *);
static WORD_LIST *expand_string_internal(char *, int);
static WORD_LIST *expand_string_leave_quoted(char *, int);
static WORD_LIST *expand_string_for_rhs(char *, int, int *, int *);
...
#if defined(HANDLE_MULTIBYTE)
static int match_wpattern(wchar_t *, char **, size_t, wchar_t *, int, char **,
                          char **);
#endif
...
#ifdef ARRAY_VARS
static char *array_remove_pattern(SHELL_VAR *, char *, int, char *, int);
#endif
static char *parameter_brace_remove_pattern(char *, char *, int, char *, int,
                                            int, int);

static char *string_var_assignment(SHELL_VAR *, char *);
#if defined(ARRAY_VARS)
static char *array_var_assignment(SHELL_VAR *, int, int);
#endif
...
#if defined ARRAY_VARS
static char *array_transform(int, SHELL_VAR *, char *, int);
#endif
static char *parameter_brace_transform(char *, char *, int, char *, int, int,
                                       int);

static char *process_substitute(char *, int);

static char *read_comsub(int, int, int *);
...
static int chk_atstar(char *, int, int *, int *);
static int chk_arithsub(const char *, int);

static WORD_DESC *parameter_brace_expand_word(char *, int, int, int,
                                              arrayind_t *);
static char *parameter_brace_find_indir(char *, int, int, int);
static WORD_DESC *parameter_brace_expand_indir(char *, int, int, int *, int *);
static WORD_DESC *parameter_brace_expand_rhs(char *, char *, int, int, int,
                                             int *, int *);
static void parameter_brace_expand_error(char *, char *);

static int valid_length_expression(char *);
static intmax_t parameter_brace_expand_length(char *);

static char *skiparith(char *, int);
static int verify_substring_values(SHELL_VAR *, char *, char *, int, intmax_t *,
                                   intmax_t *);
static int get_var_and_type(char *, char *, arrayind_t, int, int, SHELL_VAR **,
                            char **);
static char *mb_substring(char *, int, int);
static char *parameter_brace_substring(char *, char *, int, char *, int, int);
...
static char *parameter_brace_casemod(char *, char *, int, int, char *, int,
                                     int);

static WORD_DESC *parameter_brace_expand(char *, int *, int, int, int *, int *);
static WORD_DESC *param_expand(char *, int *, int, int *, int *, int *, int *,
                               int);
...
static char *quoted_substring(string, start, end) char *string;
int start, end;
{
  register int len, l;
...
static int quoted_strlen(s) char *s;
{
  register char *p;
  int i;
...
static char *quoted_strchr(s, c, flags) char *s;
int c, flags;
{
  register char *p;
...
static int unquoted_member(character, string) int character;
char *string;
{
  size_t slen;
  int sindex, c;
  ...
  while (c = string[sindex]) {
    ...
    switch (c) {
      ...
      case '\\':
        sindex++;
        if (string[sindex]) ADVANCE_CHAR(string, slen, sindex);
        break;
...
static int unquoted_substring(substr, string) char *substr, *string;
{
  size_t slen;
  int sindex, c, sublen;
  DECLARE_MBSTATE;

  if (substr == 0 || *substr == '\0') return (0);

  slen = strlen(string);
  sublen = strlen(substr);
  for (sindex = 0; c = string[sindex];) {
    if (STREQN(string + sindex, substr, sublen)) return (1);
...
INLINE char *sub_append_string(source, target, indx, size) char *source,
    *target;
int *indx, *size;
{
  if (source) {
...
static char *string_extract(string, sindex, charlist, flags) char *string;
int *sindex;
char *charlist;
int flags;
{
...
static char *string_extract_double_quoted(string, sindex, stripdq) char *string;
int *sindex, stripdq;
{
  size_t slen;
  char *send;
  ...
  while (c = string[i]) {
    ...
    if (pass_next) {
      ...
      if ((stripdq == 0 && c != '"') ||
          (stripdq &&
           ((dquote && (sh_syntaxtab[c] & CBSDQUOTE)) || dquote == 0)))
        temp[j++] = '\\';
      pass_next = 0;
    ...
    if (c == '$' && ((string[i + 1] == LPAREN) || (string[i + 1] == LBRACE))) {
      ...
      if (free_ret) free(ret);
      continue;
    }

    /* Add any character but a double quote to the quoted string we're
       accumulating. */
    if (c != '"') goto add_one_character;
...
static int skip_double_quoted(string, slen, sind, flags) char *string;
size_t slen;
int sind;
int flags;
{
...
static inline char *string_extract_single_quoted(string, sindex) char *string;
int *sindex;
{
  register int i;
  size_t slen;
  ...
  slen = (MB_CUR_MAX > 1) ? strlen(string + *sindex) + *sindex : 0;
  i = *sindex;
  while (string[i] && string[i] != '\'') ADVANCE_CHAR(string, slen, i);
...
static char *string_extract_verbatim(string, slen, sindex, charlist,
                                     flags) char *string;
size_t slen;
int *sindex;
char *charlist;
...
char *extract_command_subst(string, sindex, xflags) char *string;
int *sindex;
int xflags;
{
  char *ret;
...
/* Extract the $[ construct in STRING, and return a new string. (])
   Start extracting at (SINDEX) as if we had just seen "$[".
   Make (SINDEX) get the position of the matching "]". */
char *extract_arithmetic_subst(string, sindex) char *string;
int *sindex;
...
/* Extract the <( or >( construct in STRING, and return a new string.
   Start extracting at (SINDEX) as if we had just seen "<(".
   Make (SINDEX) get the position of the matching ")". */ /*))*/
char *extract_process_subst(string, starter, sindex, xflags) char *string;
char *starter;
int *sindex;
int xflags;
{
...
char *extract_array_assignment_list(string, sindex) char *string;
int *sindex;
{
  int slen;
  char *ret;
...
static char *extract_dollar_brace_string(string, sindex, quoted,
                                         flags) char *string;
...
char *de_backslash(string) char *string;
{
...
static int skip_matched_pair(string, start, open, close, flags) const
    char *string;
...
int skip_to_delim(string, start, delims, flags) char *string;
int start;
char *delims;
int flags;
{
  ...
  slen = strlen(string + start) + start;
  oldjmp = no_longjmp_on_fatal_error;
  if (flags & SD_NOJMP) no_longjmp_on_fatal_error = 1;
  ...
  while (c = string[i]) {
    ...
    /* special case for programmable completion which takes place before
...
#if 1
      temp =
          extract_delimited_string(string, &si, (c == '<') ? "<(" : ">(", "(",
      ...
      open[1] = LPAREN;
      open[2] = '\0';
      temp = extract_delimited_string(string, &si, open, "(", ")",
                                      SX_NOALLOC); /* ) */
...
int skip_to_histexp(string, start, delims, flags) char *string;
int start;
char *delims;
int flags;
{
...
int char_is_quoted(string, eindex) char *string;
int eindex;
{
...
int unclosed_pair(string, eindex, openstr) char *string;
int eindex;
char *openstr;
{
...
WORD_LIST *split_at_delims(string, slen, delims, sentinel, flags, nwp,
                           cwp) char *string;
int slen;
char *delims;
int sentinel, flags;
int *nwp, *cwp;
{
...
#endif
      if (whitespace(delims[i]) == 0) d2[ts++] = delims[i];
  ...
  while (1) {
    ...
    if (ts == te && d2 && member(string[ts], d2)) {
      ...
      if (ifs_split)
        while (member(string[te], d) && spctabnl(string[te])) te++;
      else
        while (member(string[te], d2)) te++;
    }
    ...
    if (sentinel >= ts && sentinel <= te) cw = nw;

    /* If the cursor is at whitespace just before word start, set the
       sentinel word to the current word. */
    if (cwp && cw == -1 && sentinel == ts - 1) cw = nw;
    ...
    if (string[te] == 0) break;

    i = te;
    while (member(string[i], d) && (ifs_split || spctabnl(string[i]))) i++;
...
char *string_list_internal(list, sep) WORD_LIST *list;
char *sep;
{
  ...
  if (list == 0) return ((char *)NULL);

  /* Short-circuit quickly if we don't need to separate anything. */
  if (list->next == 0) return (savestring(list->word->word));
...
char *string_list(list) WORD_LIST *list;
...
char *ifs_firstchar(lenp) int *lenp;
{
  char *ret;
  int len;
...
char *string_list_dollar_star(list) WORD_LIST *list;
{
  char *ret;
#if defined(HANDLE_MULTIBYTE)
#if defined(__GNUC__)
...
/* Turn $@ into a string.  If (quoted & (Q_HERE_DOCUMENT|Q_DOUBLE_QUOTES))
   ...
   exactly. */
char *string_list_dollar_at(list, quoted, flags) WORD_LIST *list;
int quoted;
int flags;
{
  char *ifs, *ret;
...
char *string_list_pos_params(pchar, list, quoted) int pchar;
WORD_LIST *list;
int quoted;
{
  char *ret;
...
WORD_LIST *list_string(string, separators, quoted) register char *string,
    *separators;
int quoted;
{
  WORD_LIST *result;
  ...
  if (!string || !*string) return ((WORD_LIST *)NULL);
  ...
  for (result = (WORD_LIST *)NULL, sindex = 0; string[sindex];) {
    ...
    if (string[sindex] && whitesep && issep(string[sindex]) &&
      ...
      while (string[sindex] && spctabnl(string[sindex]) &&
             isifs(string[sindex]))
        sindex++;
    }
  }
...
/* Parse a single word from STRING, using SEPARATORS to separate fields.
   ...
   XXX - this function is very similar to list_string; they should be
         combined - XXX */
char *get_word_from_string(stringp, separators, endptr) char **stringp,
    *separators, **endptr;
{
  register char *s;
  char *current_word;
  ...
  if (!stringp || !*stringp || !**stringp) return ((char *)NULL);
  ...
  /* Now skip sequences of space, tab, or newline characters if they are
     in the list of separators. */
  while (s[sindex] && spctabnl(s[sindex]) && isifs(s[sindex])) sindex++;
...
/* Remove IFS white space at the end of STRING.  Start at the end
   ...
   non-zero.  */
char *strip_trailing_ifs_whitespace(string, separators, saw_escape) char *string
    ,
    *separators;
int saw_escape;
{
...
#if defined(ARRAY_VARS)
static SHELL_VAR *do_compound_assignment(name, value, flags) char *name, *value;
int flags;
{
  SHELL_VAR *v;
  ...
  if (mklocal && variable_context) {
    ...
    if (v && ((readonly_p(v) && (flags & ASS_FORCE) == 0) || noassign_p(v))) {
      if (v && readonly_p(v)) err_readonly(name);
      return (v); /* XXX */
    }
    ...
    if (v) assign_compound_array_list(v, list, flags);
    if (list) dispose_words(list);
  }
...
int expand;
{
  ...
  if (word == 0 || word->word == 0) return 0;
...
#define ASSIGN_RETURN(r) \
  ...
  if (t = mbschr(name, '[')) /*]*/
    ...
    if (entry == 0) ASSIGN_RETURN(0);
  } else if (assign_list) {
    if ((word->flags & W_ASSIGNARG) && (word->flags & W_ASSNGLOBAL) == 0)
      aflags |= ASS_MKLOCAL;
    if ((word->flags & W_ASSIGNARG) && (word->flags & W_ASSNGLOBAL))
      aflags |= ASS_MKGLOBAL;
    if (word->flags & W_ASSIGNASSOC) aflags |= ASS_MKASSOC;
...
int do_assignment(string) char *string;
{
  WORD_DESC td;
...
int do_word_assignment(word, flags) WORD_DESC *word;
int flags;
...
int do_assignment_no_expand(string) char *string;
{
  WORD_DESC td;
...
int number_of_args() {
  ...
  for (n = 0; n < 9 && dollar_vars[n + 1]; n++)
    ;
  for (list = rest_of_args; list; list = list->next) n++;
  return n;
}

/* Return the value of a positional parameter.  This handles values > 10. */
char *get_dollar_var_value(ind) intmax_t ind;
{
  char *temp;
  WORD_LIST *p;
...
char *string_rest_of_args(dollar_star) int dollar_star;
{
  register WORD_LIST *list;
  char *string;
...
static char *pos_params(string, start, end, quoted) char *string;
int start, end, quoted;
{
  ...
  if (save == 0 && start > 0) return ((char *)NULL);
  ...
  for (i = start ? 1 : 0; params && i < start; i++) params = params->next;
  if (params == 0) {
    dispose_words(save);
    return ((char *)NULL);
  }
...
static char *expand_string_if_necessary(string, quoted, func) char *string;
int quoted;
EXPFUNC *func;
{
  WORD_LIST *list;
...
static inline char *expand_string_to_string_internal(string, quoted,
                                                     func) char *string;
int quoted;
EXPFUNC *func;
{
  ...
  if (string == 0 || *string == '\0') return ((char *)NULL);
...
char *expand_string_to_string(string, quoted) char *string;
int quoted;
{ return (expand_string_to_string_internal(string, quoted, expand_string)); }

char *expand_string_unsplit_to_string(string, quoted) char *string;
int quoted;
{
  return (
...
char *expand_assignment_string_to_string(string, quoted) char *string;
int quoted;
{
...
char *expand_arith_string(string, quoted) char *string;
int quoted;
{
  WORD_DESC td;
...
char *remove_backslashes(string) char *string;
{
  char *r, *ret, *s;
...
char *cond_expand_word(w, special) WORD_DESC *w;
int special;
{
  char *r, *p;
  WORD_LIST *l;
  ...
  if (w->word == 0 || w->word[0] == '\0') return ((char *)NULL);
  ...
  if (l) {
    if (special == 0) /* LHS */
      ...
      if (special == 2) qflags |= QGLOB_REGEXP;
...
static WORD_LIST *call_expand_word_internal(w, q, i, c, e) WORD_DESC *w;
int q, i, *c, *e;
{
  WORD_LIST *result;
...
/* Perform parameter expansion, command substitution, and arithmetic
   ...
   in the result.  */
static WORD_LIST *expand_string_internal(string, quoted) char *string;
int quoted;
{
  WORD_DESC td;
  WORD_LIST *tresult;

  if (string == 0 || *string == 0) return ((WORD_LIST *)NULL);
...
WORD_LIST *expand_string_unsplit(string, quoted) char *string;
int quoted;
{
  WORD_LIST *value;
...
WORD_LIST *expand_string_assignment(string, quoted) char *string;
int quoted;
{
  WORD_DESC td;
  WORD_LIST *value;
...
WORD_LIST *expand_prompt_string(string, quoted, wflags) char *string;
int quoted;
int wflags;
{
  WORD_LIST *value;
...
static WORD_LIST *expand_string_leave_quoted(string, quoted) char *string;
int quoted;
{
  WORD_LIST *tlist;
  WORD_LIST *tresult;
...
/* This does not perform word splitting or dequote the WORD_LIST
   it returns. */
static WORD_LIST *expand_string_for_rhs(string, quoted, dollar_at_p,
                                        expanded_p) char *string;
...
WORD_LIST *expand_string(string, quoted) char *string;
int quoted;
{
  WORD_LIST *result;
...
char *quote_escapes(string) char *string;
{
  register char *s, *t;
  size_t slen;
  char *result, *send;
  ...
  while (*s) {
    if ((skip_ctlesc == 0 && *s == CTLESC) ||
        (skip_ctlnul == 0 && *s == CTLNUL) || (quote_spaces && *s == ' '))
      *t++ = CTLESC;
...
static WORD_LIST *list_quote_escapes(list) WORD_LIST *list;
{
...
char *dequote_escapes(string) char *string;
{
  ...
  if (strchr(string, CTLESC) == 0) return (strcpy(result, string));
  ...
  while (*s) {
    if (*s == CTLESC &&
        (s[1] == CTLESC || s[1] == CTLNUL || (quote_spaces && s[1] == ' '))) {
      s++;
      if (*s == '\0') break;
    }
    COPY_CHAR_P(t, s, send);
...
static WORD_LIST *list_dequote_escapes(list) WORD_LIST *list;
{
  register WORD_LIST *w;
  char *t;
...
static char *make_quoted_char(c) int c;
{
  char *temp;
...
char *quote_string(string) char *string;
{
  register char *t;
  size_t slen;
  char *result, *send;
...
char *dequote_string(string) char *string;
{
  register char *s, *t;
  size_t slen;
  char *result, *send;
  ...
  if (strchr(string, CTLESC) == NULL) return (strcpy(result, string));
...
static WORD_LIST *quote_list(list) WORD_LIST *list;
{
  register WORD_LIST *w;
  char *t;
...
WORD_LIST *dequote_list(list) WORD_LIST *list;
{
  register char *s;
  ...
  for (tlist = list; tlist; tlist = tlist->next) {
    ...
    if (QUOTED_NULL(tlist->word->word)) tlist->word->flags &= ~W_HASQUOTEDNULL;
...
/* Remove CTLESC protecting a CTLESC or CTLNUL in place.  Return the passed
   string. */
char *remove_quoted_escapes(string) char *string;
{
  char *t;
...
char *remove_quoted_nulls(string) char *string;
{
  register size_t slen;
  register int i, j, prev_i;
  DECLARE_MBSTATE;
...
static char *remove_upattern(param, pattern, op) char *param, *pattern;
int op;
{
  register int len;
  register char *end;
...
#if defined(HANDLE_MULTIBYTE)
/* Returns its first argument if nothing matched; new memory otherwise */
static wchar_t *remove_wpattern(wparam, wstrlen, wpattern, op) wchar_t *wparam;
size_t wstrlen;
wchar_t *wpattern;
int op;
{
...
static char *remove_pattern(param, pattern, op) char *param, *pattern;
int op;
{
  char *xret;
...
static int match_upattern(string, pat, mtype, sp, ep) char *string, *pat;
int mtype;
char **sp, **ep;
{
  int c, len, mlen;
  ...
  if (npat != pat) free(npat);
  if (c == FNM_NOMATCH) return (0);
  ...
  switch (mtype) {
    ...
    case MATCH_BEG:
      if (match_pattern_char(pat, string, FNMATCH_IGNCASE) == 0) return (0);
...
/* Match WPAT anywhere in WSTRING and return the match boundaries.
   ...
   character version. */
static int match_wpattern(wstring, indices, wstrlen, wpat, mtype, sp,
                          ep) wchar_t *wstring;
char **indices;
size_t wstrlen;
wchar_t *wpat;
...
char **sp, **ep;
{
  ...
  simple = (wpat[0] != L'\\' && wpat[0] != L'*' && wpat[0] != L'?' &&
            wpat[0] != L'[');
#if defined(EXTENDED_GLOB)
  if (extended_glob)
    simple &= (wpat[1] != L'(' ||
  ...
  if (wpat[0] != L'*' ||
    ...
    if (*wp1 != L'*' || (*wp1 == '*' && wp1[1] == WLPAREN && extended_glob))
      *wp++ = L'*';
    while (*wp1 != L'\0') *wp++ = *wp1++;
    if (wp1[-1] != L'*' || wp1[-2] == L'\\') *wp++ = L'*';
    *wp = '\0';
  } else
    nwpat = wpat;
  len = wcsmatch(nwpat, wstring, FNMATCH_EXTFLAG | FNMATCH_IGNCASE);
  if (nwpat != wpat) free(nwpat);
  if (len == FNM_NOMATCH) return (0);
  ...
  switch (mtype) {
        ...
        if (n2) {
          ...
          for (; n1 >= n; n1--) {
            ...
            if (wcsmatch(wpat, wstring + n,
                         FNMATCH_EXTFLAG | FNMATCH_IGNCASE) == 0) {
              wstring[n1] = wc;
              *sp = indices[n];
              *ep = indices[n1];
              return 1;
    ...
    case MATCH_BEG:
      if (match_pattern_wchar(wpat, wstring, FNMATCH_IGNCASE) == 0) return (0);
...
static int match_pattern(string, pat, mtype, sp, ep) char *string, *pat;
int mtype;
char **sp, **ep;
{
#if defined(HANDLE_MULTIBYTE)
  ...
  if (string == 0 || pat == 0 || *pat == 0) return (0);
  ...
  if (MB_CUR_MAX > 1) {
    ...
    if (n == (size_t)-1) return (match_upattern(string, pat, mtype, sp, ep));
...
static int getpatspec(c, value) int c;
char *value;
{
  if (c == '#')
...
static char *getpattern(value, quoted, expandpat) char *value;
...
/* There is a problem here:  how to handle single or double quotes in the
  ...
  /* expand_string_for_rhs () leaves WORD quoted and does not perform
     word splitting. */
  l = *value ? expand_string_for_rhs(
                   value,
                   (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) ? Q_PATQUOTE
                                                                  : quoted,
                   (int *)NULL, (int *)NULL)
             : (WORD_LIST *)0;
  pat = string_list(l);
...
static char *list_remove_pattern(list, pattern, patspec, itype,
                                 quoted) WORD_LIST *list;
char *pattern;
...
static char *parameter_list_remove_pattern(itype, pattern, patspec,
                                           quoted) int itype;
char *pattern;
int patspec, quoted;
{
...
#if defined(ARRAY_VARS)
static char *array_remove_pattern(var, pattern, patspec, varname,
                                  quoted) SHELL_VAR *var;
char *pattern;
int patspec;
...
int quoted;
{
  ...
  /* XXX */
  if (v && invisible_p(v)) return ((char *)NULL);
...
static char *parameter_brace_remove_pattern(varname, value, ind, patstr, rtype,
                                            quoted, flags) char *varname,
    *value;
int ind;
char *patstr;
int rtype, quoted, flags;
{
  ...
  if (value == 0) return ((char *)NULL);

  this_command_name = varname;

  vtype = get_var_and_type(varname, value, ind, quoted, flags, &v, &val);
  if (vtype == -1) return ((char *)NULL);

  starsub = vtype & VT_STARSUB;
  vtype &= ~VT_STARSUB;

  patspec = getpatspec(rtype, patstr);
  if (patspec == RP_LONG_LEFT || patspec == RP_LONG_RIGHT) patstr++;
  ...
  switch (vtype) {
    ...
    case VT_POSPARMS:
      temp1 =
          parameter_list_remove_pattern(varname[0], pattern, patspec, quoted);
      if (temp1 && ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) == 0)) {
        val = quote_escapes(temp1);
        free(temp1);
        temp1 = val;
...
static char *string_var_assignment(v, s) SHELL_VAR *v;
char *s;
{
  ...
  ret =
      (char *)xmalloc(i + strlen(val) + strlen(v->name) + 16 + MAX_ATTRIBUTES);
  if (i > 0)
...
#if defined(ARRAY_VARS)
static char *array_var_assignment(v, itype, quoted) SHELL_VAR *v;
int itype, quoted;
{
  char *ret, *val, flags[MAX_ATTRIBUTES];
  int i;

  if (v == 0) return (char *)NULL;
  ...
  if (val == 0) {
    ...
    ret = (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ? quote_string(val)
                                                         : quote_escapes(val);
    free(val);
    val = ret;
  }
...
static char *pos_params_assignment(list, itype, quoted) WORD_LIST *list;
int itype;
int quoted;
{
  char *temp, *ret;
...
static char *string_transform(xc, v, s) int xc;
SHELL_VAR *v;
char *s;
{
...
static char *list_transform(xc, v, list, itype, quoted) int xc;
SHELL_VAR *v;
WORD_LIST *list;
int itype, quoted;
{
...
static char *parameter_list_transform(xc, itype, quoted) int xc;
int itype;
int quoted;
{
  char *ret;
  ...
  list = list_rest_of_args();
  if (list == 0) return ((char *)NULL);
  if (xc == 'A') return (pos_params_assignment(list, itype, quoted));
...
#if defined(ARRAY_VARS)
static char *array_transform(xc, var, varname, quoted) int xc;
SHELL_VAR *var;
char *varname; /* so we can figure out how it's indexed */
int quoted;
{
  ...
  if (xc == 'A') return (array_var_assignment(v, itype, quoted));
...
static char *parameter_brace_transform(varname, value, ind, xform, rtype,
                                       quoted, flags) char *varname,
    *value;
int ind;
char *xform;
int rtype, quoted, flags;
{
  ...
  xc = xform[0];
  if (value == 0 && xc != 'A' && xc != 'a') return ((char *)NULL);
...
WORD_LIST *expand_word(word, quoted) WORD_DESC *word;
int quoted;
{
  WORD_LIST *result, *tresult;

  tresult =
      call_expand_word_internal(word, quoted, 0, (int *)NULL, (int *)NULL);
...
/* Expand WORD, but do not perform word splitting on the result.  This
   does parameter expansion, command substitution, arithmetic expansion,
   and quote removal. */
WORD_LIST *expand_word_unsplit(word, quoted) WORD_DESC *word;
int quoted;
{
  WORD_LIST *result;
...
WORD_LIST *expand_word_leave_quoted(word, quoted) WORD_DESC *word;
int quoted;
{
  WORD_LIST *result;
...
char *copy_fifo_list(sizep) int *sizep;
{
  if (sizep) *sizep = 0;
  return (char *)NULL;
}
...
void close_new_fifos(list, lsize) char *list;
int lsize;
{
  int i;
  ...
  for (i = lsize; i < fifo_list_size; i++) unlink_fifo(i);
}

int fifos_pending() { return nfifo; }

int num_fifos() { return nfifo; }
...
void clear_fifo_list() {
  ...
  for (i = 0; nfds && i < totfds; i++) clear_fifo(i);
...
static void add_fifo_list(fd) int fd;
{
  if (dev_fd_list == 0 || fd >= totfds) {
    ...
    if (totfds < 0 || totfds > 256) totfds = 256;
    if (fd >= totfds) totfds = fd + 2;
...
void unlink_fifo_list() {
  ...
  for (i = 0; nfds && i < totfds; i++) unlink_fifo(i);
...
int lsize;
{
  ...
  for (i = 0; i < lsize; i++)
    if (list[i] == 0 && i < totfds && dev_fd_list[i]) unlink_fifo(i);

  for (i = lsize; i < totfds; i++) unlink_fifo(i);
}
...
print_dev_fd_list() {
  ...
  for (i = 0; i < totfds; i++) {
    if (dev_fd_list[i]) fprintf(stderr, " %d", i);
  }
  fprintf(stderr, "\n");
}
#endif /* NOTDEF */

static char *make_dev_fd_filename(fd) int fd;
{
...
static char *process_substitute(string, open_for_read_in_child) char *string;
int open_for_read_in_child;
{
  ...
  if (!string || !*string || wordexp_only) return ((char *)NULL);
  ...
  if (pipeline_pgrp == 0 ||
      (subshell_environment &
       (SUBSHELL_PIPE | SUBSHELL_FORK | SUBSHELL_ASYNC)) == 0)
  ...
  if (pid == 0) {
    ...
    if (expanding_redir) flush_temporary_env();
  }
  ...
  if (fd != (open_for_read_in_child ? 0 : 1)) close(fd);
...
static char *read_comsub(fd, quoted, rflag) int fd, quoted;
int *rflag;
{
  ...
  if (istring) istring[istring_index] = '\0';

  /* If we read no output, just return now and save ourselves some
     trouble. */
  if (istring_index == 0) {
    FREE(istring);
    if (rflag) *rflag = tflag;
    return (char *)NULL;
  }
  ...
  if (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) {
    while (istring_index > 0) {
      if (istring[istring_index - 1] == '\n') {
        ...
        if (istring[istring_index - 1] == CTLESC) --istring_index;
      } else
        break;
    }
...
WORD_DESC *command_substitute(string, quoted) char *string;
int quoted;
{
...
/* Don't fork () if there is no need to.  In the case of no command to
  ...
  for (s = string; s && *s && (shellblank(*s) || *s == '\n'); s++)
    ;
  if (s == 0 || *s == 0) return ((WORD_DESC *)NULL);
#else
  ...
  if ((subshell_environment & SUBSHELL_PIPE) == 0) pipeline_pgrp = shell_pgrp;
...
static arrayind_t array_length_reference(s) char *s;
{
  int len;
  arrayind_t ind;
  char *akey;
...
static int valid_brace_expansion_word(name, var_is_special) char *name;
int var_is_special;
{
...
static int chk_atstar(name, quoted, quoted_dollar_atp,
                      contains_dollar_at) char *name;
int quoted;
int *quoted_dollar_atp, *contains_dollar_at;
{
  ...
  if (name == 0) {
    if (quoted_dollar_atp) *quoted_dollar_atp = 0;
    if (contains_dollar_at) *contains_dollar_at = 0;
    return 0;
  }
  ...
  if (name[0] == '@' && name[1] == 0) {
    if ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && quoted_dollar_atp)
      *quoted_dollar_atp = 1;
    if (contains_dollar_at) *contains_dollar_at = 1;
    return 1;
...
static WORD_DESC *parameter_brace_expand_word(name, var_is_special, quoted,
                                              pflags, indp) char *name;
...
static char *parameter_brace_find_indir(name, var_is_special, quoted,
                                        find_nameref) char *name;
int var_is_special, quoted, find_nameref;
{
  ...
  if (find_nameref && var_is_special == 0 &&
  ...
  if (t) {
    temp = (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ? dequote_string(t)
                                                          : dequote_escapes(t);
    free(t);
    t = temp;
  }
...
static WORD_DESC *parameter_brace_expand_indir(name, var_is_special, quoted,
                                               quoted_dollar_atp,
                                               contains_dollar_at) char *name;
...
static WORD_DESC *parameter_brace_expand_rhs(name, value, c, quoted, pflags,
                                             qdollaratp, hasdollarat) char *name
    ,
    *value;
int c, quoted, pflags, *qdollaratp, *hasdollarat;
{
  ...
  l = *temp ? expand_string_for_rhs(temp, quoted, &l_hasdollat, (int *)NULL)
            : (WORD_LIST *)0;
  if (hasdollarat) *hasdollarat = l_hasdollat || (l && l->next);
  if (temp != value) free(temp);
  if (l) {
    /* If l->next is not null, we know that TEMP contained "$@", since that
       is the only expansion that creates more than one word. */
    if (qdollaratp && ((l_hasdollat && quoted) || l->next)) {
      /*itrace("parameter_brace_expand_rhs: %s:%s: l != NULL, set *qdollaratp",
       * name, value);*/
      *qdollaratp = 1;
    }
    ...
    if (l->next && ifs_is_null) {
      ...
      temp = (l_hasdollat || l->next) ? string_list_dollar_star(l)
                                      : string_list(l);
    ...
    if (l->next == 0 && (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) &&
      ...
      if (qdollaratp && l_hasdollat) *qdollaratp = 0;
    }
    dispose_words(l);
    ...
    w->flags |= W_HASQUOTEDNULL;
    /*itrace("parameter_brace_expand_rhs (%s:%s): returning quoted null", name,
     * value);*/
  } else
    temp = (char *)NULL;
  ...
  /* bash-4.4/5.0 */
  vname = name;
  if (*name == '!' && (legal_variable_starter((unsigned char)name[1]) ||
...
/* Deal with the right hand side of a ${name:?value} expansion in the case
   ...
   printed. */
static void parameter_brace_expand_error(name, value) char *name, *value;
{
  WORD_LIST *l;
  char *temp;
...
/* Return 1 if NAME is something for which parameter_brace_expand_length is
   OK to do. */
static int valid_length_expression(name) char *name;
{
  return (name[1] == '\0' || /* ${#} */
...
static intmax_t parameter_brace_expand_length(name) char *name;
{
  char *t, *newname;
  ...
  if (name[1] == '\0') /* ${#} */
    ...
    if (legal_number(name + 1, &arg_index)) /* ${#1} */
      ...
      if (t == 0 && unbound_vars_is_error) return INTMAX_MIN;
      number = MB_STRLEN(t);
      FREE(t);
    }
#if defined(ARRAY_VARS)
...
static char *skiparith(substr, delim) char *substr;
int delim;
{
  int i;
  char delims[2];
...
static int verify_substring_values(v, value, substr, vtype, e1p,
                                   e2p) SHELL_VAR *v;
char *value, *substr;
int vtype;
intmax_t *e1p, *e2p;
  ...
  *e1p = evalexp(temp1, &expok);
  ...
  switch (vtype) {
    ...
    case VT_POSPARMS:
      ...
      if (*e1p == 0) len++; /* add one arg if counting from $0 */
      break;
#if defined(ARRAY_VARS)
    case VT_ARRAYVAR:
      ...
      if (assoc_p(v)) {
        ...
        a = (ARRAY *)value;
        len =
            array_max_index(a) + (*e1p < 0); /* arrays index from 0 to n - 1 */
      }
      break;
#endif
  }
...
static int get_var_and_type(varname, value, ind, quoted, flags, varp,
                            valp) char *varname,
    *value;
arrayind_t ind;
int quoted, flags;
...
char **valp;
{
  ...
  want_indir =
      *varname == '!' && (legal_variable_starter((unsigned char)varname[1]) ||
  ...
  if (vtype == VT_POSPARMS && vname[0] == '*') vtype |= VT_STARSUB;
  *varp = (SHELL_VAR *)NULL;
...
static char *mb_substring(string, s, e) char *string;
int s, e;
{
  char *tt;
  ...
  i = s;
  while (string[start] && i--) ADVANCE_CHAR(string, slen, start);
  stop = start;
  i = e - s;
  while (string[stop] && i--) ADVANCE_CHAR(string, slen, stop);
...
static char *parameter_brace_substring(varname, value, ind, substr, quoted,
                                       flags) char *varname,
    *value;
int ind;
char *substr;
...
char *pat_subst(string, pat, rep, mflags) char *string, *pat, *rep;
int mflags;
{
  char *ret, *s, *e, *str, *rstr, *mstr;
  int rsize, rptr, l, replen, mtype, rxpand, rslen, mlen;

  if (string == 0) return (savestring(""));
  ...
  for (replen = STRLEN(rep), rptr = 0, str = string; *str;) {
    ...
    if (rstr != rep) free(rstr);

    if (((mflags & MATCH_GLOBREP) == 0) || mtype != MATCH_ANY) break;
...
static char *pos_params_pat_subst(string, pat, rep, mflags) char *string, *pat,
    *rep;
int mflags;
{
...
static char *parameter_brace_patsub(varname, value, ind, patsub, quoted, pflags,
                                    flags) char *varname,
    *value;
int ind;
char *patsub;
int quoted, pflags, flags;
{
  ...
  if (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) mflags |= MATCH_QUOTED;

  if (starsub) mflags |= MATCH_STARSUB;

  if (pflags & PF_ASSIGNRHS) mflags |= MATCH_ASSIGNRHS;
...
static char *pos_params_modcase(string, pat, modop, mflags) char *string, *pat;
int modop;
int mflags;
{
  WORD_LIST *save, *params;
...
/* Perform case modification on VALUE, which is the expansion of
   ...
   currently in effect. */
static char *parameter_brace_casemod(varname, value, ind, modspec, patspec,
                                     quoted, flags) char *varname,
    *value;
int ind, modspec;
char *patspec;
...
static WORD_DESC *parameter_brace_expand(string, indexp, quoted, pflags,
                                         quoted_dollar_atp,
                                         contains_dollar_at) char *string;
...
/* To enable case-toggling expansions using the `~' operator character
  ...
  want_indir =
      *name == '!' && (legal_variable_starter((unsigned char)name[1]) ||
                       DIGIT(name[1]) || VALID_INDIR_PARAM(name[1]));

  /* Determine the value of this variable. */

  /* Check for special variables, directly referenced. */
  if (SPECIAL_VAR(name, want_indir)) var_is_special++;
  ...
  if (want_indir && string[sindex - 1] == RBRACE && string[sindex - 2] == ']' &&
  ...
  if (want_indir) {
    ...
    if (tdesc && tdesc->flags) tdesc->flags &= ~W_ARRAYIND;
  } else
    tdesc = parameter_brace_expand_word(
        name, var_is_special, quoted,
  ...
  if (valid_array_reference(name, 0)) {
    ...
    if (pflags & PF_ASSIGNRHS) qflags |= Q_DOUBLE_QUOTES;
    chk_atstar(name, qflags, quoted_dollar_atp, contains_dollar_at);
    /* We duplicate a little code here */
    t = mbschr(name, '[');
    if (t && ALL_ELEMENT_SUB(t[1]) && t[2] == ']') all_element_arrayref = 1;
  }
#endif
  ...
  *indexp = sindex;
  ...
  if (want_substring) {
    temp1 = parameter_brace_substring(name, temp, ind, value, quoted,
    ...
    temp1 = parameter_brace_casemod(name, temp, ind, modspec, value, quoted,
                                    (tflag & W_ARRAYIND) ? AV_USEIND : 0);
    FREE(name);
    FREE(value);
    FREE(temp);
  ...
  switch (c) {
    ...
    case '@':
      temp1 = parameter_brace_transform(name, temp, ind, value, c, quoted,
                                        (tflag & W_ARRAYIND) ? AV_USEIND : 0);
      free(temp);
      free(value);
      free(name);
    ...
    case '%': /* ${param%[%]pattern} */
      ...
      temp1 =
          parameter_brace_remove_pattern(name, temp, ind, value, c, quoted,
                                         (tflag & W_ARRAYIND) ? AV_USEIND : 0);
      free(temp);
      free(value);
      free(name);
      ...
      if (var_is_set && var_is_null == 0) {
        ...
        if (c == '=' && var_is_special) {
          ...
          if (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES))
            quoted |= Q_DOLBRACE;
          ret =
              parameter_brace_expand_rhs(name, value, c, quoted, pflags,
                                         quoted_dollar_atp, contains_dollar_at);
          /* XXX - fix up later, esp. noting presence of
...
static WORD_DESC *param_expand(string, sindex, quoted, expanded_something,
                               ...
                               had_quoted_null_p, pflags) char *string;
      ...
      if (list == 0 && unbound_vars_is_error && (pflags & PF_IGNUNBOUND) == 0)
	{
      ...
      if ((quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && list == 0)
        ...
        if (temp) {
          ...
          if (*temp == 0) tflag |= W_HASQUOTEDNULL;
          if (temp != temp1) free(temp);
          temp = temp1;
        }
      } else {
   ...
   identical to $@ */
      ...
      if (list == 0 && unbound_vars_is_error && (pflags & PF_IGNUNBOUND) == 0)
	{
      ...
      /* XXX - what to do when in a context where word splitting is not
         ...
         here. */
      temp = string_list_dollar_at(
          list, (pflags & PF_ASSIGNRHS) ? (quoted | Q_DOUBLE_QUOTES) : quoted,
          0);
    ...
    case LBRACE:
      ...
      if (tdesc && tdesc->word && (tdesc->flags & W_HASQUOTEDNULL) &&
          QUOTED_NULL(temp)) {
        if (had_quoted_null_p) *had_quoted_null_p = 1;
    ...
    comsub:
      if (pflags & PF_NOCOMSUB)
        ...
        if (tdesc) dispose_word_desc(tdesc);
      }
      FREE(temp);
      temp = temp1;
      break;
    ...
    default:
      ...
      for (t_index = zindex; (c = string[zindex]) && legal_variable_char(c);
           zindex++)
        ;
      temp1 = (zindex > t_index) ? substring(string, t_index, zindex)
                                 : (char *)NULL;
      ...
      if (var && invisible_p(var) == 0 && var_isset(var)) {
        ...
        if (temp && *temp && valid_array_reference(temp, 0)) {
          tdesc = parameter_brace_expand_word(
              temp, SPECIAL_VAR(temp, 0), quoted, pflags, (arrayind_t *)NULL);
          if (tdesc == &expand_wdesc_error || tdesc == &expand_wdesc_fatal)
            return (tdesc);
          ret = tdesc;
          goto return0;
...
static WORD_LIST *expand_word_internal(word, quoted, isexp, contains_dollar_at,
                                       expanded_something) WORD_DESC *word;
int quoted, isexp;
...
int *expanded_something;
{
  ...
  if (STREQ(word->word, "\"$@\"") &&
      ...
      {
    if (contains_dollar_at) *contains_dollar_at = 1;
    if (expanded_something) *expanded_something = 1;
  ...
  string = word->word;
  if (string == 0) goto finished_with_string;
    ...
    switch (c) {
      ...
      case '=':
        ...
        if (word->flags & (W_ASSIGNRHS | W_NOTILDE)) {
          if (isexp == 0 && (word->flags & (W_NOSPLIT | W_NOSPLIT2)) == 0 &&
              isifs(c))
      ...
      case '$':
        ...
        if (word->flags & W_NOSPLIT2) pflags |= PF_NOSPLIT2;
        if (word->flags & W_ASSIGNRHS) pflags |= PF_ASSIGNRHS;
        ...
        if (contains_dollar_at && has_dollar_at) *contains_dollar_at = 1;

        if (tword && (tword->flags & W_HASQUOTEDNULL)) had_quoted_null = 1;
      ...
      case '`': /* Backquoted command substitution. */
        ...
        if (word->flags & W_NOCOMSUB)
          ...
          if (tword) dispose_word_desc(tword);
        }
        FREE(temp);
        temp = temp1;
        goto dollar_add_string;
      ...
      case '\'':
        if ((quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) goto add_character;
        ...
        if (temp == 0 && (quoted_state == PARTIALLY_QUOTED)) continue;
...
finished_with_string:
  ...
  if (*istring == '\0') {
    ...
    tword = make_bare_word(istring);
    if (word->flags & W_ASSIGNMENT) tword->flags |= W_ASSIGNMENT; /* XXX */
    if (word->flags & W_COMPASSIGN) tword->flags |= W_COMPASSIGN; /* XXX */
    if (word->flags & W_NOGLOB) tword->flags |= W_NOGLOB;         /* XXX */
    if (word->flags & W_NOBRACE) tword->flags |= W_NOBRACE;       /* XXX */
    if (word->flags & W_NOEXPAND) tword->flags |= W_NOEXPAND;     /* XXX */
    if (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) tword->flags |= W_QUOTED;
    ...
    /* If we have $@ (has_dollar_at != 0) and we are in a context where we
      ...
      if (tstring != istring) free(tstring);
      goto set_word_flags;
    }
   ...
   compatibility with other shells, are not backwards compatible. */
    ...
    set_word_flags:
      if ((quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT)) ||
        ...
        tword->flags |= W_QUOTED;
      if (word->flags & W_ASSIGNMENT) tword->flags |= W_ASSIGNMENT;
      if (word->flags & W_COMPASSIGN) tword->flags |= W_COMPASSIGN;
      if (word->flags & W_NOGLOB) tword->flags |= W_NOGLOB;
      if (word->flags & W_NOBRACE) tword->flags |= W_NOBRACE;
      if (word->flags & W_NOEXPAND) tword->flags |= W_NOEXPAND;
      if (had_quoted_null && QUOTED_NULL(istring))
        tword->flags |= W_HASQUOTEDNULL; /* XXX */
      list = make_word_list(tword, (WORD_LIST *)NULL);
    }
...
char *string_quote_removal(string, quoted) char *string;
int quoted;
{
  size_t slen;
...
char *getifs() { return ifs_value; }

/* This splits a single word into a WORD LIST on $IFS, but only if the word
   is not quoted.  list_string () performs quote removal for us, even if we
   don't do any splitting. */
WORD_LIST *word_split(w, ifs_chars) WORD_DESC *w;
char *ifs_chars;
{
  WORD_LIST *result;
...
static WORD_LIST *word_list_split(list) WORD_LIST *list;
{
  WORD_LIST *result, *t, *tresult, *e;
...
static void exp_jump_to_top_level(v) int v;
{
  ...
  if (parse_and_execute_level == 0) top_level_cleanup(); /* from sig.c */
...
static WORD_LIST *separate_out_assignments(tlist) WORD_LIST *tlist;
{
  register WORD_LIST *vp, *lp;

  if (tlist == 0) return ((WORD_LIST *)NULL);
  ...
  */
  ...
  if (!tlist) /* All the words in tlist were assignment statements */
    return ((WORD_LIST *)NULL);
...
WORD_LIST *expand_words(list) WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_ALL)); }

/* Same as expand_words (), but doesn't hack variable or environment
   variables. */
WORD_LIST *expand_words_no_vars(list) WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_NOVARS)); }

WORD_LIST *expand_words_shellexp(list) WORD_LIST *list;
{ return (expand_word_list_internal(list, WEXP_SHELLEXP)); }

static WORD_LIST *glob_expand_word_list(tlist, eflags) WORD_LIST *tlist;
int eflags;
{
  ...
  if (disposables) dispose_words(disposables);
...
static WORD_LIST *brace_expand_word_list(tlist, eflags) WORD_LIST *tlist;
int eflags;
{
...
static int make_internal_declare(word, option, cmd) char *word;
char *option;
char *cmd;
{
  int t, r;
...
static WORD_LIST *shell_expand_word_list(tlist, eflags) WORD_LIST *tlist;
int eflags;
{
  ...
  for (wcmd = tlist; wcmd; wcmd = wcmd->next)
    if (wcmd->word->flags & W_ASSNBLTIN) break;
  ...
  for (orig_list = tlist; tlist; tlist = next) {
    ...
    if ((tlist->word->flags & (W_COMPASSIGN | W_ASSIGNARG)) ==
      ...
      /*	  if (opti > 0) */
      {
        ...
        for (l = orig_list->next; l != tlist; l = l->next) {
          if (l->word->word[0] != '-') break; /* non-option argument */
        ...
        for (oind = 0; oind < sizeof(omap); oind++)
          if (omap[oind]) opts[opti++] = oind;
      }

      opts[opti] = '\0';
      if (opti > 0) {
        t = make_internal_declare(tlist->word->word, opts,
                                  wcmd ? wcmd->word->word : (char *)0);
    ...
    expanded = expand_word_internal(tlist->word, 0, 0, &has_dollar_at,
                                    &expanded_something);
  ...
  if (orig_list) dispose_words(orig_list);

  if (new_list) new_list = REVERSE_LIST(new_list, WORD_LIST *);
...
static WORD_LIST *expand_word_list_internal(list, eflags) WORD_LIST *list;
int eflags;
{
  ...
  if (list == 0) return ((WORD_LIST *)NULL);
  ...
  if (eflags & WEXP_VARASSIGN) {
    ...
    if (new_list == 0) {
      if (subst_assign_varlist) {
        ...
        for (temp_list = subst_assign_varlist; temp_list;
