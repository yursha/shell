...
int find_string_in_alist(string, alist, flags) char *string;
...
char *find_token_in_alist(token, alist, flags) int token;
STRING_INT_ALIST *alist;
int flags;
{
  register int i;
  ...
  for (i = 0; alist[i].word; i++) {
    if (alist[i].token == token) return (savestring(alist[i].word));
  }
  return ((char *)NULL);
}

int find_index_in_alist(string, alist, flags) char *string;
STRING_INT_ALIST *alist;
int flags;
{
  register int i;
...
char *strsub(string, pat, rep, global) char *string, *pat, *rep;
int global;
{
...
char *strcreplace(string, c, text, do_glob) char *string;
int c;
const char *text;
int do_glob;
{
