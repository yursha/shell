...
static COMMAND *make_for_or_select(enum command_type, WORD_DESC *, WORD_LIST *,
                                   COMMAND *, int);
...
WORD_DESC *make_word_flags(w, string) WORD_DESC *w;
const char *string;
{
  register int i;
  size_t slen;
...
WORD_DESC *make_word_from_token(token) int token;
{
  char tokenizer[2];
...
WORD_LIST *make_word_list(word, wlink) WORD_DESC *word;
WORD_LIST *wlink;
{
  WORD_LIST *temp;
...
COMMAND *command_connect(com1, com2, connector) COMMAND *com1, *com2;
int connector;
{
  CONNECTION *temp;
...
COMMAND *make_for_command(name, map_list, action, lineno) WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
int lineno;
{ return (make_for_or_select(cm_for, name, map_list, action, lineno)); }

COMMAND *make_select_command(name, map_list, action, lineno) WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
int lineno;
{
...
#if defined(ARITH_FOR_COMMAND)
static WORD_LIST *make_arith_for_expr(s) char *s;
{
  WORD_LIST *result;
  WORD_DESC *wd;

  if (s == 0 || *s == '\0') return ((WORD_LIST *)NULL);
  wd = make_word(s);
...
/* Note that this function calls dispose_words on EXPRS, since it doesn't
   ...
   any arguments. */
COMMAND *make_arith_for_command(exprs, action, lineno) WORD_LIST *exprs;
COMMAND *action;
int lineno;
{
#if defined(ARITH_FOR_COMMAND)
...
COMMAND *make_group_command(command) COMMAND *command;
{
  GROUP_COM *temp;
...
COMMAND *make_case_command(word, clauses, lineno) WORD_DESC *word;
...
PATTERN_LIST *make_pattern_list(patterns, action) WORD_LIST *patterns;
COMMAND *action;
{
  PATTERN_LIST *temp;
...
COMMAND *make_if_command(test, true_case, false_case) COMMAND *test, *true_case,
    *false_case;
{
  IF_COM *temp;
...
COMMAND *make_while_command(test, action) COMMAND *test, *action;
{ return (make_until_or_while(cm_while, test, action)); }

COMMAND *make_until_command(test, action) COMMAND *test, *action;
{ return (make_until_or_while(cm_until, test, action)); }

COMMAND *make_arith_command(exp) WORD_LIST *exp;
{
...
struct cond_com *make_cond_node(type, op, left, right) int type;
WORD_DESC *op;
...
COMMAND *make_cond_command(cond_node) COND_COM *cond_node;
{
#if defined(COND_COMMAND)
  COMMAND *command;
...
COMMAND *make_simple_command(element, command) ELEMENT element;
COMMAND *command;
{
...
int lineno;
{
  ...
  while (full_line = read_secondary_line(delim_unquoted)) {
    ...
    if (echo_input_at_read) fprintf(stderr, "%s", line);
...
REDIRECT *make_redirection(source, instruction, dest_and_filename,
                           flags) REDIRECTEE source;
    ...
    case r_duplicating_output_word: /* 1>&$foo */
      ...
      if (w->word[wlen] == '-') /* Yuck */
        ...
        if (all_digits(w->word) && legal_number(w->word, &lfd) &&
            lfd == (int)lfd) {
          dispose_word(w);
          temp->instruction = (instruction == r_duplicating_input_word)
                                  ? r_move_input
                                  : r_move_output;
...
COMMAND *make_function_def(name, command, lineno, lstart) WORD_DESC *name;
COMMAND *command;
int lineno, lstart;
{
  FUNCTION_DEF *temp;
...
COMMAND *make_subshell_command(command) COMMAND *command;
{
  SUBSHELL_COM *temp;
...
COMMAND *make_coproc_command(name, command) char *name;
COMMAND *command;
{
  COPROC_COM *temp;
...
COMMAND *clean_simple_command(command) COMMAND *command;
{
...
/* The Yacc grammar productions have a problem, in that they take a
   ...
   `;' bind `tighter' than `&'. */
COMMAND *connect_async_list(command, command2, connector) COMMAND *command,
    *command2;
int connector;
{
  COMMAND *t, *t1, *t2;
