...
void redirection_error(temp, error) REDIRECT *temp;
int error;
{
...
int do_redirections(list, flags) REDIRECT *list;
int flags;
{
  int error;
  REDIRECT *temp;
  ...
  if (flags & RX_UNDOABLE) {
    ...
    if (exec_redirection_undo_list) dispose_exec_redirects();
  }
...
static int expandable_redirection_filename(redirect) REDIRECT *redirect;
{
...
char *redirection_expand(word) WORD_DESC *word;
{
  char *result;
  ...
  w = copy_word(word);
  if (posixly_correct) w->flags |= W_NOSPLIT;
  ...
  if (tlist2 == 0 || tlist2->next) {
    ...
    if (tlist2) dispose_words(tlist2);
    return ((char *)NULL);
  }
...
static int write_here_string(fd, redirectee) int fd;
...
static int write_here_document(fd, redirectee) int fd;
WORD_DESC *redirectee;
{
  char *document;
...
static int here_document_to_fd(redirectee, ri) WORD_DESC *redirectee;
...
static int redir_special_open(spec, filename, flags, mode, ri) int spec;
char *filename;
int flags, mode;
enum r_instruction ri;
{
...
static int noclobber_open(filename, flags, mode, ri) char *filename;
int flags, mode;
enum r_instruction ri;
{
  int r, fd;
  ...
  if (r == 0 && (S_ISREG(finfo.st_mode))) return (NOCLOBBER_REDIRECT);
  ...
  if (fd < 0) return (errno == EEXIST ? NOCLOBBER_REDIRECT : fd);
...
static int redir_open(filename, flags, mode, ri) char *filename;
int flags, mode;
enum r_instruction ri;
{
  int fd, r, e;
  ...
  if (r >= 0) return (redir_special_open(r, filename, flags, mode, ri));
  ...
  if (noclobber && CLOBBERING_REDIRECT(ri)) {
    ...
    if (fd == NOCLOBBER_REDIRECT) return (NOCLOBBER_REDIRECT);
  } else {
    do {
      fd = open(filename, flags, mode);
      e = errno;
...
static int undoablefd(fd) int fd;
{
  int clexec;

  clexec = fcntl(fd, F_GETFD, 0);
  if (clexec == -1 || (fd >= SHELL_FD_BASE && clexec == 1)) return 0;
  return 1;
}
...
/* Do the specific redirection requested.  Returns errno or one of the
   ...
   close-on-exec.  */
static int do_redirection_internal(redirect, flags) REDIRECT *redirect;
int flags;
{
  WORD_DESC *redirectee;
  ...
  if (redirect->flags & RX_INTERNAL) flags |= RX_INTERNAL;
  ...
  if (TRANSLATE_REDIRECT(ri)) {
    ...
    if (new_redirect->instruction == r_err_and_out) {
      ...
      alloca_hack =
          (char *)alloca(1 + strlen(new_redirect->redirectee.filename->word));
  ...
  switch (ri) {
    ...
    case r_output_force:
      ...
      if (fd == NOCLOBBER_REDIRECT) return (fd);
      ...
      if (ri == r_err_and_out || ri == r_append_err_and_out) {
        if (flags & RX_ACTIVE) {
          if (flags & RX_UNDOABLE) add_undo_redirect(2, ri, -1);
          if (dup2(1, 2) < 0) return (errno);
        }
      }
      break;
...
/* if ((already_set || set_unconditionally) && (ok_to_set))
	  ...
	  if (((fcntl (redir_fd, F_GETFD, 0) == 1) || redir_fd < 2 || (flags & RX_CLEXEC)) &&
        ...
        if (((fcntl(redir_fd, F_GETFD, 0) == 1) ||
             (redir_fd < 2 && (flags & RX_INTERNAL)) || (flags & RX_CLEXEC)) &&
            (redirector > 2))
#endif
        SET_CLOSE_ON_EXEC(redirector);
    ...
    case r_close_this:
      if (flags & RX_ACTIVE) {
        if (redirect->rflags & REDIR_VARASSIGN) {
          ...
          if (redirector < 0) return AMBIGUOUS_REDIRECT;
        }
...
static int add_undo_redirect(fd, ri, fdbase) int fd;
enum r_instruction ri;
int fdbase;
{
  ...
  savefd_flag = 0;
  new_fd =
      fcntl(fd, F_DUPFD, (fdbase < SHELL_FD_BASE) ? SHELL_FD_BASE : fdbase + 1);
  if (new_fd < 0) new_fd = fcntl(fd, F_DUPFD, SHELL_FD_BASE);
  if (new_fd < 0) {
  ...
  if (savefd_flag) new_redirect->flags |= RX_SAVEFD;
...
static int add_undo_close_redirect(fd) int fd;
{
  REDIRECT *closer;
  REDIRECTEE sd;
...
/* Return non-zero if any of the redirections in REDIRS alter the standard
   input. */
int stdin_redirects(redirs) REDIRECT *redirs;
{
  REDIRECT *rp;
  int n;
...
static int redir_varassign(redir, fd) REDIRECT *redir;
int fd;
{
  WORD_DESC *w;
  SHELL_VAR *v;
  ...
  if (v == 0 || readonly_p(v) || noassign_p(v)) return BADVAR_REDIRECT;
...
static int redir_varvalue(redir) REDIRECT *redir;
{
  SHELL_VAR *v;
  char *val, *w;
  intmax_t vmax;
...
/* XXX - handle set -u here? */
  ...
  if (legal_number(val, &vmax) < 0) return -1;
