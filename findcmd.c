...
int file_status(name) const char *name;
{
  ...
  if (stat(name, &finfo) < 0) return (0);

  /* If the file is a directory, then it is not "executable" in the
     sense of the shell. */
  if (S_ISDIR(finfo.st_mode)) return (FS_EXISTS | FS_DIRECTORY);
  ...
  if (exec_name_should_ignore(name) == 0 && eaccess(name, X_OK) == 0)
    r |= FS_EXECABLE;
  if (eaccess(name, R_OK) == 0) r |= FS_READABLE;
  ...
  if (exec_name_should_ignore(name) == 0 && access(name, X_OK) == 0)
    r |= FS_EXECABLE;
  if (access(name, R_OK) == 0) r |= FS_READABLE;
  ...
  if (current_user.euid == finfo.st_uid) {
    if (exec_name_should_ignore(name) == 0 && (finfo.st_mode & S_IXUSR))
      r |= FS_EXECABLE;
    if (finfo.st_mode & S_IRUSR) r |= FS_READABLE;
  }
  ...
  /* If we are in the owning group, the group permissions apply. */
    ...
    if (exec_name_should_ignore(name) == 0 && (finfo.st_mode & S_IXGRP))
      r |= FS_EXECABLE;
    if (finfo.st_mode & S_IRGRP) r |= FS_READABLE;
  }
  ...
  /* Else we check whether `others' have permission to execute the file */
    ...
    if (exec_name_should_ignore(name) == 0 && finfo.st_mode & S_IXOTH)
      r |= FS_EXECABLE;
    if (finfo.st_mode & S_IROTH) r |= FS_READABLE;
  }
...
int flags;
{
  ...
  if (path_list == 0 || *path_list == '\0') return (savestring(name));
...
int flags;
{
  ...
  free(dotexe);
  if (res == 0) res = _find_user_command_internal(name, flags);
  return res;
#else
...
static char *get_next_path_element(path_list,
                                   path_index_pointer) char *path_list;
...
int flags;
{
  ...
  if (hashed_file)
    ...
    command = find_user_command_in_path(pathname, pathlist,
    ...
    if (flags & CMDSRCH_STDPATH) free(pathlist);
  }
...
int flags, state;
{
  ...
  if (state == 0) {
    ...
    for (i = 0; i < match_list_size; i++) match_list[i] = 0;
    ...
    while (path_list && path_list[path_index]) {
      ...
      if (path_element == 0) break;

      match =
          find_in_path_element(name, path_element, flags, name_len, &dotinfo);
...
int flags;
{
  ...
  if ((st & FS_EXISTS) == 0) return ((char *)NULL);
...
static char *find_in_path_element(name, path, flags, name_len, dotinfop) const
    char *name;
char *path;
int flags, name_len;
struct stat *dotinfop;
{
  ...
  xpath = (posixly_correct == 0 && *path == '~') ? bash_tilde_expand(path, 0)
                                                 : path;
  ...
  if (flags & FS_EXISTS) return (full_path);

  /* If we have a readable file, and the caller wants a readable file, this
     is it. */
  if ((flags & FS_READABLE) && (status & FS_READABLE)) return (full_path);
...
*/
static char *find_user_command_in_path(name, path_list, flags) const char *name;
char *path_list;
int flags;
{
  char *full_path, *path;
  ...
  if (path_list == 0 || *path_list == '\0') return (savestring(name)); /* XXX */

  file_to_lose_on = (char *)NULL;
  name_len = strlen(name);
  if (stat(".", &dotinfo) < 0) dotinfo.st_dev = dotinfo.st_ino = 0;
  path_index = 0;
