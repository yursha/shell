...
void set_default_locale() {
  ...
  if (default_locale) default_locale = savestring(default_locale);
...
void set_default_locale_vars() {
  ...
  if (val == 0 && lc_all && *lc_all) setlocale(LC_COLLATE, lc_all);
#endif /* LC_COLLATE */

#if defined(LC_MESSAGES)
  val = get_string_value("LC_MESSAGES");
  if (val == 0 && lc_all && *lc_all) setlocale(LC_MESSAGES, lc_all);
#endif /* LC_MESSAGES */

#if defined(LC_NUMERIC)
  val = get_string_value("LC_NUMERIC");
  if (val == 0 && lc_all && *lc_all) setlocale(LC_NUMERIC, lc_all);
#endif /* LC_NUMERIC */

#if defined(LC_TIME)
  val = get_string_value("LC_TIME");
  if (val == 0 && lc_all && *lc_all) setlocale(LC_TIME, lc_all);
#endif /* LC_TIME */
...
int set_locale_var(var, value) char *var, *value;
{
  int r;
  char *x;
...
/* Called when LANG is assigned a value.  Tracks value in `lang'.  Calls
   ...
   null. */
int set_lang(var, value) char *var, *value;
{
  FREE(lang);
  if (value)
...
char *get_locale_var(var) char *var;
{
  char *locale;
  ...
  if (locale == 0 || *locale == 0) locale = lang;
  if (locale == 0 || *locale == 0)
#if 0
...
static int reset_locale_vars() {
  ...
  if (lang == 0 || *lang == '\0')
    maybe_make_export_env(); /* trust that this will change environment for
                                setlocale */
  if (setlocale(LC_ALL, lang ? lang : "") == 0) return 0;
...
char *localetrans(string, len, lenp) char *string;
int len, *lenp;
{
  char *locale, *t;
  char *translated;
...
char *mk_msgstr(string, foundnlp) char *string;
int *foundnlp;
{
  register int c, len;
  char *result, *r, *s;
...
/* $"..." -- Translate the portion of STRING between START and END
   ...
   if non-null. */
char *localeexpand(string, start, end, lineno, lenp) char *string;
int start, end, lineno, *lenp;
{
  ...
  for (tlen = 0, len = start; len < end;) temp[tlen++] = string[len++];
  temp[tlen] = '\0';
...
static int locale_isutf8(lspec) char *lspec;
{
  char *cp;
