...
static int term() {
  ...
  if ((pos + 3 <= argc) && test_binop(argv[pos + 1])) value = binary_operator();
...
static int stat_mtime(fn, st, ts) char *fn;
struct stat *st;
struct timespec *ts;
{
  int r;
...
static int filecomp(s, t, op) char *s, *t;
int op;
{
  struct stat st1, st2;
...
static int arithcomp(s, t, op, flags) char *s, *t;
int op, flags;
{
  intmax_t l, r;
  int expok;
  ...
  if (flags & TEST_ARITHEXP) {
    l = evalexp(s, &expok);
    if (expok == 0) return (FALSE); /* should probably longjmp here */
    r = evalexp(t, &expok);
    if (expok == 0) return (FALSE); /* ditto */
  } else {
    if (legal_number(s, &l) == 0) integer_expected_error(s);
    if (legal_number(t, &r) == 0) integer_expected_error(t);
  }
...
static int patcomp(string, pat, op) char *string, *pat;
int op;
{
  int m;
...
int binary_test(op, arg1, arg2, flags) char *op, *arg1, *arg2;
int flags;
{
  int patmatch;
...
static int unary_operator() {
  ...
  op = argv[pos];
  if (test_unop(op) == 0) return (FALSE);
...
int unary_test(op, arg) char *op, *arg;
{
  intmax_t r;
  ...
  switch (op[1]) {
    ...
    case 'f': /* File is a file? */
      if (sh_stat(arg, &stat_buf) < 0) return (FALSE);
...
/* -f is true if the given file exists and is a regular file. */
    ...
    case 't': /* File fd is a terminal? */
      if (legal_number(arg, &r) == 0) return (FALSE);
...
int test_unop(op) char *op;
{
  if (op[0] != '-' || op[2] != 0) return (0);
 ...
 *	test expr
 */
int test_command(margc, margv) int margc;
char **margv;
{
  int value;
  int code;
  ...
  if (pos >= argc) test_exit(SHELL_BOOLEAN(FALSE));
