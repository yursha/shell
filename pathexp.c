...
int unquoted_glob_pattern_p(string) register char *string;
{
  register int c;
  char *send;
  int open;
...
static inline int ere_char(c) int c;
{
  switch (c) {
    case '.':
    case '[':
...
int qflags;
{
  ...
  for (i = j = 0; pathname[i]; i++) {
    ...
    else if ((qflags & QGLOB_REGEXP) && pathname[i] == CTLESC &&
             pathname[i + 1] == CTLESC) {
      i++;
      temp[j++] = pathname[i];
      continue;
    } else if (pathname[i] == CTLESC) {
      if ((qflags & QGLOB_FILENAME) && pathname[i + 1] == '/') continue;
    ...
    } else if ((qflags & QGLOB_REGEXP) &&
               (i == 0 || pathname[i - 1] != CTLESC) &&
               pathname[i] == '[') /*]*/
        {
      brack = 1;
...
char **shell_glob_filename(pathname) const char *pathname;
{
#if defined(USE_POSIX_GLOB_LIBRARY)
  register int i;
  char *temp, **results;
  ...
  if (results && ((GLOB_FAILED(results)) == 0)) {
    if (should_ignore_glob_matches()) ignore_glob_matches(results);
...
int should_ignore_glob_matches() { return globignore.num_ignores; }
...
static int glob_name_is_acceptable(name) const char *name;
{
  ...
  for (p = globignore.ignores; p->val; p++) {
    if (strmatch(p->val, (char *)name, flags) != FNM_NOMATCH) return (0);
  }
  return (1);
}
...
sh_ignore_func_t *name_func;
{
  ...
  /* Copy the acceptable names from NEWNAMES back to NAMES and set the
     new array end. */
  for (n = 0; newnames[n]; n++) names[n] = newnames[n];
  names[n] = (char *)NULL;
  free(newnames);
}
...
{
  if (globignore.num_ignores == 0) return;

  ignore_globbed_names(names, glob_name_is_acceptable);
}

static char *split_ignorespec(s, ip) char *s;
int *ip;
{
  char *t;
  int n, i;
...
void setup_ignore_patterns(ivp) struct ignorevar *ivp;
{
  ...
  if (ivp->ignores) {
    for (p = ivp->ignores; p->val; p++) free(p->val);
    free(ivp->ignores);
  ...
  if (this_ignoreval == 0 || *this_ignoreval == '\0') return;
...
#endif
  {
    if (numitems + 1 >= maxitems) {
      maxitems += 10;
      ivp->ignores =
          (struct ign *)xrealloc(ivp->ignores, maxitems * sizeof(struct ign));
    }
    ivp->ignores[numitems].val = colon_bit;
    ivp->ignores[numitems].len = strlen(colon_bit);
    ivp->ignores[numitems].flags = 0;
    if (ivp->item_func) (*ivp->item_func)(&ivp->ignores[numitems]);
    numitems++;
  }
