...
char *get_name_for_error() {
  ...
  if (name == 0 && shell_name && *shell_name) name = base_pathname(shell_name);
  if (name == 0)
#if defined(PROGRAM)
    name = PROGRAM;
#else
...
#endif
{
  ...
  if (exit_immediately_on_error) {
    if (last_command_exit_value == 0) last_command_exit_value = 1;
...
#endif
{
  ...
  if (tracefp == NULL) tracefp = fopen("/tmp/bash-trace.log", "a+");
