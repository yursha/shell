...
int reader_loop() {
  ...
  while (EOF_Reached == 0) {
    ...
    executing = 0;
    if (temporary_env) dispose_used_env_vars();
    ...
    if (read_command() == 0) {
      ...
      if (interactive == 0) EOF_Reached = EOF;
    }
...
static void send_pwd_to_eterm() {
  ...
  if (pwd == 0) f = pwd = get_working_directory("eterm");
...
int parse_command() {
  ...
  if (interactive && bash_input.type != st_string &&
    ...
    if (running_under_emacs == 2) send_pwd_to_eterm(); /* Yuck */
  }
