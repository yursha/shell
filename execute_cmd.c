...
static void print_formatted_time __P((FILE *, char *, time_t, int, time_t, int,
                                      time_t, int, int));
...
static int execute_simple_command(SIMPLE_COM *, int, int, int,
                                  struct fd_bitmap *);
static int execute_builtin(sh_builtin_func_t *, WORD_LIST *, int, int);
static int execute_function(SHELL_VAR *, WORD_LIST *, int, struct fd_bitmap *,
                            int, int);
...
__P((WORD_LIST *, REDIRECT *, sh_builtin_func_t *, SHELL_VAR *, int, int, int,
     struct fd_bitmap *, int));
static int execute_disk_command __P((WORD_LIST *, REDIRECT *, char *, int, int,
                                     int, struct fd_bitmap *, int));
...
/* Functions to allocate and deallocate the structures used to pass
   ...
   to close. */
struct fd_bitmap *new_fd_bitmap(size) int size;
{
...
int execute_command(command) COMMAND *command;
{
  ...
  /* don't unlink fifos if we're in a shell function; wait until the function
     returns. */
  if (variable_context == 0) unlink_fifo_list();
...
static int restore_signal_mask(set) sigset_t *set;
...
static void async_redirect_stdin() {
  int fd;
...
#define DESCRIBE_PID(pid)               \
  do {                                  \
    if (interactive) describe_pid(pid); \
  } while (0)
...
/* Execute the command passed in COMMAND, perhaps doing it asynchronously.
   ...
   return values.  Executing a command with nothing in it returns
   EXECUTION_SUCCESS. */
int execute_command_internal(command, asynchronous, pipe_in, pipe_out,
                             fds_to_close) COMMAND *command;
int asynchronous;
...
struct fd_bitmap *fds_to_close;
{
  ...
  if (breaking || continuing) return (last_command_exit_value);
  if (command == 0 || read_but_dont_execute) return (EXECUTION_SUCCESS);
  ...
  if (command->type == cm_subshell ||
    ...
    if (paren_pid == 0) {
      ...
      if (pipe_out != NO_PIPE) return (EXECUTION_SUCCESS);
...
#if defined(PROCESS_SUBSTITUTION)
    if (saved_fifo) free((void *)ofifo_list);
#endif
  ...
  if (my_undo_list || exec_undo_list) begin_unwind_frame("loop_redirections");
...
/* We can't rely on variables retaining their values across a
   ...
   result of a `return' builtin.  This is true for sure with gcc. */
      ...
      exec_result = execute_simple_command(
          command->value.Simple, pipe_in, pipe_out, asynchronous, fds_to_close);
      line_number = save_line_number;
    ...
    case cm_for:
      if (ignore_return) command->value.For->flags |= CMD_IGNORE_RETURN;
      exec_result = execute_for_command(command->value.For);
      break;

#if defined(ARITH_FOR_COMMAND)
    case cm_arith_for:
      if (ignore_return) command->value.ArithFor->flags |= CMD_IGNORE_RETURN;
...
#if defined(SELECT_COMMAND)
    case cm_select:
      if (ignore_return) command->value.Select->flags |= CMD_IGNORE_RETURN;
      exec_result = execute_select_command(command->value.Select);
      break;
#endif

    case cm_case:
      if (ignore_return) command->value.Case->flags |= CMD_IGNORE_RETURN;
      exec_result = execute_case_command(command->value.Case);
      break;

    case cm_while:
      if (ignore_return) command->value.While->flags |= CMD_IGNORE_RETURN;
    ...
    case cm_if:
      if (ignore_return) command->value.If->flags |= CMD_IGNORE_RETURN;
    ...
    case cm_connection:
      exec_result = execute_connection(command, asynchronous, pipe_in, pipe_out,
                                       fds_to_close);
      if (asynchronous) invert = 0; /* XXX */
    ...
    case cm_arith:
      was_error_trap =
          signal_is_trapped(ERROR_TRAP) && signal_is_ignored(ERROR_TRAP) == 0;
      if (ignore_return) command->value.Arith->flags |= CMD_IGNORE_RETURN;
    ...
    case cm_cond:
      was_error_trap =
          signal_is_trapped(ERROR_TRAP) && signal_is_ignored(ERROR_TRAP) == 0;
      if (ignore_return) command->value.Cond->flags |= CMD_IGNORE_RETURN;
  ...
  if (exec_undo_list) dispose_redirects(exec_undo_list);

  if (my_undo_list || exec_undo_list) discard_unwind_frame("loop_redirections");

#if defined(PROCESS_SUBSTITUTION)
  if (saved_fifo) {
    nfifo = num_fifos();
    if (nfifo > ofifo) close_new_fifos((char *)ofifo_list, osize);
...
static int mkfmt(buf, prec, lng, sec, sec_fraction) char *buf;
int prec, lng;
time_t sec;
int sec_fraction;
{
  ...
  if (lng) {
    ...
    while (min /= 10);
    aind++;
    while (abuf[aind]) buf[ind++] = abuf[aind++];
    buf[ind++] = 'm';
  }
...
/* Interpret the format string FORMAT, interpolating the following escape
   ...
   resectively. */
static void print_formatted_time(fp, format, rs, rsf, us, usf, ss, ssf,
                                 cpu) FILE *fp;
char *format;
time_t rs;
int rsf;
...
static int time_command(command, asynchronous, pipe_in, pipe_out,
                        fds_to_close) COMMAND *command;
...
struct fd_bitmap *fds_to_close;
{
  ...
  if (asynchronous) {
    ...
    /* This test is to prevent alias expansion by interactive shells that
       ...
       aliases. */
    if (ois != interactive_shell) expand_aliases = 0;
  }
  ...
  if (user_subshell)
    ...
    if (asynchronous) subshell_environment |= SUBSHELL_ASYNC;
    if (pipe_in != NO_PIPE || pipe_out != NO_PIPE)
      subshell_environment |= SUBSHELL_PIPE;
    if (user_coproc) subshell_environment |= SUBSHELL_COPROC;
  }
  ...
  if (fds_to_close) close_fd_bitmap(fds_to_close);
  ...
  if (should_redir_stdin && stdin_redir == 0) async_redirect_stdin();
  ...
  if (command->flags & CMD_TIME_PIPELINE) tcom->flags |= CMD_TIME_PIPELINE;
  if (command->flags & CMD_TIME_POSIX) tcom->flags |= CMD_TIME_POSIX;
...
static struct cpelement *cpe_alloc(cp) Coproc *cp;
{
...
static struct cpelement *cpl_add(cp) Coproc *cp;
{
  struct cpelement *cpe;
...
static void cpl_closeall() {
  ...
  for (cpe = coproc_list.head; cpe; cpe = cpe->next) coproc_close(cpe->coproc);
}
...
static struct cpelement *cpl_search(pid) pid_t pid;
{
  struct cpelement *cpe;

  for (cpe = coproc_list.head; cpe; cpe = cpe->next)
    if (cpe->coproc->c_pid == pid) return cpe;
...
static struct cpelement *cpl_searchbyname(name) const char *name;
{
  ...
  for (cp = coproc_list.head; cp; cp = cp->next)
    if (STREQ(cp->coproc->c_name, name)) return cp;
  return (struct cpelement *)NULL;
}
...
struct coproc *getcoprocbypid(pid) pid_t pid;
{
#if MULTIPLE_COPROCS
  struct cpelement *p;
...
struct coproc *coproc_alloc(name, pid) char *name;
pid_t pid;
{
  struct coproc *cp;
...
void coproc_reap() {
  ...
  if (cp && (cp->c_flags & COPROC_DEAD)) coproc_dispose(cp);
#endif
}
...
int fd;
{
  ...
  update = 0;
  if (cp->c_rfd >= 0 && cp->c_rfd == fd) update = cp->c_rfd = -1;
  if (cp->c_wfd >= 0 && cp->c_wfd == fd) update = cp->c_wfd = -1;
  if (update) coproc_setvars(cp);
}
...
void coproc_setvars(cp) struct coproc *cp;
{
  ...
  w.word = cp->c_name;
  w.flags = 0;
  if (check_identifier(&w, 1) == 0) return;
  ...
  if (v == 0) {
    ...
    if (v == INVALID_NAMEREF_VALUE) return;
    if (v && nameref_p(v)) {
      free(cp->c_name);
  ...
  if (v && (readonly_p(v) || noassign_p(v))) {
    if (readonly_p(v)) err_readonly(cp->c_name);
    return;
  }
  if (v == 0) v = make_new_array_variable(cp->c_name);
  if (array_p(v) == 0) v = convert_var_to_array(v);
...
static int execute_coproc(command, pipe_in, pipe_out,
                          fds_to_close) COMMAND *command;
int pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
...
static int execute_pipeline(command, asynchronous, pipe_in, pipe_out,
                            fds_to_close) COMMAND *command;
int asynchronous, pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
  ...
  while (cmd && cmd->type == cm_connection && cmd->value.Connection &&
    ...
    new_bitmap_size =
        (fildes[0] < fds_to_close->size) ? fds_to_close->size : fildes[0] + 8;
    ...
    if (prev >= 0) add_unwind_protect(close, prev);
    dummyfd = fildes[1];
  ...
  if (ignore_return && cmd) cmd->flags |= CMD_IGNORE_RETURN;
  ...
  if (lastpipe_opt && job_control == 0 && asynchronous == 0 &&
    ...
    if (cmd) cmd->flags |= CMD_LASTPIPE;
  }
  if (prev >= 0) add_unwind_protect(close, prev);

  exec_result =
      execute_command_internal(cmd, asynchronous, prev, pipe_out, fds_to_close);

  if (lstdin > 0) restore_stdin(lstdin);
...
static int execute_connection(command, asynchronous, pipe_in, pipe_out,
                              fds_to_close) COMMAND *command;
int asynchronous, pipe_in, pipe_out;
struct fd_bitmap *fds_to_close;
{
  ...
  switch (command->value.Connection->connector) {
    ...
    case '&':
      ...
      if (tc == 0) return (EXECUTION_SUCCESS);

      if (ignore_return) tc->flags |= CMD_IGNORE_RETURN;
...
/* If this shell was compiled without job control support,
   ...
   asynchronous command is forced to /dev/null. */
      ...
      if (tc->flags & CMD_STDIN_REDIR) tc->flags &= ~CMD_STDIN_REDIR;

      second = command->value.Connection->second;
      if (second) {
        if (ignore_return) second->flags |= CMD_IGNORE_RETURN;

        exec_result = execute_command_internal(second, asynchronous, pipe_in,
    ...
    case '|':
      ...
      line_number_for_err_trap = line_number; /* XXX - save value? */
      exec_result = execute_pipeline(command, asynchronous, pipe_in, pipe_out,
                                     fds_to_close);
    ...
    case OR_OR:
      ...
      if (((command->value.Connection->connector == AND_AND) &&
          ...
          ((command->value.Connection->connector == OR_OR) &&
           (exec_result != EXECUTION_SUCCESS))) {
        second = command->value.Connection->second;
        if (ignore_return && second) second->flags |= CMD_IGNORE_RETURN;
...
#define REAP()                                                        \
  do {                                                                \
    if (job_control == 0 || interactive_shell == 0) reap_dead_jobs(); \
  } while (0)

/* Execute a FOR command.  The syntax is: FOR word_desc IN word_list;
   DO command; DONE */
static int execute_for_command(for_command) FOR_COM *for_command;
{
  ...
  for (retval = EXECUTION_SUCCESS; list; list = list->next) {
    ...
    if (echo_command_at_execute) xtrace_print_for_command_head(for_command);
    ...
    /* In debugging mode, if the DEBUG trap returns a non-zero status, we
       skip the command. */
    if (debugging_mode && retval != EXECUTION_SUCCESS) continue;
#endif
    ...
    if (ifsname(identifier)) setifs(v);
...
*/
static intmax_t eval_arith_for_expr(l, okp) WORD_LIST *l;
int *okp;
{
  WORD_LIST *new;
  intmax_t expresult;
  ...
  if (new) {
    if (echo_command_at_execute) xtrace_print_arith_cmd(new);
...
static int print_index_and_element(len, ind, list) int len, ind;
WORD_LIST *list;
{
...
static char *select_query(list, list_len, prompt, print_menu) WORD_LIST *list;
int list_len;
char *prompt;
int print_menu;
{
  ...
  for (l = list; l; l = l->next) {
    ...
    if (len > max_elem_len) max_elem_len = len;
  }
  ...
  while (1) {
    ...
    if (repl_string == 0) return ((char *)NULL);
    if (*repl_string == 0) {
      print_menu = 1;
      continue;
    }
    if (legal_number(repl_string, &reply) == 0) return "";
    if (reply < 1 || reply > list_len) return "";
...
/* Execute a SELECT command.  The syntax is:
   ...
   the command. */
static int execute_select_command(select_command) SELECT_COM *select_command;
{
  ...
  if (echo_command_at_execute) xtrace_print_select_command_head(select_command);
  ...
  /* In debugging mode, if the DEBUG trap returns a non-zero status, we
     skip the command. */
  if (debugging_mode && retval != EXECUTION_SUCCESS) return (EXECUTION_SUCCESS);
#endif
  ...
  while (1) {
    ...
    if (selection && *selection == '\0') show_menu = 1;
#endif
  }
...
/* Execute a CASE command.  The syntax is: CASE word_desc IN pattern_list ESAC.
   ...
   execute. */
static int execute_case_command(case_command) CASE_COM *case_command;
{
  ...
  if (echo_command_at_execute) xtrace_print_case_command_head(case_command);
...
/* The WHILE command.  Syntax: WHILE test DO action; DONE.
   ...
   EXECUTION_SUCCESS. */
static int execute_while_command(while_command) WHILE_COM *while_command;
{ return (execute_while_or_until(while_command, CMD_WHILE)); }

/* UNTIL is just like WHILE except that the test result is negated. */
static int execute_until_command(while_command) WHILE_COM *while_command;
...
static int execute_while_or_until(while_command, type) WHILE_COM *while_command;
int type;
{
...
static int execute_if_command(if_command) IF_COM *if_command;
{
...
#if defined(DPAREN_ARITHMETIC)
static int execute_arith_command(arith_command) ARITH_COM *arith_command;
{
...
static int execute_cond_node(cond) COND_COM *cond;
{
  ...
  if (ignore) {
    if (cond->left) cond->left->flags |= CMD_IGNORE_RETURN;
    if (cond->right) cond->right->flags |= CMD_IGNORE_RETURN;
  }
  ...
  if (cond->type == COND_EXPR)
    ...
    result = execute_cond_node(cond->left);
    if (result != EXECUTION_SUCCESS) result = execute_cond_node(cond->right);
  ...
  if (invert)
    result =
        (result == EXECUTION_SUCCESS) ? EXECUTION_FAILURE : EXECUTION_SUCCESS;

  return result;
}

static int execute_cond_command(cond_command) COND_COM *cond_command;
{
  int retval, save_line_number;
...
static void bind_lastarg(arg) char *arg;
{
  ...
  if (var) VUNSETATTR(var, att_exported);
}

/* Execute a null command.  Fork a subshell if the command uses pipes or is
   to be run asynchronously.  This handles all the side effects that are
   supposed to take place. */
static int execute_null_command(redirects, pipe_in, pipe_out,
                                async) REDIRECT *redirects;
int pipe_in, pipe_out, async;
{
  ...
  if (forcefork || pipe_in != NO_PIPE || pipe_out != NO_PIPE || async) {
    ...
    if (make_child((char *)NULL, async) == 0) {
      ...
      subshell_environment = 0;
      if (async) subshell_environment |= SUBSHELL_ASYNC;
      if (pipe_in != NO_PIPE || pipe_out != NO_PIPE)
        subshell_environment |= SUBSHELL_PIPE;
      ...
      close_pipes(pipe_in, pipe_out);
#if defined(PROCESS_SUBSTITUTION) && defined(HAVE_DEV_FD)
      if (pipe_out == NO_PIPE) unlink_fifo_list();
#endif
      return (EXECUTION_SUCCESS);
    }
  } else {
...
static void fix_assignment_words(words) WORD_LIST *words;
{
  ...
  for (wcmd = words; wcmd; wcmd = wcmd->next)
    if ((wcmd->word->flags & W_ASSIGNMENT) == 0) break;
  ...
  for (w = wcmd; w; w = w->next)
    if (w->word->flags & W_ASSIGNMENT) {
...
#if defined(ARRAY_VARS)
      if (assoc) w->word->flags |= W_ASSIGNASSOC;
      if (array) w->word->flags |= W_ASSIGNARRAY;
#endif
      if (global) w->word->flags |= W_ASSNGLOBAL;
      ...
      /* If we have an assignment builtin that does not create local variables,
         ...
         `declare' */
      if (b && ((b->flags & (ASSIGNMENT_BUILTIN | LOCALVAR_BUILTIN)) ==
                ASSIGNMENT_BUILTIN))
...
static int is_dirname(pathname) char *pathname;
{
  char *temp;
  int ret;
...
/* The meaty part of all the executions.  We have to start hacking the
   ...
   execute the command. */
static int execute_simple_command(simple_command, pipe_in, pipe_out, async,
                                  fds_to_close) SIMPLE_COM *simple_command;
int pipe_in, pipe_out, async;
struct fd_bitmap *fds_to_close;
{
  ...
  /* In debugging mode, if the DEBUG trap returns a non-zero status, we
     skip the command. */
  if (debugging_mode && result != EXECUTION_SUCCESS) return (EXECUTION_SUCCESS);
#endif

  first_word_quoted = simple_command->words
                          ? (simple_command->words->word->flags & W_QUOTED)
                          : 0;
  ...
  if (dofork) {
    ...
    if (make_child(savestring(the_printed_command_except_trap), async) == 0) {
      ...
      if (async) subshell_level++; /* not for pipes yet */
    } else {
      /* Don't let simple commands that aren't the last command in a
         pipeline change $? for the rest of the pipeline (or at all). */
      if (pipe_out != NO_PIPE) result = last_command_exit_value;
      close_pipes(pipe_in, pipe_out);
#if defined(PROCESS_SUBSTITUTION) && defined(HAVE_DEV_FD)
  ...
  if ((simple_command->flags & CMD_INHIBIT_EXPANSION) == 0) {
    ...
    if (simple_command->flags & CMD_IGNORE_RETURN) comsub_ignore_return--;
    current_fds_to_close = (struct fd_bitmap *)NULL;
  } else
  ...
  if (echo_command_at_execute) xtrace_print_word_list(words, 1);
  ...
  if ((simple_command->flags & CMD_NO_FUNCTIONS) == 0) {
    ...
    if (posixly_correct) {
      ...
      if (builtin) builtin_is_special = 1;
    }
    if (builtin == 0) func = find_function(words->word->word);
  }
  ...
  /* One other possibililty.  The user may want to resume an existing job.
     ...
     job. */
  if (job_control && already_forked == 0 && async == 0 && !first_word_quoted &&
...
run_builtin:
  ...
  if (builtin || func) {
    ...
    if (already_forked) {
      ...
      if (async) {
        if ((simple_command->flags & CMD_STDIN_REDIR) && pipe_in == NO_PIPE &&
            (stdin_redirects(simple_command->redirects) == 0))
          async_redirect_stdin();
        setup_async_signals();
      }

      if (async == 0) subshell_level++;
      execute_subshell_builtin_or_function(
          words, simple_command->redirects, builtin, func, pipe_in, pipe_out,
      ...
      if (builtin) {
        if (result > EX_SHERRBASE) {
          ...
          if (builtin_is_special) special_builtin_failed = 1;
        }
  ...
  if (already_forked == 0 && (simple_command->flags & CMD_NO_FORK) &&
      fifos_pending() > 0)
    simple_command->flags &= ~CMD_NO_FORK;
#endif
...
static int builtin_status(result) int result;
{
  int r;
...
static int execute_builtin(builtin, words, flags,
                           subshell) sh_builtin_func_t *builtin;
WORD_LIST *words;
int flags, subshell;
{
  ...
  if (subshell == 0 && (flags & CMD_IGNORE_RETURN) &&
      (builtin == eval_builtin || builtin == command_builtin ||
  ...
  if (isbltinenv) {
    if (subshell == 0) begin_unwind_frame("builtin_env");
  ...
  if (subshell == 0 && isbltinenv) run_unwind_frame("builtin_env");
...
void restore_funcarray_state(fa) struct func_array_state *fa;
{
  ...
  if (nfv == fa->funcname_v) array_pop(funcname_a);

  free(fa);
}
#endif

static int execute_function(var, words, flags, fds_to_close, async,
                            subshell) SHELL_VAR *var;
WORD_LIST *words;
int flags;
struct fd_bitmap *fds_to_close;
int async, subshell;
{
  ...
  if (tc && (flags & CMD_IGNORE_RETURN)) tc->flags |= CMD_IGNORE_RETURN;
...
/* Shell functions inherit the RETURN trap if function tracing is on
...
#else
  if (return_trap && (signal_in_progress(DEBUG_TRAP) ||
  ...
  if (subshell == 0) add_unwind_protect(restore_funcarray_state, fa);
#endif
  ...
  if (debugging_mode) {
    ...
    if (subshell == 0) add_unwind_protect(pop_args, 0);
  }
  ...
  if (return_val) {
    ...
    if (from_return_trap == 0) run_return_trap();
  ...
  if (gv && gv->context == variable_context) gs->gs_flags |= 1;

  if (subshell == 0) run_unwind_frame("function_calling");
#if defined(ARRAY_VARS)
  else {
    restore_funcarray_state(fa);
    /* Restore BASH_ARGC and BASH_ARGV */
    if (debugging_mode) pop_args();
  }
#endif
...
int execute_shell_function(var, words) SHELL_VAR *var;
WORD_LIST *words;
{
  int ret;
  struct fd_bitmap *bitmap;
...
/* Execute a shell builtin or function in a subshell environment.  This
   ...
   command is executed. */
static void execute_subshell_builtin_or_function(words, redirects, builtin, var,
                                                 pipe_in, pipe_out, async,
                                                 fds_to_close,
                                                 flags) WORD_LIST *words;
REDIRECT *redirects;
sh_builtin_func_t *builtin;
SHELL_VAR *var;
int pipe_in, pipe_out, async;
...
int flags;
{
  ...
  if (do_redirections(redirects, RX_ACTIVE) != 0) exit(EXECUTION_FAILURE);
...
static int execute_builtin_or_function(words, builtin, var, redirects,
                                       fds_to_close, flags) WORD_LIST *words;
sh_builtin_func_t *builtin;
SHELL_VAR *var;
REDIRECT *redirects;
struct fd_bitmap *fds_to_close;
int flags;
{
  ...
  if (builtin == command_builtin && this_shell_builtin == exec_builtin) {
    ...
    if (discard) discard_unwind_frame("saved-redirects");
  }
  ...
  nfifo = num_fifos();
  if (nfifo > ofifo) close_new_fifos(ofifo_list, osize);
  free(ofifo_list);
#endif
...
static int execute_disk_command(words, redirects, command_line, pipe_in,
...
static char *getinterp(sample, sample_len, endp) char *sample;
int sample_len, *endp;
{
  register int i;
  char *execname;
...
static int execute_shell_script(sample, sample_len, command, args,
                                env) char *sample;
int sample_len;
char *command;
char **args, **env;
{
  ...
  for (i = larry - 1; i; i--) args[i] = args[i - size_increment];
...
static void initialize_subshell() {
  ...
  /* Zero out builtin_env, since this could be a shell script run from a
     ...
     memory leak. */
  if (vc_isbltnenv(shell_variables)) shell_variables = shell_variables->down;
  ...
  if (interactive_shell == 0) unset_bash_input(0);
}
...
/* Call execve (), handling interpreting shell scripts, and handling
   exec failures. */
int shell_execve(command, args, env) char *command;
char **args, **env;
{
  int larray, i, fd;
...
/* The file has the execute bits set, but the kernel refuses to
  ...
  for (i = larray - 1; i; i--) args[i] = args[i - 1];
  ...
  if (subshell_argv) {
    ...
    for (i = 1; i < subshell_argc; i++) free(subshell_argv[i]);
    free(subshell_argv);
  }
...
static int execute_intern_function(name, funcdef) WORD_DESC *name;
...
void close_all_files() {
  ...
  for (i = 3; i < fd_table_size; i++) close(i);
}
#endif /* PROCESS_SUBSTITUTION */
#endif
...
{
  if (pipe_in != NO_PIPE) {
    if (dup2(pipe_in, 0) < 0) dup_error(pipe_in, 0);
  ...
  if (pipe_out != NO_PIPE) {
    if (pipe_out != REDIRECT_BOTH) {
      if (dup2(pipe_out, 1) < 0) dup_error(pipe_out, 1);
      if (pipe_out == 0 || pipe_out > 1) close(pipe_out);
    } else {
      if (dup2(1, 2) < 0) dup_error(1, 2);
    }
#ifdef __CYGWIN__
