...
void initialize_signames() {
  ...
  for (i = 1; i < signal_names_size; i++) signal_names[i] = (char *)NULL;
...
#if defined( \
    SIGCANCEL) /* Solaris: thread cancellation signal used by libthread */
...
/* 4.4 BSD */
  ...
  for (i = 0; i < NSIG; i++)
    if (signal_names[i] == (char *)NULL) {
      ...
      if (signal_names[i]) sprintf(signal_names[i], "SIGJUNK(%d)", i);
    }
