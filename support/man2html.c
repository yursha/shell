...
static char *read_man_page(char *filename) {
  ...
  if (stat(filename, &stbuf) == -1) return NULL;
  ...
  while (!finished) {
    ...
    if ((*c == '_' || *c == '=') && (c[1] == itemsep || c[1] == '\n')) {
      ...
      *c = 'T';
      ...
      while (*c++ != '\n')
        ;
      if (currow->prev) currow->prev->next = NULL;
      currow->prev = NULL;
      clear_table(currow);
    } else if (*c == '.' && c[-1] == '\n' && !isdigit(c[1])) {
      /*
    ...
    switch (i) {
      ...
      case V('s', 'o'): {
        ...
        *c = '\0';
        ...
        if (!out_length) {
          ...
          if (!s) s = t;
          printf(
              "<HTML><HEAD><TITLE> Manpage of %s</TITLE>\n"
              "</HEAD><BODY>\n"
      ...
      case V('T', 'X'):
        ...
        *sl = '\0';
        ...
        out_html(c);
        out_html(change_to_font('R'));
        if (words > 1) out_html(wordlist[1]);
        *sl = '\n';
        c = sl + 1;
        break;
      case V('r', 'm'):
      ...
      default:
        ...
        if (owndef) {
          ...
          *sl = '\0';
          ...
          for (i = 0; i < words; i++)
            if (wordlist[i]) free(wordlist[i]);
          *sl = '\n';
        } else if (mandoc_command &&
                   ((isupper(*c) && islower(*(c + 1))) ||
                    (islower(*c) &&
                     isupper(*(c + 1))))) { /* Let through any BSD mandoc
    ...
    if (*h == escapesym) {
      ...
      switch (*h) {
        ...
        case '\t': {
          ...
          while (curtab < maxtstop && tabstops[curtab] <= curpos) curtab++;
          if (curtab < maxtstop) {
            if (!fillout) {
