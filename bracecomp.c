...
static int string_gcd(s1, s2) char *s1, *s2;
{
  register int i;
...
static char *really_munge_braces(array, real_start, real_end,
                                 gcd_zero) char **array;
int real_start, real_end, gcd_zero;
{
  ...
  *result = '\0';
  ...
  if (gcd_zero == 0) result[strlen(result) - 1] = flag ? '}' : '\0';
  return (result);
}

static int _strcompare(s1, s2) char **s1, **s2;
{
  int result;
...
static int hack_braces_completion(names) char **names;
{
  register int i;
  char *temp;
...
int bash_brace_completion(count, ignore) int count, ignore;
{
