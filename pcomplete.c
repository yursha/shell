...
void clean_itemlist(itp) ITEMLIST *itp;
{
  ...
  if (sl) {
    ...
    if ((itp->flags & LIST_DONTFREE) == 0) free(sl->list);
    free(sl);
  }
  itp->slist = (STRINGLIST *)NULL;
  itp->flags &=
      ~(LIST_DONTFREE | LIST_DONTFREEMEMBERS | LIST_INITIALIZED | LIST_DIRTY);
}

static int shouldexp_filterpat(s) char *s;
{
  register char *p;
...
static char *preproc_filterpat(pat, text) char *pat;
const char *text;
{
  char *ret;
...
STRINGLIST *filter_stringlist(sl, filterpat, text) STRINGLIST *sl;
char *filterpat;
const char *text;
{
  int i, m, not;
  ...
  if (sl == 0 || sl->list == 0 || sl->list_len == 0) return sl;

  npat = shouldexp_filterpat(filterpat) ? preproc_filterpat(filterpat, text)
                                        : filterpat;
...
STRINGLIST *completions_to_stringlist(matches) char **matches;
{
  STRINGLIST *sl;
  int mlen, i, n;
  ...
  if (matches == 0 || matches[0] == 0) return sl;
  ...
  for (i = 1, n = 0; i < mlen; i++, n++) sl->list[n] = STRDUP(matches[i]);
  sl->list_len = n;
  sl->list[n] = (char *)NULL;
...
static int it_init_aliases(itp) ITEMLIST *itp;
{
#ifdef ALIAS
  alias_t **alias_list;
  register int i, n;
  ...
  for (i = 0; i < n; i++) sl->list[i] = STRDUP(alias_list[i]->name);
  sl->list[n] = (char *)NULL;
...
SVFUNC *svfunc;
{
  ...
  for (n = 0; vlist[n]; n++)
    ;
  sl = strlist_create(n + 1);
  for (i = 0; i < n; i++) sl->list[i] = savestring(vlist[i]->name);
  sl->list[sl->list_len = n] = (char *)NULL;
  itp->slist = sl;
}

static int it_init_arrayvars(itp) ITEMLIST *itp;
{
#if defined(ARRAY_VARS)
...
static int it_init_bindings(itp) ITEMLIST *itp;
{
  char **blist;
  STRINGLIST *sl;
...
static int it_init_builtins(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
  register int i, n;
  ...
  for (i = n = 0; i < num_shell_builtins; i++)
    if (shell_builtins[i].function) sl->list[n++] = shell_builtins[i].name;
  sl->list[sl->list_len = n] = (char *)NULL;
  itp->flags |= LIST_DONTFREEMEMBERS;
  itp->slist = sl;
  return 0;
...
static int it_init_enabled(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
  register int i, n;
...
static int it_init_disabled(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
  register int i, n;
...
static int it_init_exported(itp) ITEMLIST *itp;
{
  init_itemlist_from_varlist(itp, all_exported_variables);
  return 0;
}

static int it_init_functions(itp) ITEMLIST *itp;
{
...
static int it_init_helptopics(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
  register int i, n;
...
static int it_init_hostnames(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
...
static int it_init_joblist(itp, jstate) ITEMLIST *itp;
int jstate;
{
#if defined(JOB_CONTROL)
  STRINGLIST *sl;
...
static int it_init_jobs(itp) ITEMLIST *itp;
{ return (it_init_joblist(itp, -1)); }

static int it_init_running(itp) ITEMLIST *itp;
{ return (it_init_joblist(itp, 0)); }

static int it_init_stopped(itp) ITEMLIST *itp;
{ return (it_init_joblist(itp, 1)); }

static int it_init_keywords(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
  register int i, n;
  ...
  for (n = 0; word_token_alist[n].word; n++)
    ;
  sl = strlist_create(n);
  for (i = 0; i < n; i++) sl->list[i] = word_token_alist[i].word;
...
static int it_init_signals(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
...
static int it_init_variables(itp) ITEMLIST *itp;
{
  init_itemlist_from_varlist(itp, all_visible_variables);
  return 0;
}

static int it_init_setopts(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
...
static int it_init_shopts(itp) ITEMLIST *itp;
{
  STRINGLIST *sl;
...
/* Generate a list of all matches for TEXT using the STRINGLIST in itp->slist
   ...
   match. */
static STRINGLIST *gen_matches_from_itemlist(itp, text) ITEMLIST *itp;
const char *text;
{
  STRINGLIST *ret, *sl;
  int tlen, i, n;
  ...
  if ((itp->flags & (LIST_DIRTY | LIST_DYNAMIC)) ||
    ...
    if (itp->flags & (LIST_DIRTY | LIST_DYNAMIC)) clean_itemlist(itp);
    if ((itp->flags & LIST_INITIALIZED) == 0) initialize_itemlist(itp);
  }
  if (itp->slist == 0) return ((STRINGLIST *)NULL);
...
static STRINGLIST *gen_action_completions(cs, text) COMPSPEC *cs;
const char *text;
{
  ...
  GEN_XCOMPS(flags, CA_FILE, text, pcomp_filename_completion_function, cmatches,
             ret, tmatches);
...
/* Generate a list of matches for CS->globpat.  Unresolved: should this use
   ...
   glob_filename. */
static STRINGLIST *gen_globpat_matches(cs, text) COMPSPEC *cs;
const char *text;
{
  STRINGLIST *sl;
  ...
  if (GLOB_FAILED(sl->list)) sl->list = (char **)NULL;
  if (sl->list) sl->list_len = sl->list_size = strvec_len(sl->list);
  return sl;
}

/* Perform the shell word expansions on CS->words and return the results.
   Again, this ignores TEXT. */
static STRINGLIST *gen_wordlist_matches(cs, text) COMPSPEC *cs;
const char *text;
{
  WORD_LIST *l, *l2;
  STRINGLIST *sl;
  ...
  if (cs->words == 0 || cs->words[0] == '\0') return ((STRINGLIST *)NULL);
...
static SHELL_VAR *bind_comp_words(lwords) WORD_LIST *lwords;
{
  SHELL_VAR *v;

  v = find_variable("COMP_WORDS");
  if (v == 0) v = make_new_array_variable("COMP_WORDS");
  if (readonly_p(v)) VUNSETATTR(v, att_readonly);
...
int cw, exported;
{
  ...
  if (v && exported) VSETATTR(v, att_exported);
...
#endif
  if (exported) array_needs_making = 1;
}
...
static WORD_LIST *build_arg_list(cmd, cname, text, lwords, ind) char *cmd;
const char *cname;
const char *text;
WORD_LIST *lwords;
int ind;
...
int *foundp;
{
  ...
  if (fval == EX_RETRYFAIL) found |= PCOMP_RETRYFAIL;
  if (foundp) *foundp = found;
...
static STRINGLIST *gen_command_matches(cs, cmd, text, line, ind, lwords, nw,
                                       cw) COMPSPEC *cs;
const char *cmd;
const char *text;
char *line;
...
static WORD_LIST *command_line_to_word_list(line, llen, sentinel, nwp,
                                            cwp) char *line;
...
int *foundp;
{
  ...
  if (cs->funcname) {
    foundf = 0;
    tmatches = gen_shell_function_matches(cs, cmd, word, line, rl_point - start,
                                          lwords, nw, cw, &foundf);
    if (foundf != 0) found = foundf;
    if (tmatches) {
#ifdef DEBUG
      if (progcomp_debug) {
        debug_printf("gen_shell_function_matches (%p, %s, %s, %p, %d, %d) -->",
  ...
  if (cs->command) {
    tmatches = gen_command_matches(cs, cmd, word, line, rl_point - start,
                                   lwords, nw, cw);
    if (tmatches) {
#ifdef DEBUG
      if (progcomp_debug) {
        debug_printf("gen_command_matches (%p, %s, %s, %p, %d, %d) -->", cs,
                     cmd, word, lwords, nw, cw);
        strlist_print(tmatches, "\t");
        rl_on_new_line();
      }
#endif
  ...
  if (cs->command || cs->funcname) {
    if (lwords) dispose_words(lwords);
    FREE(line);
  }
...
#ifdef DEBUG
    if (progcomp_debug) {
      debug_printf("filter_stringlist (%p, %s, %s) -->", ret, cs->filterpat,
                   word);
      strlist_print(tmatches, "\t");
      rl_on_new_line();
    }
...
void pcomp_set_readline_variables(flags, nval) int flags, nval;
{
  ...
  if (flags & COPT_FILENAMES) rl_filename_completion_desired = nval;
  /* If the user doesn't want a space appended, tell readline. */
  if (flags & COPT_NOSPACE) rl_completion_suppress_append = nval;
  /* The value here is inverted, since the default is on and the `noquote'
     option is supposed to turn it off */
  if (flags & COPT_NOQUOTE) rl_filename_quoting_desired = 1 - nval;
  if (flags & COPT_NOSORT) rl_sort_completion_matches = 1 - nval;
}
...
{
  if (cs == 0 && ((cs = pcomp_curcs) == 0)) return;
  if (set_or_unset)
    cs->options |= flags;
  else
    cs->options &= ~flags;
...
static STRINGLIST *gen_progcomp_completions(ocmd, cmd, word, start, end, foundp,
                                            retryp, lastcs) const char *ocmd;
const char *cmd;
const char *word;
int start, end;
int *foundp, *retryp;
COMPSPEC **lastcs;
{
  ...
  if (*lastcs) compspec_dispose(*lastcs);
  cs->refcount++; /* XXX */
  *lastcs = cs;
  ...
  if (retryp) *retryp = foundp && (*foundp & PCOMP_RETRYFAIL);
...
int start, end, *foundp;
{
  ...
  do {
    ...
    ret = gen_progcomp_completions(cmd, cmd, word, start, end, &found, &retry,
                                   &lastcs);
    if (found == 0) {
      t = strrchr(cmd, '/');
      if (t && *(++t))
        ret = gen_progcomp_completions(t, cmd, word, start, end, &found, &retry,
                                       &lastcs);
    }

    if (found == 0)
      ret = gen_progcomp_completions(DEFAULTCMD, cmd, word, start, end, &found,
                                     &retry, &lastcs);
