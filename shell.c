...
#include "findcmd.h"
    ...
    !defined(DISABLE_MALLOC_WRAPPERS)
...
int main(int argc, char **argv, char **env) {
  ...
  if (dump_po_strings) dump_translatable_strings = 1;

  if (dump_translatable_strings) read_but_dont_execute = 1;
  ...
  if (shopt_alist) run_shopt_alist();
  ...
  if (command_execution_string) {
    ...
    if (debugging_mode) start_debugger();
...
static int parse_long_options(argv, arg_start, arg_end) char **argv;
...
static int parse_shell_options(argv, arg_start, arg_end) char **argv;
int arg_start, arg_end;
{
  int arg_index;
...
/* Clean up the terminal if we are in a state where it's been modified. */
...
#endif
  if (read_tty_modified()) read_tty_cleanup();

  /* Do trap[0] if defined.  Allow it to override the exit status
     passed to us. */
  if (signal_is_trapped(0)) s = run_exit_trap();
...
#if defined(HISTORY)
  if (remember_on_history) maybe_save_shell_history();
#endif /* HISTORY */
  ...
  if (interactive_shell && login_shell && hup_on_exit) hangup_all_jobs();
  ...
  if (subshell_environment == 0) end_job_control();
#endif /* JOB_CONTROL */
...
static void execute_env_file(env_file) char *env_file;
{
  ...
  if (env_file && *env_file) {
    ...
    if (fn && *fn) maybe_execute_file(fn, 1);
    FREE(fn);
  }
}
...
#if defined(WORDEXP_OPTION)
static int run_wordexp(words) char *words;
{
  int code, nw, nb;
  ...
  if (words && *words) {
    ...
    if (parse_command() != 0) return (126);
    if (global_command == 0) {
      printf("0\n0\n");
      return (0);
    }
    if (global_command->type != cm_simple) return (126);
  ...
  for (wl = result; wl; wl = wl->next) printf("%s\n", wl->word->word);
...
static int run_one_command(command) char *command;
{
  int code;
...
static int bind_args(argv, arg_start, arg_end, start_index) char **argv;
...
static int open_shell_script(script_name) char *script_name;
{
  int fd, e, fd_is_tty;
  ...
  if (fileno(default_input) != fd) SET_CLOSE_ON_EXEC(fileno(default_input));
#endif /* !BUFFERED_INPUT */
...
/* Make sure the fd from which we are reading input is not in
  ...
  if (interactive && no_line_editing == 0) with_input_from_stdin();
#if defined(BUFFERED_INPUT)
  else if (interactive == 0)
...
static void shell_initialize() {
  ...
  /* Initialize the stuff in current_user that comes from the password
     ...
     interactive. */
  if (interactive_shell) get_current_user_info();
