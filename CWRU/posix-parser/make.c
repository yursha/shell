...
WORD_DESC *make_word(string) char *string;
{
  WORD_DESC *temp;
...
WORD_LIST *add_string_to_list(string, list) char *string;
WORD_LIST *list;
{
...
WORD_DESC *coerce_to_word(number) int number;
{
  char string[24];
...
COMMAND *make_for_command(name, map_list, action) WORD_DESC *name;
WORD_LIST *map_list;
COMMAND *action;
{
...
COMMAND *make_case_command(word, clauses) WORD_DESC *word;
...
COMMAND *make_until_or_while(test, action, which) COMMAND *test, *action;
...
COMMAND *new_make_simple_command(word, prefix, suffix) WORD_DESC *word;
ELEMENT *prefix;
ELEMENT *suffix;
{
...
make_here_document(temp) REDIRECT *temp;
{
  int kill_leading = 0;
  ...
  switch (temp->instruction) {
    ...
    case r_reading_until: /* <<foo */
         ...
         list of lines in the document. */
      {
        ...
        while (line = read_secondary_line()) {
          ...
          if (l + document_index >= document_size) {
            document = (char *)xrealloc(document, (document_size += (10 * l)));
          }
      ...
      document_done:
        if (!document) document = savestring("");
...
REDIRECT *make_redirection(source, instruction,
                           dest) enum r_instruction instruction;
{
...
COMMAND *make_function_def(name, command) WORD_DESC *name;
COMMAND *command;
{
  FUNCTION_DEF *temp;
...
COMMAND *clean_simple_command(command) COMMAND *command;
{
  ...
  if (command->type != cm_simple) {
    programming_error("clean_simple_command () got a command with type %d.",
                      command->type);
  } else {
...
char **make_word_array(list) WORD_LIST *list;
{
