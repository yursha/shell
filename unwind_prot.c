...
void uwp_init() { ocache_create(uwcache, UNWIND_ELT, UWCACHESIZE); }
...
void remove_unwind_protect() {
  if (unwind_protect_list)
    without_interrupts(remove_unwind_protect_internal, (char *)NULL,
                       (char *)NULL);
}
...
void run_unwind_protects() {
  if (unwind_protect_list)
    without_interrupts(run_unwind_protects_internal, (char *)NULL,
                       (char *)NULL);
}
...
int have_unwind_protects() { return (unwind_protect_list != 0); }
...
int unwind_protect_tag_on_stack(tag) const char *tag;
{
  ...
  while (elt) {
    if (elt->head.cleanup == 0 && STREQ(elt->arg.v, tag)) return 1;
    elt = elt->head.next;
  }
  return 0;
}
...
static void remove_unwind_protect_internal(ignore1, ignore2) char *ignore1,
    *ignore2;
{
  UNWIND_ELT *elt;
...
static void run_unwind_protects_internal(ignore1, ignore2) char *ignore1,
    *ignore2;
...
void print_unwind_protect_tags() {
  ...
  elt = unwind_protect_list;
  while (elt) {
    if (elt->head.cleanup == 0) fprintf(stderr, "tag: %s\n", elt->arg.v);
    elt = elt->head.next;
  }
}
#endif
