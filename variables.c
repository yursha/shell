...
#if defined(ARRAY_VARS)
static SHELL_VAR *init_dynamic_array_var(char *, sh_var_value_func_t *,
                                         sh_var_assign_func_t *, int);
static SHELL_VAR *init_dynamic_assoc_var(char *, sh_var_value_func_t *,
                                         sh_var_assign_func_t *, int);
#endif
...
static void sbrand(unsigned long); /* set bash random number generator. */
static void seedrand(void);        /* seed generator randomly */
...
static SHELL_VAR *assign_comp_wordbreaks(SHELL_VAR *, char *, arrayind_t,
                                         char *);
#endif
...
static SHELL_VAR *bind_variable_internal(const char *, char *, HASH_TABLE *,
                                         int, int);
...
static SHELL_VAR *find_variable_nameref_context(SHELL_VAR *, VAR_CONTEXT *,
                                                VAR_CONTEXT **);
static SHELL_VAR *find_variable_last_nameref_context(SHELL_VAR *, VAR_CONTEXT *,
                                                     VAR_CONTEXT **);
...
int privmode;
{
  ...
  for (string_index = 0; env && (string = env[string_index++]);) {
    ...
    while ((c = *string++) && c != '=')
      ;
    if (string[-1] == '=') char_index = string - name - 1;
    ...
    if (privmode == 0 && read_but_dont_execute == 0 &&
      ...
      if (absolute_program(tname) == 0 &&
        ...
        parse_and_execute(temp_string, tname, SEVAL_NONINT | SEVAL_NOHIST |
      ...
      if (posixly_correct && STREQ(name, "SHELLOPTS")) {
        ...
        if (temp_var) VUNSETATTR(temp_var, att_readonly);
      }
      ...
      if (temp_var) {
        if (legal_identifier(name))
          ...
          VSETATTR(temp_var, (att_exported | att_imported | att_invisible));
        if (ro) VSETATTR(temp_var, att_readonly);
        array_needs_making = 1;
      }
    }
  ...
  /* set node id -- don't import it from the environment */
  {
    ...
    if (temp_var) set_auto_export(temp_var);
  }
#endif
...
#else
    if (current_user.uid == -1) get_current_user_info();
  ...
  if (!temp_var) temp_var = find_variable("POSIX_PEDANTIC");
  if (temp_var && imported_p(temp_var)) sv_strict_posix(temp_var->name);
  ...
  if (interactive_shell) {
    ...
    if (!temp_var) temp_var = find_variable("ignoreeof");
    if (temp_var && imported_p(temp_var)) sv_ignoreeof(temp_var->name);
  }
 ...
 */
  ...
  if (temp_var && imported_p(temp_var)) sv_xtracefd(temp_var->name);
...
char *sh_get_home_dir() {
  if (current_user.home_dir == 0) get_current_user_info();
...
static void set_home_var() {
  ...
  if (temp_var == 0) temp_var = bind_variable("HOME", sh_get_home_dir(), 0);
#if 0
...
static void set_shell_var() {
  ...
  if (temp_var == 0) {
    if (current_user.shell == 0) get_current_user_info();
...
static void initialize_shell_level() { adjust_shell_level(1); }
...
void set_ppid() {
  ...
  if (temp_var) VUNSETATTR(temp_var, (att_readonly | att_exported));
...
void print_var_list(list) register SHELL_VAR **list;
{
  ...
  for (i = 0; list && (var = list[i]); i++)
    if (invisible_p(var) == 0) print_assignment(var);
}
...
static SHELL_VAR *null_assign(self, value, unused, key) SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
{ return (self); }
...
#if defined(ARRAY_VARS)
static SHELL_VAR *null_array_assign(self, value, ind, key) SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
{ return (self); }
...
/* Degenerate `dynamic_value' function; just returns what's passed without
   manipulation. */
static SHELL_VAR *get_self(self) SHELL_VAR *self;
{ return (self); }

#if defined(ARRAY_VARS)
/* A generic dynamic array variable initializer.  Initialize array variable
   NAME with dynamic value function GETFUNC and assignment function SETFUNC. */
static SHELL_VAR *init_dynamic_array_var(name, getfunc, setfunc,
                                         attrs) char *name;
...
int attrs;
{
  ...
  if (attrs) VSETATTR(v, attrs);
  return v;
}

static SHELL_VAR *init_dynamic_assoc_var(name, getfunc, setfunc,
                                         attrs) char *name;
sh_var_value_func_t *getfunc;
sh_var_assign_func_t *setfunc;
int attrs;
...
static SHELL_VAR *assign_seconds(self, value, unused, key) SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
{
...
static SHELL_VAR *get_seconds(var) SHELL_VAR *var;
{
...
static void sbrand(seed) unsigned long seed;
{
  rseed = seed;
  last_random_value = 0;
}
...
char *key;
{
  ...
  if (subshell_environment) seeded_subshell = getpid();
  return (self);
}
...
static SHELL_VAR *get_random(var) SHELL_VAR *var;
{
  int rv;
  char *p;
...
static SHELL_VAR *assign_lineno(var, value, unused, key) SHELL_VAR *var;
char *value;
arrayind_t unused;
char *key;
{
...
static SHELL_VAR *get_lineno(var) SHELL_VAR *var;
{
  char *p;
  int ln;
...
static SHELL_VAR *assign_subshell(var, value, unused, key) SHELL_VAR *var;
char *value;
arrayind_t unused;
char *key;
{
...
static SHELL_VAR *get_subshell(var) SHELL_VAR *var;
{
  char *p;
...
static SHELL_VAR *get_bashpid(var) SHELL_VAR *var;
{
  int pid;
  char *p;
...
static SHELL_VAR *get_bash_command(var) SHELL_VAR *var;
{
  char *p;
...
#if defined(HISTORY)
static SHELL_VAR *get_histcmd(var) SHELL_VAR *var;
{
  char *p;
...
static SHELL_VAR *get_comp_wordbreaks(var) SHELL_VAR *var;
{
...
/* When this function returns, rl_completer_word_break_characters points to
   malloced memory. */
static SHELL_VAR *assign_comp_wordbreaks(self, value, unused,
                                         key) SHELL_VAR *self;
char *value;
arrayind_t unused;
char *key;
...
#if defined(PUSHD_AND_POPD) && defined(ARRAY_VARS)
static SHELL_VAR *assign_dirstack(self, value, ind, key) SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
{
...
static SHELL_VAR *get_dirstack(self) SHELL_VAR *self;
{
  ARRAY *a;
  WORD_LIST *l;
...
static SHELL_VAR *get_groupset(self) SHELL_VAR *self;
{
  register int i;
  int ng;
  ARRAY *a;
  ...
  if (group_set == 0) {
    ...
    a = array_cell(self);
    for (i = 0; i < ng; i++) array_insert(a, i, group_set[i]);
  }
  return (self);
}

static SHELL_VAR *build_hashcmd(self) SHELL_VAR *self;
{
  HASH_TABLE *h;
  int i;
  char *k, *v;
...
static SHELL_VAR *get_hashcmd(self) SHELL_VAR *self;
{
  build_hashcmd(self);
  return (self);
}

static SHELL_VAR *assign_hashcmd(self, value, ind, key) SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
{
...
#if defined(ALIAS)
static SHELL_VAR *build_aliasvar(self) SHELL_VAR *self;
{
  HASH_TABLE *h;
  int i;
  char *k, *v;
...
static SHELL_VAR *get_aliasvar(self) SHELL_VAR *self;
{
  build_aliasvar(self);
  return (self);
}

static SHELL_VAR *assign_aliasvar(self, value, ind, key) SHELL_VAR *self;
char *value;
arrayind_t ind;
char *key;
{
...
static SHELL_VAR *get_funcname(self) SHELL_VAR *self;
{
#if !defined(ARRAY_VARS)
  char *t;
...
void make_funcname_visible(on_or_off) int on_or_off;
{
  ...
  if (v == 0 || v->dynamic_value == 0) return;
...
HASH_TABLE *hashed_vars;
{
  ...
  /* If we find the name in HASHED_VARS, set LAST_TABLE_SEARCHED to that
     table. */
  if (bucket) last_table_searched = hashed_vars;
...
VAR_CONTEXT *vcontext;
{
  ...
  for (vc = vcontext; vc; vc = vc->down)
    if (v = hash_lookup(name, vc->table)) break;
...
int flags;
{
  ...
  if (search_tempenv && temporary_env) var = hash_lookup(name, temporary_env);
...
SHELL_VAR *find_variable_nameref(v) SHELL_VAR *v;
{
  int level, flags;
  char *newname, *t;
  SHELL_VAR *orig, *oldv;
  ...
  while (v && nameref_p(v)) {
    level++;
    if (level > NAMEREF_MAX) return ((SHELL_VAR *)0); /* error message here? */
    newname = nameref_cell(v);
    if (newname == 0 || *newname == '\0') return ((SHELL_VAR *)0);
    oldv = v;
    flags = 0;
...
static SHELL_VAR *find_nameref_at_context(v, vc) SHELL_VAR *v;
VAR_CONTEXT *vc;
{
  SHELL_VAR *nv, *nv2;
  VAR_CONTEXT *nvc;
  ...
  while (nv && nameref_p(nv)) {
    level++;
    if (level > NAMEREF_MAX) return (&nameref_maxloop_value);
    newname = nameref_cell(nv);
    if (newname == 0 || *newname == '\0') return ((SHELL_VAR *)NULL);
...
static SHELL_VAR *find_variable_nameref_context(v, vc, nvcp) SHELL_VAR *v;
VAR_CONTEXT *vc;
VAR_CONTEXT **nvcp;
{
  SHELL_VAR *nv, *nv2;
  ...
  for (nv = v, nvc = vc; nvc; nvc = nvc->down) {
    ...
    if (nv2 == &nameref_maxloop_value) return (nv2); /* XXX */
    if (nv2 == 0) continue;
    nv = nv2;
    if (*nvcp) *nvcp = nvc;
    if (nameref_p(nv) == 0) break;
  }
...
/* Do nameref resolution from the VC, which is the local context for some
   ...
   nameref resolution (so the bind_variable_internal can use the correct
   variable context and hash table). */
static SHELL_VAR *find_variable_last_nameref_context(v, vc, nvcp) SHELL_VAR *v;
VAR_CONTEXT *vc;
VAR_CONTEXT **nvcp;
{
  SHELL_VAR *nv, *nv2;
...
SHELL_VAR *find_variable_tempenv(name) const char *name;
{
  ...
  if (var && nameref_p(var)) var = find_variable_nameref(var);
  return (var);
}
...
SHELL_VAR *find_variable(name) const char *name;
{
  ...
  if (v && nameref_p(v)) v = find_variable_nameref(v);
  return v;
}
...
char *get_variable_value(var) SHELL_VAR *var;
{
  if (var == 0) return ((char *)NULL);
#if defined(ARRAY_VARS)
  else if (array_p(var))
...
SHELL_VAR *make_local_variable(name) const char *name;
{
  ...
  if (old_ref && nameref_p(old_ref) == 0) old_ref = 0;
  ...
  if (was_tmpvar) tmp_value = value_cell(old_var);
  ...
  if (old_var &&
      (noassign_p(old_var) || (readonly_p(old_var) && old_var->context == 0))) {
  ...
  if (old_var == 0)
    ...
    if (was_tmpvar) var_setvalue(new_var, savestring(tmp_value));
  ...
  if (ifsname(name)) setifs(new_var);
...
#if defined(ARRAY_VARS)
SHELL_VAR *make_new_array_variable(name) char *name;
{
  SHELL_VAR *entry;
  ARRAY *array;
...
SHELL_VAR *make_local_array_variable(name, assoc_ok) char *name;
int assoc_ok;
{
  SHELL_VAR *var;
  ARRAY *array;
  ...
  if (var == 0 || array_p(var) || (assoc_ok && assoc_p(var))) return var;
...
SHELL_VAR *make_new_assoc_variable(name) char *name;
{
  SHELL_VAR *entry;
  HASH_TABLE *hash;
...
SHELL_VAR *make_local_assoc_variable(name) char *name;
{
  SHELL_VAR *var;
  HASH_TABLE *hash;

  var = make_local_variable(name);
  if (var == 0 || assoc_p(var)) return var;
...
char *make_variable_value(var, value, flags) SHELL_VAR *var;
char *value;
int flags;
{
  char *retval, *oval;
  ...
  if (integer_p(var)) {
    ...
    if (flags & ASS_APPEND) {
      ...
      strcpy(retval, oval);
      if (value) strcpy(retval + olen, value);
    } else if (*value)
...
static SHELL_VAR *bind_variable_internal(name, value, table, hflags,
                                         aflags) const char *name;
char *value;
HASH_TABLE *table;
int hflags, aflags;
{
  ...
  if (entry && nameref_p(entry) && (invisible_p(entry) == 0) &&
    ...
    /* Let's see if we have a nameref referencing a variable that hasn't yet
       been created. */
    if (entry == 0) entry = find_variable_last_nameref(name, 0); /* XXX */
  ...
  if (entry && invisible_p(entry) && nameref_p(entry)) {
    ...
    if (valid_array_reference(newval, 0)) {
      ...
      entry = assign_array_element(newval, make_variable_value(entry, value, 0),
                                   aflags | ASS_NAMEREF);
    ...
    newval = (aflags & ASS_APPEND) ? make_variable_value(entry, value, aflags)
                                   : value;
    if (assoc_p(entry))
    ...
    if (newval != value) free(newval);
    return (entry);
  } else {
  assign_value:
    if ((readonly_p(entry) && (aflags & ASS_FORCE) == 0) || noassign_p(entry)) {
      if (readonly_p(entry)) err_readonly(name_cell(entry));
      return (entry);
    }
  ...
  if (mark_modified_vars) VSETATTR(entry, att_exported);

  if (exported_p(entry)) array_needs_making = 1;
        ...
        if (nv == 0) {
          ...
          if (nv && nameref_p(nv)) {
            ...
            if (nameref_cell(nv) == 0)
              return (bind_variable_internal(nv->name, value, nvc->table, 0,
                                             flags));
#if defined(ARRAY_VARS)
...
SHELL_VAR *bind_variable_value(var, value, aflags) SHELL_VAR *var;
char *value;
int aflags;
{
  char *t;
  ...
  if (mark_modified_vars) VSETATTR(var, att_exported);

  if (exported_p(var)) array_needs_making = 1;
...
SHELL_VAR *bind_int_variable(lhs, rhs) char *lhs, *rhs;
{
  register SHELL_VAR *v;
...
#if defined(ARRAY_VARS)
    if (array_p(v) && isarr == 0) implicitarray = 1;
#endif
  }
  ...
  if (v) {
    if (isint) VSETATTR(v, att_integer);
    VUNSETATTR(v, att_invisible);
  }
...
SHELL_VAR *bind_var_to_int(var, val) char *var;
intmax_t val;
{
...
COMMAND *value;
{
  ...
  if (var_isset(entry)) dispose_command(function_cell(entry));
...
void bind_function_def(name, value) const char *name;
FUNCTION_DEF *value;
{
  FUNCTION_DEF *entry;
  BUCKET_CONTENTS *elt;
...
int assign_in_env(word, flags) WORD_DESC *word;
int flags;
{
  int offset, aflags;
  ...
  if (name[offset] == '=') {
    ...
    if (var && (readonly_p(var) || noassign_p(var))) {
      if (readonly_p(var)) err_readonly(name);
      free(name);
      return (0);
    }
    temp = name + offset + 1;
  ...
  if (temporary_env == 0) temporary_env = hash_create(TEMPENV_HASH_BUCKETS);
  ...
  if (flags) stupidly_hack_special_variables(newname);
...
SHELL_VAR *copy_variable(var) SHELL_VAR *var;
{
  ...
  if (var) {
    ...
    if (function_p(var)) var_setfunc(copy, copy_command(function_cell(var)));
#if defined(ARRAY_VARS)
    else if (array_p(var))
...
{
  if (function_p(var)) dispose_command(function_cell(var));
#if defined(ARRAY_VARS)
  else if (array_p(var))
...
void dispose_variable(var) SHELL_VAR *var;
{
  ...
  if (nofree_p(var) == 0) dispose_variable_value(var);
...
int unbind_nameref(name) const char *name;
{
  ...
  v = var_lookup(name, shell_variables);
  if (v && nameref_p(v)) return makunbound(name, shell_variables);
  return 0;
}
...
int unbind_variable_noref(name) const char *name;
{
  ...
  v = var_lookup(name, shell_variables);
  if (v) return makunbound(name, shell_variables);
  return 0;
}
...
int unbind_func(name) const char *name;
{
  ...
  if (func) {
    if (exported_p(func)) array_needs_making++;
...
int unbind_function_def(name) const char *name;
{
  ...
  if (funcdef) dispose_function_def(funcdef);
...
VAR_CONTEXT *vc;
{
  ...
  for (elt = (BUCKET_CONTENTS *)NULL, v = vc; v; v = v->down)
    if (elt = hash_remove(name, v->table, 0)) break;
...
VAR_CONTEXT *vc;
{
  ...
  if (old_var && exported_p(old_var)) array_needs_making++;
  ...
  if (old_var && local_p(old_var) && variable_context == old_var->context) {
    if (nofree_p(old_var)) var_setvalue(old_var, (char *)NULL);
#if defined(ARRAY_VARS)
...
void set_func_read_only(name) const char *name;
{
  ...
  entry = find_function(name);
  if (entry) VSETATTR(entry, att_readonly);
}
...
void set_func_auto_export(name) const char *name;
{
  ...
  entry = find_function(name);
  if (entry) set_auto_export(entry);
}
#endif
...
static VARLIST *vlist_alloc(nentries) int nentries;
{
  VARLIST *vlist;
...
static VARLIST *vlist_realloc(vlist, n) VARLIST *vlist;
int n;
{
  if (vlist == 0) return (vlist = vlist_alloc(n));
...
int flags;
{
  ...
  for (i = 0; i < vlist->list_len; i++)
    if (STREQ(var->name, vlist->list[i]->name)) break;
  if (i < vlist->list_len) return;
...
SHELL_VAR **map_over(function, vc) sh_var_map_func_t *function;
VAR_CONTEXT *vc;
{
  VAR_CONTEXT *v;
  VARLIST *vlist;
  ...
  for (nentries = 0, v = vc; v; v = v->down) nentries += HASH_ENTRIES(v->table);

  if (nentries == 0) return (SHELL_VAR **)NULL;

  vlist = vlist_alloc(nentries);

  for (v = vc; v; v = v->down) flatten(v->table, function, vlist, 0);
...
SHELL_VAR **map_over_funcs(function) sh_var_map_func_t *function;
{
  VARLIST *vlist;
  SHELL_VAR **ret;
...
static void flatten(var_hash_table, func, vlist,
                    flags) HASH_TABLE *var_hash_table;
...
int flags;
{
  ...
  for (i = 0; i < var_hash_table->nbuckets; i++) {
    for (tlist = hash_items(i, var_hash_table); tlist; tlist = tlist->next) {
      ...
      r = func ? (*func)(var) : 1;
      if (r && vlist) vlist_add(vlist, var, flags);
    }
  }
}
...
static int qsort_var_comp(var1, var2) SHELL_VAR **var1, **var2;
{
  int result;
...
static SHELL_VAR **vapply(func) sh_var_map_func_t *func;
{
  SHELL_VAR **list;

  list = map_over(func, shell_variables);
  if (list /* && posixly_correct */) sort_variables(list);
  return (list);
}

/* Apply FUNC to each variable in SHELL_FUNCTIONS, adding each one for
   which FUNC succeeds to an array of SHELL_VAR *s.  Returns the array. */
static SHELL_VAR **fapply(func) sh_var_map_func_t *func;
{
  SHELL_VAR **list;
...
static int visible_var(var) SHELL_VAR *var;
{ return (invisible_p(var) == 0); }

SHELL_VAR **all_visible_functions() { return (fapply(visible_var)); }

SHELL_VAR **all_visible_variables() { return (vapply(visible_var)); }

/* Return non-zero if the variable VAR is visible and exported.  Array
   variables cannot be exported. */
static int visible_and_exported(var) SHELL_VAR *var;
{ return (invisible_p(var) == 0 && exported_p(var)); }

/* Candidate variables for the export environment are either valid variables
   with the export attribute or invalid variables inherited from the initial
   environment and simply passed through. */
static int export_environment_candidate(var) SHELL_VAR *var;
{ return (exported_p(var) && (invisible_p(var) == 0 || imported_p(var))); }

/* Return non-zero if VAR is a local variable in the current context and
   is exported. */
static int local_and_exported(var) SHELL_VAR *var;
{
...
SHELL_VAR **local_exported_variables() { return (vapply(local_and_exported)); }

static int variable_in_context(var) SHELL_VAR *var;
{
  return (invisible_p(var) == 0 && local_p(var) &&
...
SHELL_VAR **all_local_variables() {
  ...
  ret = vlist->list;
  free(vlist);
  if (ret) sort_variables(ret);
  return ret;
}

#if defined(ARRAY_VARS)
/* Return non-zero if the variable VAR is visible and an array. */
static int visible_array_vars(var) SHELL_VAR *var;
{ return (invisible_p(var) == 0 && array_p(var)); }

SHELL_VAR **all_array_variables() { return (vapply(visible_array_vars)); }
#endif /* ARRAY_VARS */
...
char **all_variables_matching_prefix(prefix) const char *prefix;
{
  ...
  for (vind = 0; varlist && varlist[vind]; vind++)
    ;
  if (varlist == 0 || vind == 0) return ((char **)NULL);
...
static void push_temp_var(data) void *data;
{
  ...
  if (binding_table == 0) {
    if (shell_variables == global_variables) /* shouldn't happen */
  ...
  if (v) v->attributes |= var->attributes;
...
static inline char *mk_env_string(name, value, isfunc) const char *name, *value;
int isfunc;
{
...
#ifdef DEBUG
/* Debugging */
static int valid_exportstr(v) SHELL_VAR *v;
{
  char *s;
...
static char **make_env_array_from_var_list(vars) SHELL_VAR **vars;
{
...
#define USE_EXPORTSTR (value == var->exportstr)
  ...
  for (i = 0, list_index = 0; var = vars[i]; i++) {
    ...
    if (value) {
      ...
      list[list_index] = USE_EXPORTSTR
                             ? savestring(value)
                             : mk_env_string(var->name, value, function_p(var));

      if (USE_EXPORTSTR == 0) SAVE_EXPORTSTR(var, list[list_index]);
...
static char **make_var_export_array(vcxt) VAR_CONTEXT *vcxt;
{
  char **list;
  SHELL_VAR **vars;
...
char **add_or_supercede_exported_var(assign, do_alloc) char *assign;
int do_alloc;
{
  register int i;
  int equal_offset;
  ...
  if (equal_offset == 0) return (export_env);
...
static void add_temp_array_to_env(temp_array, do_alloc,
                                  do_supercede) char **temp_array;
int do_alloc, do_supercede;
{
  register int i;

  if (temp_array == 0) return;
...
void maybe_make_export_env() {
  ...
  if (array_needs_making) {
    if (export_env) strvec_flush(export_env);
...
char *value;
{
  ...
  if (value) strcpy(evar + preflen, value);
...
VAR_CONTEXT *new_var_context(name, flags) char *name;
int flags;
{
  VAR_CONTEXT *vc;
...
static int set_context(var) SHELL_VAR *var;
{ return (var->context = variable_context); }

/* Make a new variable context with NAME and FLAGS and a HASH_TABLE of
   temporary variables, and push it onto shell_variables.  This is
   for shell functions. */
VAR_CONTEXT *push_var_context(name, flags, tempvars) char *name;
int flags;
HASH_TABLE *tempvars;
{
  VAR_CONTEXT *vc;
...
static void push_func_var(data) void *data;
{
  ...
  else if (tempvar_p(var) &&
           (posixly_correct || (var->attributes & att_propagate))) {
...
VAR_CONTEXT *push_scope(flags, tmpvars) int flags;
HASH_TABLE *tmpvars;
...
/* XXX - This isn't exactly right, because all tempenv variables have the
...
#endif
  {
    ...
    v = bind_variable_internal(var->name, value_cell(var),
                               shell_variables->table, 0, 0);
    if (shell_variables == global_variables) var->attributes &= ~att_propagate;
    if (v) v->attributes |= var->attributes;
  }
  else stupidly_hack_special_variables(var->name); /* XXX */
...
void pop_scope(is_special) int is_special;
{
  ...
  ret = vcxt->down;
  if (ret) ret->up = (VAR_CONTEXT *)NULL;
...
HASH_TABLE *tempvars;
{
  if (is_subshell == 0) push_dollar_vars();
  variable_context++;
...
void push_dollar_vars() {
  if (dollar_arg_stack_index + 2 > dollar_arg_stack_slots) {
    dollar_arg_stack = (WORD_LIST **)xrealloc(
        dollar_arg_stack, (dollar_arg_stack_slots += 10) * sizeof(WORD_LIST *));
  }
...
void pop_dollar_vars() {
  if (!dollar_arg_stack || dollar_arg_stack_index == 0) return;
...
void pop_args() {
  ...
  if (ce == 0 || legal_number(element_value(ce), &i) == 0) i = 0;

  for (; i > 0; i--) array_pop(bash_argv_a);
...
static int sv_compare(sv1, sv2) struct name_and_function *sv1, *sv2;
{
  int r;

  if ((r = sv1->name[0] - sv2->name[0]) == 0) r = strcmp(sv1->name, sv2->name);
  return r;
}
...
static inline int find_special_var(name) const char *name;
{
  ...
  for (i = 0; special_vars[i].name; i++) {
    ...
    if (r == 0) r = strcmp(special_vars[i].name, name);
    if (r == 0)
      return i;
    else if (r > 0)
...
void stupidly_hack_special_variables(name) char *name;
{
  ...
  if (i != -1) (*(special_vars[i].function))(name);
}
...
{
  if (privileged_mode == 0) setup_glob_ignore(name);
}
...
void sv_comp_wordbreaks(name) char *name;
{
  ...
  if (sv == 0) reset_completer_word_break_chars();
}
  ...
  if (temp && *temp) {
    if (legal_number(temp, &num)) {
      ...
      if (hmax < 0 && name[4] == 'S')
        ...
        if (hmax < history_lines_in_file) history_lines_in_file = hmax;
      }
    }
  } else if (name[4] == 'S')
    unstifle_history();
...
void sv_histchars(name) char *name;
{
  ...
  if (temp) {
    ...
    if (temp[0] && temp[1]) {
      ...
      if (temp[2]) history_comment_char = temp[2];
    }
  } else {
...
void sv_histtimefmt(name) char *name;
{
  ...
  if (v = find_variable(name)) {
    if (history_comment_char == 0) history_comment_char = '#';
  }
...
#if defined(READLINE)
  if (interactive_shell) posix_readline_initialize(posixly_correct);
#endif             /* READLINE */
...
#if 1
  if (r == 0 && posixly_correct) last_command_exit_value = 1;
#endif
}
...
int nproc;
{
  ...
  if (v == 0) v = make_new_array_variable("PIPESTATUS");
  if (array_p(v) == 0) return; /* Do nothing if not an array variable. */
  a = array_cell(v);
...
ARRAY *save_pipestatus_array() {
  ...
  v = find_variable("PIPESTATUS");
  if (v == 0 || array_p(v) == 0 || array_cell(v) == 0) return ((ARRAY *)NULL);
...
void restore_pipestatus_array(a) ARRAY *a;
{
  ...
  if (v == 0 || array_p(v) == 0 || array_cell(v) == 0) return;
