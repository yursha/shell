...
static char *array_to_string_internal(ARRAY_ELEMENT *, ARRAY_ELEMENT *, char *,
                                      int);
...
ARRAY *array_copy(a) ARRAY *a;
{
  ARRAY *a1;
  ARRAY_ELEMENT *ae, *new;

  if (a == 0) return ((ARRAY *)NULL);
  a1 = array_create();
  a1->type = a->type;
 ...
 * S to E, inclusive.
 */
ARRAY *array_slice(array, s, e) ARRAY *array;
ARRAY_ELEMENT *s, *e;
{
  ARRAY *a;
  ARRAY_ELEMENT *p, *n;
...
void *udata;
{
  ...
  if (a == 0 || array_empty(a)) return;
  for (ae = element_forw(a->head); ae != a->head; ae = element_forw(ae))
    if ((*func)(ae, udata) < 0) return;
}
 ...
 * and returns NULL.
 */
ARRAY_ELEMENT *array_shift(a, n, flags) ARRAY *a;
int n, flags;
{
  register ARRAY_ELEMENT *ae, *ret;
  register int i;

  if (a == 0 || array_empty(a) || n <= 0) return ((ARRAY_ELEMENT *)NULL);
 ...
 * shift.
 */
int array_rshift(a, n, s) ARRAY *a;
int n;
char *s;
{
   ...
   */
  for (; ae != a->head; ae = element_forw(ae)) element_index(ae) += n;
...
ARRAY_ELEMENT *array_unshift_element(a) ARRAY *a;
{ return (array_shift(a, 1, 0)); }

int array_shift_element(a, v) ARRAY *a;
char *v;
{ return (array_rshift(a, 1, v)); }

ARRAY *array_quote(array) ARRAY *array;
{
  ARRAY_ELEMENT *a;
  char *t;
...
ARRAY *array_quote_escapes(array) ARRAY *array;
{
  ARRAY_ELEMENT *a;
  char *t;
...
ARRAY *array_dequote(array) ARRAY *array;
{
  ARRAY_ELEMENT *a;
  char *t;
...
ARRAY *array_dequote_escapes(array) ARRAY *array;
{
  ARRAY_ELEMENT *a;
  char *t;
...
ARRAY *array_remove_quoted_nulls(array) ARRAY *array;
{
  ARRAY_ELEMENT *a;
  char *t;
 ...
 */
char *array_subrange(a, start, nelem, starsub, quoted) ARRAY *a;
...
int starsub, quoted;
{
  ...
  if (starsub && (quoted & (Q_DOUBLE_QUOTES | Q_HERE_DOCUMENT))) {
    ...
    ifs = getifs();
    if (ifs == 0 || *ifs == 0) {
      if (slen < 2) sifs = xrealloc(sifs, 2);
      sifs[0] = ' ';
      sifs[1] = '\0';
    }
    t = array_to_string(a2, sifs, 0);
...
char *array_patsub(a, pat, rep, mflags) ARRAY *a;
char *pat, *rep;
int mflags;
{
  ARRAY *a2;
  ...
  if (a == 0 || array_head(a) == 0 || array_empty(a)) return ((char *)NULL);
...
char *array_modcase(a, pat, modop, mflags) ARRAY *a;
char *pat;
int modop;
int mflags;
{
 ...
 * VALUE.
 */
ARRAY_ELEMENT *array_create_element(indx, value) arrayind_t indx;
char *value;
{
  ARRAY_ELEMENT *r;
...
ARRAY_ELEMENT *array_copy_element(ae) ARRAY_ELEMENT *ae;
{
 ...
 */
int array_insert(a, i, v) ARRAY *a;
arrayind_t i;
char *v;
{
 ...
 */
ARRAY_ELEMENT *array_remove(a, i) ARRAY *a;
arrayind_t i;
{
  register ARRAY_ELEMENT *ae, *start;

  if (a == 0 || array_empty(a)) return ((ARRAY_ELEMENT *)NULL);
  start = LASTREF_START(a, i);
  for (ae = start; ae != a->head; ae = element_forw(ae))
    if (element_index(ae) == i) {
      ...
      a->num_elements--;
      if (i == array_max_index(a)) a->max_index = element_index(ae->prev);
#if 0
			INVALIDATE_LASTREF(a);
#else
      if (ae->next != a->head)
 ...
 */
char *array_reference(a, i) ARRAY *a;
arrayind_t i;
{
  register ARRAY_ELEMENT *ae, *start;

  if (a == 0 || array_empty(a)) return ((char *)NULL);
  if (i > array_max_index(a))
...
WORD_LIST *array_to_word_list(a) ARRAY *a;
{
  WORD_LIST *list;
  ARRAY_ELEMENT *ae;

  if (a == 0 || array_empty(a)) return ((WORD_LIST *)NULL);
  list = (WORD_LIST *)NULL;
  for (ae = element_forw(a->head); ae != a->head; ae = element_forw(ae))
...
ARRAY *array_from_word_list(list) WORD_LIST *list;
{
  ARRAY *a;

  if (list == 0) return ((ARRAY *)NULL);
  a = array_create();
  return (array_assign_list(a, list));
}

WORD_LIST *array_keys_to_word_list(a) ARRAY *a;
{
  WORD_LIST *list;
  ARRAY_ELEMENT *ae;
  char *t;
...
ARRAY *array_assign_list(array, list) ARRAY *array;
WORD_LIST *list;
{
 ...
 */
static char *array_to_string_internal(start, end, sep,
                                      quoted) ARRAY_ELEMENT *start,
    *end;
char *sep;
int quoted;
{
  ...
  for (rsize = rlen = 0, ae = start; ae != end; ae = element_forw(ae)) {
    if (rsize == 0) result = (char *)xmalloc(rsize = 64);
    if (element_value(ae)) {
  ...
  if (result) result[rlen] = '\0'; /* XXX */
  return (result);
}

char *array_to_assign(a, quoted) ARRAY *a;
int quoted;
{
  ...
  for (ae = element_forw(a->head); ae != a->head; ae = element_forw(ae)) {
    ...
    if (element_forw(ae) != a->head) result[rlen++] = ' ';
...
char *array_to_string(a, sep, quoted) ARRAY *a;
char *sep;
int quoted;
{
  if (a == 0) return ((char *)NULL);
  if (array_empty(a)) return (savestring(""));
  return (
 ...
 */
ARRAY *array_from_string(s, sep) char *s, *sep;
{
  ARRAY *a;
  WORD_LIST *w;

  if (s == 0) return ((ARRAY *)NULL);
  w = list_string(s, sep, 0);
  if (w == 0) return ((ARRAY *)NULL);
...
WORD_LIST *make_word_list(x, l) WORD_DESC *x;
WORD_LIST *l;
{
  WORD_LIST *w;
...
WORD_LIST *list_string(s, t, i) char *s, *t;
int i;
{
  char *r, *a;
  WORD_LIST *wl;

  if (s == 0) return (WORD_LIST *)NULL;
  r = savestring(s);
  wl = (WORD_LIST *)NULL;
  a = strtok(r, t);
  while (a) {
...
char *pat_subst(s, t, u, i) char *s, *t, *u;
int i;
{ return ((char *)NULL); }

char *quote_string(s) char *s;
{ return savestring(s); }
