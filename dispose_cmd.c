...
void dispose_command(command) COMMAND *command;
{
  ...
  if (command->redirects) dispose_redirects(command->redirects);
...
{
  if (cond) {
    if (cond->left) dispose_cond_node(cond->left);
    if (cond->right) dispose_cond_node(cond->right);
    if (cond->op) dispose_word(cond->op);
    free(cond);
  }
}
#endif /* COND_COMMAND */
...
void dispose_word_array(array) char **array;
{
  ...
  for (count = 0; array[count]; count++) free(array[count]);
...
void dispose_redirects(list) REDIRECT *list;
{
  ...
  while (list) {
    ...
    if (t->rflags & REDIR_VARASSIGN) dispose_word(t->redirector.filename);
