...
/* Posix and USG systems do not guarantee to restart read () if it is
   ...
   if it returns EINTR. */
int getc_with_restart(stream) FILE *stream;
{
  unsigned char uc;
...
int ungetc_with_restart(c, stream) int c;
FILE *stream;
{
  if (local_index == 0 || c == EOF) return EOF;
...
#define ALLOCATE_BUFFERS(n)                   \
  do {                                        \
    if ((n) >= nbuffers) allocate_buffers(n); \
  } while (0)
...
static void allocate_buffers(n) int n;
{
  ...
  nbuffers = n + 20;
  buffers = (BUFFERED_STREAM **)xrealloc(buffers,
                                         nbuffers * sizeof(BUFFERED_STREAM *));
...
/* Construct and return a BUFFERED_STREAM corresponding to file descriptor
   FD, using BUFFER. */
static BUFFERED_STREAM *make_buffered_stream(fd, buffer, bufsize) int fd;
char *buffer;
size_t bufsize;
{
  BUFFERED_STREAM *bp;
  ...
  if (bufsize == 1) bp->b_flag |= B_UNBUFF;
  if (O_TEXT && (fcntl(fd, F_GETFL) & O_TEXT) != 0) bp->b_flag |= B_TEXT;
  return (bp);
}

/* Allocate a new BUFFERED_STREAM, copy BP to it, and return the new copy. */
static BUFFERED_STREAM *copy_buffered_stream(bp) BUFFERED_STREAM *bp;
{
  BUFFERED_STREAM *nbp;

  if (!bp) return ((BUFFERED_STREAM *)NULL);
...
int set_bash_input_fd(fd) int fd;
{
...
int fd_is_bash_input(fd) int fd;
{
...
int save_bash_input(fd, new_fd) int fd, new_fd;
{
  int nfd;
  ...
  if (buffers[fd]) sync_buffered_stream(fd);
  ...
  if (default_buffered_input == fd) default_buffered_input = nfd;
...
int check_bash_input(fd) int fd;
{
...
int duplicate_buffered_stream(fd1, fd2) int fd1, fd2;
{
  int is_bash_input, m;
  ...
  if (buffers[fd2]) buffers[fd2]->b_fd = fd2;

  if (is_bash_input) {
    if (!buffers[fd2]) fd_to_buffered_stream(fd2);
...
BUFFERED_STREAM *fd_to_buffered_stream(fd) int fd;
{
  char *buffer;
  size_t size;
  struct stat sb;
...
BUFFERED_STREAM *open_buffered_stream(file) char *file;
{
  int fd;
...
void free_buffered_stream(bp) BUFFERED_STREAM *bp;
{
  ...
  n = bp->b_fd;
  if (bp->b_buffer) free(bp->b_buffer);
  free(bp);
...
int close_buffered_stream(bp) BUFFERED_STREAM *bp;
{
  int fd;
...
int close_buffered_fd(fd) int fd;
{
  if (fd < 0) {
    errno = EBADF;
    return -1;
  }
  if (fd >= nbuffers || !buffers || !buffers[fd]) return (close(fd));
  return (close_buffered_stream(buffers[fd]));
}

/* Make the BUFFERED_STREAM associated with buffers[FD] be BP, and return
   the old BUFFERED_STREAM. */
BUFFERED_STREAM *set_buffered_stream(fd, bp) int fd;
BUFFERED_STREAM *bp;
{
  BUFFERED_STREAM *ret;
...
static int b_fill_buffer(bp) BUFFERED_STREAM *bp;
{
  ssize_t nr;
  off_t o;
...
static int bufstream_ungetc(c, bp) int c;
BUFFERED_STREAM *bp;
{
  if (c == EOF || bp->b_inputp == 0) return (EOF);
...
int sync_buffered_stream(bfd) int bfd;
{
  BUFFERED_STREAM *bp;
  off_t chars_left;

  if (buffers == 0 || (bp = buffers[bfd]) == 0) return (-1);

  chars_left = bp->b_used - bp->b_inputp;
  if (chars_left) lseek(bp->b_fd, -chars_left, SEEK_CUR);
...
int buffered_ungetchar(c) int c;
...
#if defined(TEST)
void *xmalloc(s) int s;
{ return (malloc(s)); }

void *xrealloc(s, size) char *s;
int size;
{
  if (!s)
    return (malloc(size));
...
process(bp) BUFFERED_STREAM *bp;
{
  ...
  while ((c = bufstream_getc(bp)) != EOF) putchar(c);
}
