...
int assoc_insert(hash, key, value) HASH_TABLE *hash;
char *key;
char *value;
{
  BUCKET_CONTENTS *b;
...
char *assoc_reference(hash, string) HASH_TABLE *hash;
char *string;
{
  BUCKET_CONTENTS *b;

  if (hash == 0) return (char *)0;
...
/* Quote the data associated with each element of the hash table ASSOC,
   using quote_string */
HASH_TABLE *assoc_quote(h) HASH_TABLE *h;
{
  int i;
  BUCKET_CONTENTS *tlist;
  char *t;

  if (h == 0 || assoc_empty(h)) return ((HASH_TABLE *)NULL);
...
HASH_TABLE *assoc_quote_escapes(h) HASH_TABLE *h;
{
  int i;
  BUCKET_CONTENTS *tlist;
  char *t;
...
HASH_TABLE *assoc_dequote(h) HASH_TABLE *h;
{
  int i;
  BUCKET_CONTENTS *tlist;
  char *t;
...
HASH_TABLE *assoc_dequote_escapes(h) HASH_TABLE *h;
{
  int i;
  BUCKET_CONTENTS *tlist;
  char *t;
...
HASH_TABLE *assoc_remove_quoted_nulls(h) HASH_TABLE *h;
{
  int i;
  BUCKET_CONTENTS *tlist;
  char *t;
 ...
 */
char *assoc_subrange(hash, start, nelem, starsub, quoted) HASH_TABLE *hash;
...
int starsub, quoted;
{
  ...
  if (assoc_empty(hash)) return ((char *)NULL);

  save = l = assoc_to_word_list(hash);
  if (save == 0) return ((char *)NULL);

  for (i = 1; l && i < start; i++) l = l->next;
  if (l == 0) {
    dispose_words(save);
    return ((char *)NULL);
  }
...
char *assoc_patsub(h, pat, rep, mflags) HASH_TABLE *h;
char *pat, *rep;
int mflags;
{
  BUCKET_CONTENTS *tlist;
  ...
  if (h == 0 || assoc_empty(h)) return ((char *)NULL);
...
char *assoc_modcase(h, pat, modop, mflags) HASH_TABLE *h;
char *pat;
int modop;
int mflags;
{
...
char *assoc_to_assign(hash, quoted) HASH_TABLE *hash;
int quoted;
{
  char *ret;
  char *istr, *vstr;
  ...
  if (hash == 0 || assoc_empty(hash)) return (char *)0;
...
static WORD_LIST *assoc_to_word_list_internal(h, t) HASH_TABLE *h;
int t;
{
  WORD_LIST *list;
  int i;
  ...
  if (h == 0 || assoc_empty(h)) return ((WORD_LIST *)NULL);
  list = (WORD_LIST *)NULL;
...
WORD_LIST *assoc_to_word_list(h) HASH_TABLE *h;
{ return (assoc_to_word_list_internal(h, 0)); }

WORD_LIST *assoc_keys_to_word_list(h) HASH_TABLE *h;
{ return (assoc_to_word_list_internal(h, 1)); }

char *assoc_to_string(h, sep, quoted) HASH_TABLE *h;
char *sep;
int quoted;
{
  BUCKET_CONTENTS *tlist;
  ...
  if (h == 0) return ((char *)NULL);
  if (assoc_empty(h)) return (savestring(""));
