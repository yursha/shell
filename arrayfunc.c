...
static SHELL_VAR *bind_array_var_internal(SHELL_VAR *, arrayind_t, char *,
                                          char *, int);
static SHELL_VAR *assign_array_element_internal(SHELL_VAR *, char *, char *,
                                                char *, int, char *, int);
...
/* Convert a shell variable to an array variable.  The original value is
   saved as array[0]. */
SHELL_VAR *convert_var_to_array(var) SHELL_VAR *var;
{
  char *oldval;
  ARRAY *array;
  ...
  array = array_create();
  if (oldval) array_insert(array, 0, oldval);
  ...
  if (exported_p(var)) array_needs_making++;
...
/* Convert a shell variable to an array variable.  The original value is
   saved as array[0]. */
SHELL_VAR *convert_var_to_assoc(var) SHELL_VAR *var;
{
  char *oldval;
  HASH_TABLE *hash;
  ...
  oldval = value_cell(var);
  hash = assoc_create(0);
  if (oldval) assoc_insert(hash, savestring("0"), oldval);
...
char *make_array_variable_value(entry, ind, key, value, flags) SHELL_VAR *entry;
arrayind_t ind;
char *key;
char *value;
int flags;
...
static SHELL_VAR *bind_array_var_internal(entry, ind, key, value,
                                          flags) SHELL_VAR *entry;
arrayind_t ind;
char *key;
char *value;
int flags;
...
/* Perform an array assignment name[ind]=value.  If NAME already exists and
   ...
   IND's value may be. */
SHELL_VAR *bind_array_variable(name, ind, value, flags) char *name;
arrayind_t ind;
char *value;
int flags;
{
  ...
  if (entry == (SHELL_VAR *)0) {
    ...
    if (entry == INVALID_NAMEREF_VALUE) return ((SHELL_VAR *)0);
  ...
  if (entry == (SHELL_VAR *)0)
    entry = make_new_array_variable(name);
           ...
           noassign_p(entry)) {
    if (readonly_p(entry)) err_readonly(name);
    return (entry);
...
SHELL_VAR *bind_array_element(entry, ind, value, flags) SHELL_VAR *entry;
arrayind_t ind;
char *value;
int flags;
{ return (bind_array_var_internal(entry, ind, 0, value, flags)); }

SHELL_VAR *bind_assoc_variable(entry, name, key, value, flags) SHELL_VAR *entry;
char *name;
char *key;
char *value;
int flags;
...
SHELL_VAR *assign_array_element(name, value, flags) char *name, *value;
int flags;
{
  char *sub, *vname;
  int sublen;
  ...
  if (vname == 0) return ((SHELL_VAR *)NULL);
...
static SHELL_VAR *assign_array_element_internal(entry, name, vname, sub, sublen,
                                                value, flags) SHELL_VAR *entry;
...
SHELL_VAR *find_or_make_array_variable(name, flags) char *name;
int flags;
{
  SHELL_VAR *var;
...
SHELL_VAR *assign_array_from_string(name, value, flags) char *name, *value;
int flags;
{
  SHELL_VAR *var;
  int vflags;
...
/* Sequentially assign the indices of indexed array variable VAR from the
   words in LIST. */
SHELL_VAR *assign_array_var_from_word_list(var, list, flags) SHELL_VAR *var;
WORD_LIST *list;
int flags;
{
...
WORD_LIST *expand_compound_array_assignment(var, value, flags) SHELL_VAR *var;
char *value;
int flags;
{
  ...
  if (*value == '(') /*)*/
    ...
    if (val == 0) return (WORD_LIST *)NULL;
  } else
    val = value;
  ...
  if (list) quote_array_assignment_chars(list);
...
int flags;
{
  ...
  for (list = nlist; list; list = list->next) {
    ...
    if ((list->word->flags & W_ASSIGNMENT) && w[0] == '[') {
      ...
      /* XXX - changes for `+=' -- just accept the syntax.  ksh93 doesn't do
       * this */
      if (w[len + 1] == '+' && w[len + 2] == '=') {
        iflags |= ASS_APPEND;
        val = w + len + 3;
...
SHELL_VAR *assign_array_var_from_string(var, value, flags) SHELL_VAR *var;
char *value;
int flags;
{
  WORD_LIST *nlist;
  ...
  if (nlist) dispose_words(nlist);

  if (var) VUNSETATTR(var, att_invisible); /* no longer invisible */
...
static char *quote_assign(string) const char *string;
{
  ...
  for (s = string; *s;) {
    ...
    if (saw_eq == 0 && (glob_char_p(s) || isifs(*s))) *t++ = '\\';
...
static void quote_array_assignment_chars(list) WORD_LIST *list;
{
  ...
  for (l = list; l; l = l->next) {
    ...
    if ((l->word->flags & W_ASSIGNMENT) == 0) continue;
...
int unbind_array_element(var, sub) SHELL_VAR *var;
char *sub;
{
  int len;
  arrayind_t ind;
  ...
  if (assoc_p(var)) {
    ...
    if (ae) array_dispose_element(ae);
...
int valid_array_reference(name, flags) char *name;
int flags;
{
  char *t;
  int r, len;
  ...
  if (t) {
    ...
    *t = '[';
    ...
    if (t[len] != ']' || len == 1) return 0;
    if (t[len + 1] != '\0') return 0;
    for (r = 1; r < len; r++)
      if (whitespace(t[r]) == 0) return 1;
    return 0;
  }
  return 0;
}
...
arrayind_t array_expand_index(var, s, len) SHELL_VAR *var;
char *s;
int len;
{
  char *exp, *t, *savecmd;
...
char *array_variable_name(s, subp, lenp) char *s, **subp;
int *lenp;
{
  char *t, *ret;
  int ind, ni;
...
SHELL_VAR *array_variable_part(s, subp, lenp) char *s, **subp;
int *lenp;
{
  char *t;
  SHELL_VAR *var;
...
static char *array_value_internal(s, quoted, flags, rtype, indp) char *s;
...
/* Expand the index, even if the variable doesn't exist, in case side
  ...
  if (len == 0) return ((char *)NULL); /* error message already printed */

  /* [ */
  akey = 0;
  if (ALL_ELEMENT_SUB(t[0]) && t[1] == ']') {
    if (rtype) *rtype = (t[0] == '*') ? 1 : 2;
    if ((flags & AV_ALLOWALL) == 0) {
      err_badarraysub(s);
      return ((char *)NULL);
    } else if (var == 0 ||
               value_cell(var) == 0) /* XXX - check for invisible_p(var) ? */
      return ((char *)NULL);
    else if (array_p(var) == 0 && assoc_p(var) == 0)
      l = add_string_to_list(value_cell(var), (WORD_LIST *)NULL);
    else if (assoc_p(var)) {
      l = assoc_to_word_list(assoc_cell(var));
      if (l == (WORD_LIST *)NULL) return ((char *)NULL);
    } else {
...
char *array_value(s, quoted, flags, rtype, indp) char *s;
int quoted, flags, *rtype;
arrayind_t *indp;
...
/* Return the value of the array indexing expression S as a single string.
   ...
   evaluator in expr.c. */
char *get_array_value(s, flags, rtype, indp) char *s;
int flags, *rtype;
arrayind_t *indp;
{ return (array_value_internal(s, 0, flags, rtype, indp)); }

char *array_keys(s, quoted) char *s;
int quoted;
{
  int len;
  char *retval, *t, *temp;
  ...
  if (var_isset(var) == 0 || invisible_p(var)) return (char *)NULL;
