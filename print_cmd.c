...
#define PRINT_DEFERRED_HEREDOCS(x)                     \
  do {                                                 \
    if (deferred_heredocs) print_deferred_heredocs(x); \
  } while (0)
...
/* Make a string which is the printed representation of the command
   ...
   remain around. */
char *make_command_string(command) COMMAND *command;
{
...
static void make_command_string_internal(command) COMMAND *command;
{
  ...
  if (command == 0)
    ...
    if (command->flags & CMD_TIME_PIPELINE) {
      cprintf("time ");
      if (command->flags & CMD_TIME_POSIX) cprintf("-p ");
    }

    if (command->flags & CMD_INVERT_RETURN) cprintf("! ");
      ...
      case cm_connection:
        ...
        switch (command->value.Connection->connector) {
          ...
          case AND_AND:
            ...
            if (command->value.Connection->second) skip_this_indent++;
            break;
...
char *indirection_level_string() {
  ...
  if (ps4 == 0 || *ps4 == '\0') return (indirection_string);
...
void xtrace_print_assignment(name, value, assign_list, xflags) char *name,
    *value;
int assign_list, xflags;
{
  ...
  if (xflags) fprintf(xtrace_fp, "%s", indirection_level_string());
...
int xtflags;
{
  ...
  if (xtflags) fprintf(xtrace_fp, "%s", indirection_level_string());
...
#if defined(ARITH_FOR_COMMAND)
static void print_arith_for_command(arith_for_command)
...
void xtrace_print_select_command_head(select_command)
...
static void print_case_command(case_command) CASE_COM *case_command;
{
  ...
  if (case_command->clauses) print_case_clauses(case_command->clauses);
  newline("esac");
}
...
{
  if (cond->flags & CMD_INVERT_RETURN) cprintf("! ");
...
/* Return the string representation of the named function.
  ...
  */
char *named_function_string(name, command, flags) char *name;
COMMAND *command;
int flags;
{
  char *result;
  ...
  if (name && *name) {
    if (find_reserved_word(name) >= 0) cprintf("function ");
    cprintf("%s ", name);
  }
  ...
  if (flags & FUNC_EXTERNAL) result = remove_quoted_escapes(result);
...
static void newline(string) char *string;
{
  cprintf("\n");
  indent(indentation);
  if (string && *string) cprintf("%s", string);
}
...
static void indent(amount) int amount;
{
  ...
  for (i = 0; amount > 0; amount--) indentation_string[i++] = ' ';
...
static void the_printed_command_resize(length) int length;
{
  if (the_printed_command == 0) {
    ...
    new = (new + PRINTED_COMMAND_GROW_SIZE - 1) &
