    ...
    if (locale_mb_cur_max > 1) {                               \
      ...
      if (mblength == (size_t)-2 || mblength == (size_t)-1) {  \
        ...
        mblength = (mblength < 1) ? 1 : mblength;              \
                                                               \
      for (_k = 0; _k < mblength; _k++) *(_dst)++ = *(_src)++; \
    } else                                                     \
      *(_dst)++ = *(_src)++;                                   \
  } while (0)
#else
  ...
  do {                                                                    \
    if (locale_mb_cur_max > 1) {                                          \
      ...
      if (_k)                                                             \
        ...
        state_bak = state;                                                \
        mblength =                                                        \
            mbrlen((_src) + (_si), (_srcend) - ((_src) + (_si)), &state); \
      }                                                                   \
      if (mblength == (size_t)-2 || mblength == (size_t)-1) {             \
        state = state_bak;                                                \
        mblength = 1;                                                     \
        ...
        mblength = (mblength < 1) ? 1 : mblength;                         \
                                                                          \
      for (_k = 0; _k < mblength; _k++) _dst[_di++] = _src[_si++];        \
    } else                                                                \
      _dst[_di++] = _src[_si++];                                          \
  } while (0)
#else
...
#define SCOPY_CHAR_I(_dst, _escchar, _sc, _src, _si, _slen)         \
  do {                                                              \
    if (locale_mb_cur_max > 1) {                                    \
      ...
      temp[0] = _escchar;                                           \
      for (_i = 0; _i < mblength; _i++) temp[_i + 1] = _src[_si++]; \
      temp[mblength + 1] = '\0';                                    \
                                                                    \
      goto add_string;                                              \
    } else {                                                        \
...
#define SADD_MBCHAR(_dst, _src, _si, _srcsize)                         \
  do {                                                                 \
    if (locale_mb_cur_max > 1) {                                       \
      ...
      if (mblength < 1) mblength = 1;                                  \
                                                                       \
      _dst = (char *)xmalloc(mblength + 1);                            \
      for (i = 0; i < mblength; i++) (_dst)[i] = (_src)[(_si)++];      \
      (_dst)[mblength] = '\0';                                         \
                                                                       \
      goto add_string;                                                 \
    }                                                                  \
...
#define SADD_MBQCHAR_BODY(_dst, _src, _si, _srcsize)               \
  ...
  (_dst)[0] = CTLESC;                                              \
  for (i = 0; i < mblength; i++) (_dst)[i + 1] = (_src)[(_si)++];  \
  (_dst)[mblength + 1] = '\0';                                     \
                                                                   \
  goto add_string
