...
char **brace_expand(text) char *text;
{
  register int start;
  size_t tlen;
  ...
  if (i > 0) strncpy(preamble, text, i);
  preamble[i] = '\0';
  ...
  while (amble[j]) {
    ...
    if (amble[j] == brace_arg_separator) break;
...
add_tack:
  ...
  free(amble);
  if (tack != result) strvec_dispose(tack);
...
/* Expand the text found inside of braces.  We simply try to split the
   ...
   need it. */
static char **expand_amble(text, tlen, flags) char *text;
size_t tlen;
int flags;
{
  ...
  while (c) {
    ...
    if (!result)
      ...
      for (j = 0; j < lp; j++) result[lr + j] = partial[j];
...
static char **mkseq(start, end, incr, type, width) intmax_t start, end, incr;
int type, width;
{
  intmax_t n, prevn;
  int i, j, nelem;
  ...
  do {
    ...
    if (ADDOVERFLOW(n, incr, INTMAX_MIN, INTMAX_MAX)) break;

    n += incr;

    if ((incr < 0 && n < end) || (incr > 0 && n > end)) break;
  } while (1);
...
static char **expand_seqterm(text, tlen) char *text;
size_t tlen;
{
  char *t, *lhs, *rhs;
  ...
  if (lhs_t == ST_CHAR) {
    ...
    width = 0;
    if (lhs_l > 1 && lhs[0] == '0') width = lhs_l, lhs_t = ST_ZINT;
    ...
    if (width < lhs_l && lhs_t == ST_ZINT) width = lhs_l;
    if (width < rhs_l && lhs_t == ST_ZINT) width = rhs_l;
  }
...
*/
static int brace_gobbler(text, tlen, indx, satisfy) char *text;
size_t tlen;
int *indx;
int satisfy;
{
...
static int degenerate_array(arr) char **arr;
{
  register int i;
...
/* Return a new array of strings which is the result of appending each
   ...
   is returned. */
static char **array_concat(arr1, arr2) char **arr1, **arr2;
{
  ...
  if (arr2 == 0) return (arr1); /* XXX - caller expects us to free arr1 */
...
fatal_error(format, arg1, arg2) char *format, *arg1, *arg2;
{
  report_error(format, arg1, arg2);
  exit(1);
}

report_error(format, arg1, arg2) char *format, *arg1, *arg2;
{
...
main() {
  ...
  for (;;) {
    ...
    if (strlen(example)) example[strlen(example) - 1] = '\0';
