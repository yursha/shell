...
/* Okay, we have checked the mail.  Perhaps I should make this function
   go away. */
void reset_mail_timer() { last_time_mail_checked = NOW; }

/* Locate a file in the list.  Return index of
   entry, or -1 if not found. */
static int find_mail_file(file) char *file;
{
  register int i;

  for (i = 0; i < mailfiles_count; i++)
    if (STREQ(mailfiles[i]->name, file)) return i;
...
static int add_mail_file(file, msg) char *file, *msg;
{
  struct stat finfo;
  char *filename;
  int i;
  ...
  if (i >= 0) {
    if (mailstat(filename, &finfo) == 0) UPDATE_MAIL_FILE(i, finfo);
  ...
  i = mailfiles_count++;
  mailfiles =
      (FILEINFO **)xrealloc(mailfiles, mailfiles_count * sizeof(FILEINFO *));
...
void reset_mail_files() {
  ...
  for (i = 0; i < mailfiles_count; i++) RESET_MAIL_FILE(i);
}

static FILEINFO *alloc_mail_file(filename, msg) char *filename, *msg;
{
  FILEINFO *mf;
...
void free_mail_files() {
  ...
  for (i = 0; i < mailfiles_count; i++) dispose_mail_file(mailfiles[i]);

  if (mailfiles) free(mailfiles);
...
void init_mail_dates() {
  if (mailfiles == 0) remember_mail_dates();
}
...
static int file_mod_date_changed(i) int i;
{
  time_t mtime;
  struct stat finfo;
  char *file;
  ...
  if (mailstat(file, &finfo) != 0) return (0);

  if (finfo.st_size > 0) return (mtime < finfo.st_mtime);
...
static int file_access_date_changed(i) int i;
{
  time_t atime;
  struct stat finfo;
  char *file;
  ...
  if (finfo.st_size > 0) return (atime < finfo.st_atime);

  return (0);
}

/* Return non-zero if FILE's size has increased. */
static int file_has_grown(i) int i;
{
  off_t size;
  struct stat finfo;
  char *file;
...
static char *parse_mailpath_spec(str) char *str;
{
  char *s;
  int pass_next;
...
void check_mail() {
  ...
  if (dollar_underscore) dollar_underscore = savestring(dollar_underscore);

  for (i = 0; i < mailfiles_count; i++) {
    current_mail_file = mailfiles[i]->name;

    if (*current_mail_file == '\0') continue;
...
#define mtime mailfiles[i]->mod_time
      ...
      if ((atime >= mtime) && !file_is_bigger) continue;
