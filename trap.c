...
#define GETORIGSIG(sig)                                                     \
  do {                                                                      \
    ...
    if (original_signals[sig] == SIG_IGN) sigmodes[sig] |= SIG_HARD_IGNORE; \
  } while (0)
...
static char *trap_handler_string(sig) int sig;
{
...
int decode_signal(string, flags) char *string;
int flags;
{
  intmax_t sig;
  char *name;
...
#if defined(DEBUG)
    internal_warning(
        "run_pending_traps: recursive invocation while running trap for signal "
        "%d",
        running_trap - 1);
#endif
#if defined(SIGWINCH)
...
sighandler trap_handler(sig) int sig;
{
  int oerrno;
  ...
  if ((sig >= NSIG) || (trap_list[sig] == (char *)DEFAULT_SIG) ||
    ...
    if (interrupt_immediately) run_pending_traps();
...
int any_signals_trapped() {
  ...
  for (i = 1; i < NSIG; i++)
    if (sigmodes[i] & SIG_TRAPPED) return i;
  return -1;
}
...
char *command;
{
  if ((sigmodes[sig] & SIG_TRAPPED) == 0) set_signal(sig, command);
}
...
char *string;
{
  ...
  if (SPECIAL_TRAP(sig)) {
    ...
    if (sig == EXIT_TRAP && interactive == 0) initialize_terminating_signals();
    return;
  }
  ...
  if (sigmodes[sig] & SIG_HARD_IGNORE) return;

  /* Make sure we have original_signals[sig] if the signal has not yet
     been trapped. */
  if ((sigmodes[sig] & SIG_TRAPPED) == 0) {
    /* If we aren't sure of the original value, check it. */
    if (original_signals[sig] == IMPOSSIBLE_TRAP_HANDLER) GETORIGSIG(sig);
    if (original_signals[sig] == SIG_IGN) return;
  }
...
char *value;
{
  if ((sigmodes[sig] & SIG_INPROGRESS) == 0) free_trap_command(sig);
  trap_list[sig] = value;
  ...
  if (value == (char *)IGNORE_SIG)
    ...
    sigmodes[sig] &= ~SIG_IGNORED;
  if (sigmodes[sig] & SIG_INPROGRESS) sigmodes[sig] |= SIG_CHANGED;
}
...
void get_all_original_signals() {
  ...
  for (i = 1; i < NSIG; i++) GET_ORIGINAL_SIGNAL(i);
}
...
void ignore_signal(sig) int sig;
{
  ...
  if (sigmodes[sig] & SIG_IGNORED) return;

  /* Only change the signal handler for SIG if it allows it. */
  if ((sigmodes[sig] & SIG_NO_TRAP) == 0) set_signal_handler(sig, SIG_IGN);
...
static int _run_trap_internal(sig, tag) int sig;
char *tag;
{
...
#endif
    if (pipeline_pgrp > 0) give_terminal_to(pipeline_pgrp, 1);
    notify_and_cleanup();
#endif
...
void run_interrupt_trap(
    will_throw) int will_throw; /* from throw_to_top_level? */
{
  if (will_throw && running_trap > 0) run_trap_cleanup(running_trap - 1);
...
void free_trap_strings() {
  ...
  for (i = 0; i < NSIG; i++) {
    if (trap_list[i] != (char *)IGNORE_SIG) free_trap_string(i);
  }
...
static void reset_or_restore_signal_handlers(reset) sh_resetsig_func_t *reset;
{
  ...
  if (error_trace_mode == 0) sigmodes[ERROR_TRAP] &= ~SIG_TRAPPED;
}

/* Reset trapped signals to their original values, but don't free the
   trap strings.  Called by the command substitution code and other places
   that create a "subshell environment". */
void reset_signal_handlers() { reset_or_restore_signal_handlers(reset_signal); }
...
int maybe_call_trap_handler(sig) int sig;
{
...
int signal_is_pending(sig) int sig;
{ return (pending_traps[sig]); }

int signal_is_special(sig) int sig;
{ return (sigmodes[sig] & SIG_SPECIAL); }

int signal_is_ignored(sig) int sig;
...
int signal_in_progress(sig) int sig;
