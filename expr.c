...
static void pushexp() {
  ...
  if (expr_depth >= expr_stack_size) {
    ...
    expr_stack = (EXPR_CONTEXT **)xrealloc(
        expr_stack, expr_stack_size * sizeof(EXPR_CONTEXT *));
  }
...
static void expr_unwind() {
  while (--expr_depth > 0) {
    if (expr_stack[expr_depth]->tokstr) free(expr_stack[expr_depth]->tokstr);
...
intmax_t evalexp(expr, validp) char *expr;
int *validp;
{
  intmax_t val;
  int c;
...
static intmax_t subexpr(expr) char *expr;
{
  intmax_t val;
  char *p;
...
static intmax_t expassign() {
  ...
  if (curtok == EQ || curtok == OP_ASSIGN) {
    ...
    if (curlval.tokstr && curlval.tokstr == tokstr) init_lvalue(&curlval);
...
static intmax_t exp4() {
  ...
  val1 = expshift();
  while ((curtok == LEQ) || (curtok == GEQ) || (curtok == LT) ||
         (curtok == GT)) {
    int op = curtok;
...
static intmax_t ipow(base, exp) intmax_t base, exp;
{
  intmax_t result;
...
static intmax_t expr_streval(tok, e, lvalue) char *tok;
int e;
...
/* [[[[[ */
#if defined(ARRAY_VARS)
  v = (e == ']') ? array_variable_part(tok, (char **)0, (int *)0)
                 : find_variable(tok);
#else
  v = find_variable(tok);
#endif
  ...
  value = (e == ']') ? get_array_value(tok, 0, (int *)NULL, &ind)
...
static int _is_multiop(c) int c;
{
  switch (c) {
    case EQEQ:
    case NEQ:
...
static int _is_arithop(c) int c;
{
  switch (c) {
    case EQ:
    case GT:
  ...
  if (legal_variable_starter(c)) {
    ...
    *cp = c;
    ...
    if ((c == EQ) && (c1 == EQ))
      ...
      xp = cp;
      while (xp && *xp && cr_whitespace(*xp)) xp++;
...
static intmax_t strlong(num) char *num;
{
  register char *s;
...
void *xmalloc(n) int n;
{ return (malloc(n)); }

void *xrealloc(s, n) char *s;
int n;
{ return (realloc(s, n)); }
...
int builtin_error(format, arg1, arg2, arg3, arg4, arg5) char *format;
{
  fprintf(stderr, "expr: ");
