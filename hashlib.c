...
HASH_TABLE *hash_create(buckets) int buckets;
{
  HASH_TABLE *new_table;
  register int i;

  new_table = (HASH_TABLE *)xmalloc(sizeof(HASH_TABLE));
  if (buckets == 0) buckets = DEFAULT_HASH_BUCKETS;
...
int hash_size(table) HASH_TABLE *table;
{ return (HASH_ENTRIES(table)); }

static BUCKET_CONTENTS *copy_bucket_array(ba, cpdata) BUCKET_CONTENTS *ba;
sh_string_func_t *cpdata; /* data copy function */
{
  BUCKET_CONTENTS *new_bucket, *n, *e;

  if (ba == 0) return ((BUCKET_CONTENTS *)0);
...
HASH_TABLE *hash_copy(table, cpdata) HASH_TABLE *table;
sh_string_func_t *cpdata;
{
  HASH_TABLE *new_table;
  int i;

  if (table == 0) return ((HASH_TABLE *)NULL);
...
int flags;
{
  ...
  if (table == 0 || HASH_ENTRIES(table) == 0) return (BUCKET_CONTENTS *)NULL;
...
BUCKET_CONTENTS *hash_insert(string, table, flags) char *string;
HASH_TABLE *table;
int flags;
{
...
char *name;
{
  ...
  if (name == 0) name = "unknown hash table";
  ...
  for (slot = 0; slot < table->nbuckets; slot++) {
    ...
    for (bcount = 0; bc; bc = bc->next) bcount++;
...
main() {
  ...
  for (;;) {
    char *temp_string;
    if (fgets(string, sizeof(string), stdin) == 0) break;
    if (!*string) break;
