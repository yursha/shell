...
void phash_flush() {
  if (hashed_filenames) hash_flush(hashed_filenames, phash_freedata);
}
...
int phash_remove(filename) const char *filename;
{
  ...
  if (item) {
    if (item->data) phash_freedata(item->data);
    free(item->key);
    free(item);
    return 0;
  }
...
void phash_insert(filename, full_path, check_dot, found) char *filename,
    *full_path;
int check_dot, found;
{
  register BUCKET_CONTENTS *item;

  if (hashing_enabled == 0) return;

  if (hashed_filenames == 0) phash_create();
  ...
  if (check_dot) pathdata(item)->flags |= HASH_CHKDOT;
  if (*full_path != '/') pathdata(item)->flags |= HASH_RELPATH;
...
char *phash_search(filename) const char *filename;
{
  ...
  if (hashing_enabled == 0 || hashed_filenames == 0) return ((char *)NULL);

  item = hash_search(filename, hashed_filenames, 0);

  if (item == NULL) return ((char *)NULL);
  ...
  if (pathdata(item)->flags & (HASH_CHKDOT | HASH_RELPATH)) {
    ...
    if (executable_file(dotted_filename)) return (dotted_filename);
